...
sap.ui.define(["jquery.sap.global",
	...
	function(jQuery, Element, ParsingBackendMediator, RuleServices, resourcesConvertor, RtsInitiator, responseCollector, Locale, LocaleData, vocabularyConstants) {
		"use strict";
		/**
		 ...
		 */
		vocabularyConstants = vocabularyConstants.lib;
		...
		ExpressionLanguage.prototype.init = function() {
			...
			this._valueListName = "valueListName";
			this._ofModifier = " of the "
			this._initLocale();
		...
		ExpressionLanguage.prototype._setDataFromModel = function() {
			if (this.hasModel() && this.getBindingContextPath()) {
				this.getModel().read(this.getBindingContextPath(), {
					...
					error: this.makeJavaServiceCall()
				});
			}
		};

		ExpressionLanguage.prototype.makeJavaServiceCall = function() {
			this.isJavaService = true;
			if (this.hasModel() && this.getBindingContextPath()) {
				this.getModel().read(this.getBindingContextPath(), {
					urlParameters: {
		...
		ExpressionLanguage.prototype._validateRule = function(oRule, flags) {
			...
			if (bValueHelpValidation && !bCallback && !finalResponse.output.valueHelp.info[0].metadata.hasOwnProperty(this._hasValueSource)) {
		...
		ExpressionLanguage.prototype._validateExpression = function(sExpression, eType, bCollection, bToken, flags) {
			...
			var parserResult = this._backendParser.parseExpression(sExpression, sap.rules.ui.BackendParserRequest.Validate, this._runtimeService,
				null, eType, this._vocabularyName, flags);
		...
		ExpressionLanguage.prototype.validateExpression = function(sExpression, eType, bCollection, bToken) {
			...
			if (bValueHelpValidation) {
				...
				if ((typeof this.getValueHelpCallback()) !== "function" && !validationResult.valueHelp.info[0].metadata.hasOwnProperty(
						this._hasValueSource)) {
					this._raiseError(validationResult, "value help callback is not set or is not a function");

				} else if (validationResult.status === "Success" && !validationResult.valueHelp.info[0].metadata.hasOwnProperty(
						this._hasValueSource)) {
		 ...
		 * @public
		 **/
		ExpressionLanguage.prototype.getSuggestions = function(sExpression, eType, bCollection, bToken, valueListAttribute) {
			...
			var parserResult = this._backendParser.parseExpression(sExpression, sap.rules.ui.BackendParserRequest.Suggests, this._runtimeService,
				null, eType, this._vocabularyName, flags, valueListAttribute);
		...
		/**
		 *  Utility function to decide when given attribute is valueList, If yes valueListAttributeInfo will be returned
		 *  @param {string}   [attrId] Attribute Id.
		 *	@return {object}  [valueListInfo] returns valid valuelist attribute info for the given attribute
		 * */
		ExpressionLanguage.prototype.getValueListAttribute = function(attrId) {
			var valueListInfo;
			if (this._runtimeService) {
				var valueLists = this._runtimeService.getAllVocaObjects().allValueLists;
				if (valueLists) {
					for (var iterator = 0; iterator < valueLists.length; iterator++) {
						if (valueLists[iterator].name.indexOf(attrId) > 1) {
							valueListInfo = this._getValueAttrInfo(attrId);
							if (valueListInfo) {
								valueListInfo.attributeValueList = valueLists[iterator].name;
		...
		// Utility function ValueList attribute navigationPath
		ExpressionLanguage.prototype._getValueAttrInfo = function(attrId) {
			var attrArr = this._runtimeService.getAllVocaObjects().allAttr;
			for (var iterator = 0; iterator < attrArr.length; iterator++) {
				if (attrArr[iterator].hasOwnProperty(this._valueListName) && attrArr[iterator].valueListName && attrArr[iterator].valueListName.indexOf(
						attrId) > 1) {
					return {
						navPath: attrArr[iterator].name + this._ofModifier + attrArr[iterator].objectName
					};
				}
			}
		};
		...
		ExpressionLanguage.prototype.getExpressionMetadata = function(sExpression) { //always return status Success
			...
			var parserResult = this._backendParser.parseExpression(sExpression, sap.rules.ui.BackendParserRequest.GetMetadata, this._runtimeService,
				null, null, this._vocabularyName, null);
		...
		ExpressionLanguage.prototype.getResults = function () {
			...
			for (var i = 0; i < outputs.length; i++) {
				if (outputs[i].Type !== vocabularyConstants.PROPERTY_DATA_OBJECT_TYPE_ELEMENT) {
					var labelText = outputs[i].Label ? outputs[i].Label : outputs[i].name;
					results.push({
						id: outputs[i].id,
						name: outputs[i].name,
						label: labelText,
			...
			//handle for javaservice calls
			if (this.isJavaService || this._data.DataObjects) {
				outputs = this._data.DataObjects.results;
				for (var dataObj = 0; dataObj < outputs.length; dataObj++) {
					if (outputs[dataObj].Type !== vocabularyConstants.PROPERTY_DATA_OBJECT_TYPE_ELEMENT) {
		 ...
		 * @private
		 **/
		ExpressionLanguage.prototype.getResultColumnDescription = function(oName, outputID, oLabel) {
			if (!this._isDataExist()) {
		...
		ExpressionLanguage.prototype._getResultRequiredParamIds = function(oResultInfo) {
			...
			if (convertedData.DataObjects && convertedData.DataObjects.results) {
				...
				for (i = 0; i < convertedData.DataObjects.length; i++) {
					if (convertedData.DataObjects[i].Attributes && convertedData.DataObjects[i].Attributes.results && (convertedData.DataObjects[i].Name ==
							oResultInfo.name) && (convertedData.DataObjects[i].Usage == "RESULT") && oResultInfo.requiredParams) {
		...
		ExpressionLanguage.prototype._getSubExpressions = function(aTokens, expression) {
			...
			for (i = aTokens.tokens.length - 1; i >= 0; i--) {
				if (aTokens.tokens[i].info) {
					...
					if (oTokenInfo.category === sap.rules.ui.ExpressionCategory.comparisonOp || oTokenInfo.category === sap.rules.ui.ExpressionCategory.comparisonBetweenOp ||
						oTokenInfo.category === sap.rules.ui.ExpressionCategory.comparisonExistOp) {
		...
		ExpressionLanguage.prototype._getRightSuggestions = function(sLeft, sComp, currentValue, sCompType, oTokens) {
			...
			switch (sCompType) {
				case sap.rules.ui.ExpressionCategory.comparisonOp: //in case simple comparison
					aSuggestion = this._getRightForComparisonOp(sLeftBusinessDataType, sLefAndCompExprerssion, currentValue);
					break;
		...
		ExpressionLanguage.prototype._getRightForComparisonOp = function(sLeftBusinessDataType, sLefAndCompExprerssion, currentValue) {
		...
		ExpressionLanguage.prototype._geRightForBetweenOp = function(sLeftBusinessDataType, sLefAndCompExprerssion, currentValue) {
			...
			(function() {
				...
				if (aTokens) {
					for (var i = 0; i < aTokens.length; i++) {
						if (aTokens[i].info && (aTokens[i].info.category === sap.rules.ui.ExpressionCategory.comparisonOp || aTokens[i].info.category ===
								sap.rules.ui.ExpressionCategory.comparisonBetweenOp || aTokens[i].info.category === sap.rules.ui.ExpressionCategory.comparisonExistOp
							)) {
							currentValue = "";
							return;
						}
			...
			for (i = 0; i < oSuggestion.length; i++) { //put value in suggestions and set the 'to' suggestion
				if (filtered[i] === "to" || (i === 2 && sLeftBusinessDataType === sap.rules.ui.ExpressionType.TimeSpan) //logic for putting the 'to'\'and' at currentValue
					|| (i === 1 && sLeftBusinessDataType === sap.rules.ui.ExpressionType.Date) || ((i === 1 && sLeftBusinessDataType !== sap.rules.ui.ExpressionType
						.Date) && (i === 1 && sLeftBusinessDataType != sap.rules.ui.ExpressionType.TimeSpan))) {
		...
		ExpressionLanguage.prototype.convertDecisionTableExpressionToModelValue = function(sHeaderExpression, sFixedOpr, sCellExpression,
			...
			var response = this._validateDecisionTableExpression(sHeaderExpression, sFixedOpr, sCellExpression, eExpressionType, flags,
				"RuleServiceValidation", sExpressionLanguageVersion);
			return response;
		};

		ExpressionLanguage.prototype._validateDecisionTableExpression = function(sHeaderExpression, sFixedOpr, sCellExpression, eExpressionType,
			flags, sResponseCollectorStatus, sExpressionLanguageVersion) {
			...
			var result = RuleServices.lib.validateDecisionTableExpression(sHeaderExpression, sFixedOpr, sCellExpression, eExpressionType, this._vocabularyName,
				this._runtimeService, sExpressionLanguageVersion, flags);
