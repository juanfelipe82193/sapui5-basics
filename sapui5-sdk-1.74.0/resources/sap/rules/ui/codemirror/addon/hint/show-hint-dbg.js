(function() {
	...
	var CUSTOM_DATE_PICKER = 'CustomDate';
	var VALUEHELP_CLASS = "CodeMirror-hint-valueHelp";
	var DATE_SUGGESSTION_TEXT = "<'M/d/yy'>";
	var DATE_PICKER_TEXT = "Select Date using Calendar";
	...
	Completion.prototype = {
		setNewCursorPosition: function(completion, cm) {
			...
			if (completion === ANY_OF + NEW_STRUCT_COMPLITION || completion === NEW_STRUCT_COMPLITION || completion === ALL_OF +
				NEW_STRUCT_COMPLITION) {
		...
		pick: function(data, i, keyPressed) {
			...
			var bCalendarPick = data.list[i].tokenType === CUSTOM_DATE_PICKER;

			if (!bValueHelp && !bCalendarPick) {
			...
			} else if (bCalendarPick) {
				this._setModal(true);
				CodeMirror.signal(this.cm, "onChangeDate");
			}
	...
	function Widget(completion, data) {
		...
		var isValueHelpPresent = false;
		var valueHelpIndex;
		var businessDataType = "";
		var iterator;
		for (iterator = 0; iterator < completions.length; iterator++) {
			if (completions[iterator].tokenType ===  VALUE_HELP_TOKEN_TYPE) {
				valueHelpIndex = iterator;
				isValueHelpPresent = true;
				businessDataType = completions[iterator].info.metadata.businessDataType;
			} else if (completions[iterator] && completions[iterator].info && completions[iterator].info.type && completions[iterator].info.type ===
                DATE_TYPE) {
				businessDataType = "Date";
			}
		}
		if (isValueHelpPresent || businessDataType === 'Date') {
			var completionDuplicate = [];
			if (isValueHelpPresent) {
				completionDuplicate.push(completions[valueHelpIndex]);
			}
			if (businessDataType === 'Date') {
				var datePick = {};
				datePick.text = DATE_PICKER_TEXT;
				datePick.tokenType = 'CustomDate';
				completionDuplicate.push(datePick);
			}
			for (iterator = 0; iterator < completions.length; iterator++) {
				if (completions[iterator].tokenType ===  VALUE_HELP_TOKEN_TYPE) {
					continue;
				}
				completionDuplicate.push(completions[iterator]);
			}
			completions = completionDuplicate;
		}
		data.list = completions;
		for (var i = 0; i < completions.length; ++i) {
			...
			if (completions[i].tokenType ===  VALUE_HELP_TOKEN_TYPE || completions[i].text === DATE_PICKER_TEXT) {
				className = className + " " + VALUEHELP_CLASS;
			}
