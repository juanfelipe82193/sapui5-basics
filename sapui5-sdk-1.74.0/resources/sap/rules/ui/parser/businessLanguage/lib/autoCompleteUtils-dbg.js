...
sap.rules.ui.parser.businessLanguage.lib.autoCompleteUtils.lib = (function() {
	...
	autoCompleteUtilsLib.prototype.overwriteParserReportError = function(parser, parsingOutputInfo) {
		parser.reportError = function(A) {
			...
			function setParserContext() {
				...
				parser.isInsideWhere = ((parser.isInsideWhere !== undefined && parser.isInsideWhere === true) ? true : parseModelLib.getModelManger()
					.isInsideWhere);
				parser.disableAliases = ((parser.disableAliases !== undefined && parser.disableAliases === true) ? true : parseModelLib.getModelManger()
					.disableAliases);
	...
	autoCompleteUtilsLib.prototype.overwriteLexerReportError = function(lexer, parser, parsingOutputInfo) {
		lexer.reportError = function(A) {
			if (parser.lexerErrorOccurred === false) {
				...
				if ((A instanceof org.antlr.runtime.MismatchedTokenException || A instanceof org.antlr.runtime.MismatchedSetException || A instanceof org
						.antlr.runtime.EarlyExitException) && A.c === -1) {
	...
	autoCompleteUtilsLib.prototype.disableToken = function(vocaRTServices, parser, tokenName, tokenID, modelManger) {
		...
		var tokenMap = parseUtilsLib.getTokensDisplayMap(modelManger);
		diableToken =
			((tokenName === tokenMap.DOT && parseUtilsLib.endsWith(this.parsingOutputInfo.lastText.toLowerCase(), ' ')) || (tokenName ===
					tokenMap.COUNT_OF) || (tokenName === tokenMap.MAXIMUM_OF) || (tokenName === tokenMap.MINIMUM_OF) || (tokenName === tokenMap.FIRST) ||
				(tokenName === tokenMap.LAST) || (tokenID === parserTokens.CURRENT && !parser.isInsideWhere) || (tokenName === tokenMap.EQ_SIGN) ||
				(tokenName === tokenMap.NOT_EQUAL_SIGN) || (tokenName === tokenMap.GREATER_EQUAL_SIGN) || (tokenName === tokenMap.LESS_EQUAL_SIGN) ||
				(tokenName === tokenMap.GREATER_SIGN) || (tokenName === tokenMap.LESS_SIGN) || (tokenName === tokenMap.EQUALS) || (tokenName ===
					tokenMap.DOES_NOT_EQUAL) || (tokenName === tokenMap.NOT_EQUAL) || (tokenName === tokenMap.ZERO_OR_ONE) || (tokenID === parserTokens
					.CURRENT && this.parsingOutputInfo.contextInfo.isDeprecatedAllContext === false && !this.parsingOutputInfo.contextInfo.isTermAllContext
				) || (tokenID === parserTokens.ALL_OF_OBJ) || (tokenID === parserTokens.ALL && !vocaRTServices.isTermModifierEmpty(parseModelLib.getModelManger()
					.vocabulary)) || (tokenID === parserTokens.OF && !vocaRTServices.isTermModifierEmpty(parseModelLib.getModelManger().vocabulary)) ||
	...
	autoCompleteUtilsLib.prototype.disableDateTimeFormatsForString = function(tokenName, nextTokensArray, modelManger) {
		...
		if (utilsLib.isInArray(parserTokens.TYPESTRING.toString(), nextTokensArray) &&
			!((utilsLib.isInArray(parserTokens.TYPETIMESTAMP.toString(), nextTokensArray)) || (utilsLib.isInArray(parserTokens.TYPEDATE.toString(),
				nextTokensArray)) || (utilsLib.isInArray(parserTokens.TYPETIME.toString(), nextTokensArray)))) {
			diableToken =
				((tokenName === tokenMap.DATESTRING) || (tokenName === tokenMap.TIMESTAMPSTRING) || (tokenName === tokenMap.TIMESTRING));
		}
	...
	autoCompleteUtilsLib.prototype.getTokenName = function(tokenID, parser) {
		...
		if ((tokenName === tokenMap.DATESTRING) || (tokenName === tokenMap.TIMESTAMPSTRING) || (tokenName === tokenMap.TIMESTRING)) {
			return tokenName;
		}
	...
	autoCompleteUtilsLib.prototype.disableNumericFragements = function(tokenName, nextTokensArray, modelManger) {
		...
		if (utilsLib.isInArray(parserTokens.ZERO_TO_ONE.toString(), nextTokensArray) &&
			((utilsLib.isInArray(parserTokens.INT.toString(), nextTokensArray)) || (utilsLib.isInArray(parserTokens.DECIMAL.toString(),
				nextTokensArray))))
		...
		if (this.navigationCompletionInfo.navigationTextInput.indexOf('.') !== -1 &&
			...
			if (this.parsingOutputInfo.contextInfo.contextNavigation.navigation.indexOf(startInput) === 0) {
				...
				if (tokenArray.length > 2) {
					var contextNavDetails = vocabularyUtilLib.validateNavigationDetails(this.navigationCompletionInfo.navigationTextInput,
						vocaRTServices, parseModelLib.getModelManger().vocabulary, this.navigationCompletionInfo.collectionBusinessTypes, this.parsingOutputInfo
						.contextInfo.valueListAttribute);
	...
	autoCompleteUtilsLib.prototype.completeNavSuggestions_dotMode = function(vocaRTServices, parser, prefix) {
		...
		navigationDetails = vocabularyUtilLib.validateNavigationDetails(this.navigationCompletionInfo.navigationTextInput, vocaRTServices,
			parseModelLib.getModelManger().vocabulary, this.navigationCompletionInfo.simpleBusinessTypes, this.parsingOutputInfo.contextInfo.valueListAttribute
		);
		...
		if (navigationDetails.isValid === false) {
			if (navigationDetails.root.isValid === false) {
				...
				this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(vocaRTServices, this.navigationCompletionInfo.navAlternatives,
					navigationDetails.root.name, prefix, this.navigationCompletionInfo.commaDelimiter, null);
				this.navigationCompletionInfo.navAlternatives = [];
			} else { // Example: player.L
				...
				this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(vocaRTServices, navigationDetails.attribute.altAttr.concat(
						this.navigationCompletionInfo.navAlternatives), navigationDetails.attribute.name, prefix, this.navigationCompletionInfo.commaDelimiter,
					null);
				this.navigationCompletionInfo.navAlternatives = [];
			}
		}
	 ...
	 *
	 ************************************************************************************************************************************/
	autoCompleteUtilsLib.prototype.completeNavSuggestions_termsMode = function(vocaRTServices, parser, prefix, lastToken, valueListAttribute) {
		// represents
		...
		var lastValueListAttribute = this.parsingOutputInfo.contextInfo.valueListAttribute;

		if (!this.parsingOutputInfo.contextInfo.valueListAttribute && valueListAttribute) {
			lastValueListAttribute = valueListAttribute;
		}
		...
		if (this.navigationCompletionInfo.navAlternatives.length > 0) {
			this.navigationCompletionInfo.navSuggestions += this.insertOMAlternatives(vocaRTServices, this.navigationCompletionInfo.navAlternatives,
				this.navigationCompletionInfo.navigationTextInput, prefix, this.navigationCompletionInfo.commaDelimiter, termMode);
			this.navigationCompletionInfo.navAlternatives = [];
		}
	...
	autoCompleteUtilsLib.prototype.isDotMode = function(vocaRTServices) {
		...
		return (this.navigationCompletionInfo.navigationTextInput.indexOf('.') !== -1 ||
			...
			(parseModelLib.getModelManger().flags.hasOwnProperty(constantsLib.propertiesEnum.disableTerms) && parseModelLib.getModelManger().flags[
				constantsLib.propertiesEnum.disableTerms]));
	};
	 ...
	 *
	 ************************************************************************************************************************************/
	autoCompleteUtilsLib.prototype.addNavigationSuggestions = function(vocaRTServices, parser, valueListAttribute) {
		...
		if (this.parsingOutputInfo.navigationCompleteion) {
			...
			this.parsingOutputInfo.contextInfo.isDeprecatedAllContext = this.parsingOutputInfo.contextInfo.isDeprecatedAllContext || lastToken ===
				parserTokens.ALL;
			...
			if (this.isTermMode(vocaRTServices)) {
				this.completeNavSuggestions_termsMode(vocaRTServices, parser, prefix, lastToken, valueListAttribute);
			}
	...
	autoCompleteUtilsLib.prototype.addNonNavigationSuggestions = function(selectedTokens, completionPrefix) {
		...
		for (idx = 0; idx < selectedTokens.length; idx++) {
			...
			if (sgtCompletion) {
				...
				this.strSuggestions += '{"text":"' + /*current_prefix +*/ tokenName + '","completion":"' + /*current_prefix +*/ completionPrefix +
					sgtCompletion + '","tokenType":"' + tokenTypeObject.group + '","info":' + JSON.stringify(tokenTypeObject.info) + '},';
	 ...
	 *
	 ***********************************************************************************************************************************/
	autoCompleteUtilsLib.prototype.getNextSuggestions = function(input, lexer, parser, vocaRTServices, type, valueListAttribute) {
		...
		this.addNavigationSuggestions(vocaRTServices, parser, valueListAttribute);
