...
sap.rules.ui.parser.businessLanguage.lib.vocabularyUtils.lib = (function() {
	...
	****************************************************************/
	 ...
	 vocabularyUtilsLib.prototype.validateNavigationDetails = function(navDetails, vocaRTServ, vocaName, typeArr, valueListAttribute, modelMngrTermMode) {
		 var result = {
				...
				if (pathSegments.length > 1) {
                    result.navigationFullPathId = this.getNavigationPathById(vocaRTServ, navDetails);
                }
				if (pathSegments.length < 1) {
					result.root.altDO    = this.getAltTypedObjects (vocaRTServ, vocaName, '', typeArr);
					result.root.altAlias = this.getAltTypedAliases (vocaRTServ, vocaName, '', typeArr);
	...
	vocabularyUtilsLib.prototype.getNavigationPathById = function(vocaRTServ, navDetails) {
        var navigationPathId = "";
        if (vocaRTServ && navDetails) {
            var pathSegments = navDetails.split(".");
            var dataObjectsList = vocaRTServ.allVocaObjects.allObjects;
            var associationList = vocaRTServ.allVocaObjects.allAssoc;
            var attributeList = vocaRTServ.allVocaObjects.allAttr;
            var rootObjectId = "";
 
            if (associationList && associationList.length > 0) {
                //gets the objectid in the association and appends with a "."
                for (var oSegment in pathSegments){
				   for (var object in dataObjectsList) {
					if (dataObjectsList[object].name === pathSegments[oSegment]) {
						rootObjectId = dataObjectsList[object].id;
						navigationPathId = navigationPathId + dataObjectsList[object].id + ".";
					}
				  }	
				}
 
                // following dataobject id will be an attribute id
                var objCount = pathSegments.length;
				var attrName = pathSegments[objCount-1];
                for (var attribute in attributeList) {
                    if (attrName === attributeList[attribute].name && rootObjectId === attributeList[attribute].objId) {
                        navigationPathId = navigationPathId + attributeList[attribute].id;
                    }
                }
 
            } else {
                // get the data object id from the list of data object attributes
                var objectName = pathSegments[0];
                for (var obj in dataObjectsList) {
                    if (objectName === dataObjectsList[obj].name) {
                        rootObjectId = dataObjectsList[obj].id;
                        navigationPathId = navigationPathId + dataObjectsList[obj].id + ".";
                    }
                }
 
                // following dataobject id will be an attribute id
                var attributeName = pathSegments[1];
                for (var attr in attributeList) {
                    if (attributeName === attributeList[attr].name && rootObjectId === attributeList[attr].objId) {
                        navigationPathId = navigationPathId + attributeList[attr].id;
                    }
                }
            }
