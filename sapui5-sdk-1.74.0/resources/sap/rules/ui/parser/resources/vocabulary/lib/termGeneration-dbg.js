...
sap.rules.ui.parser.resources.vocabulary.lib.termGeneration.lib = (function() {
	...
	termGenerationLib.prototype.generate = function(rtAll, dataObjectArrContent, vocaName, vocabularyId, dbConn, isPrivate, resourceID,
		scope, writeDependecies) {
		...
		function pluralize(string) {
			...
			if (rx.test(lastWord)) {
				if (endsWith(lastWord, 'y') && !isStartWithVowel(lastWord.slice(-2))) {
					...
					//country -> countries
					lastWord = lastWord.slice(0, -1) + 'IES';
				} else if (endsWith(lastWord, 's') || endsWith(lastWord, 'z') || endsWith(lastWord, 'ch') || endsWith(lastWord, 'Ch') || endsWith(
						lastWord, 'sh') || endsWith(lastWord, 'Sh') || endsWith(lastWord, 'x')) {
		...
		function addTermParamArray(termDescriptions, expression, modifiers, businessDataType, isCollection, isConditionalContext, context,
			vocabularyId, isVocaRule) {
		...
		function generateBaseTermForAssociation(dataObject, visitedAssociationsArr, termExpression, attrInfo, termContext, isCollection,
			vocabularyId) {
			...
			if (attrInfo === null && isCollection) {
				...
				addTermParamArray(termDescription, termExpression, modifiers, attrInfo.businessDataType, isCollection, false, termContext,
					vocabularyId);
		...
		function generateTermForAllModifierForAssociation(dataObject, assoc, visitedAssociationsArr, termExpression, attrInfo, termContext,
			vocabularyId) {
		...
		function generateTermForNotModifierForAssociation(dataObject, visitedAssociationsArr, termExpression, attrInfo, termContext,
			isCollection, vocabularyId) {
			...
			if (attrInfo !== null && attrInfo.businessDataType === vocabularyConstants.BUSINESS_DATA_TYPE_BOOLEAN) {
				...
				if (byDescModeIsOn) {
					friendlyDesc = contextualAssociationChain.friendlyDesc + suffix4FriendlyTerm + vocabularyConstants.TERM_IS_NOT_STRING + attrInfo[
						descProperty];
				}
				termDescription = {
					"description": description,
		...
		function generateTermForCurrentModifierForAssociation(dataObject, assoc, visitedAssociationsArr, termExpression, attrInfo, termContext,
			isCollection, vocabularyId) {
			...
			if (attrInfo === null && isCollection) {
				...
				if ((associationChain.deprecated && associationChain.deprecated[0]) || isDeprecatedPlural(assocName)) {
					...
					deprecatedArr[0] = associationChain.deprecated && associationChain.deprecated[0] && associationChain.deprecated[0].length ? ' ' +
						associationChain.deprecated[0] : associationChain.description;
				...
				addTermParamArray(termDescription, termExpression, modifiers, attrInfo.businessDataType, isCollection, true, termContext,
					vocabularyId);
		...
		function generateTermsForDOAssociationAttribute(dataObject, visitedAssociationsArr, assoc, attrInfo, isCollection, vocabularyId) {
			...
			if (!type) {
				generateTermForAllModifierForAssociation(dataObject, assoc, visitedAssociationsArr, termExpression, attrInfo, termContext,
					vocabularyId);
			} else if (type.toLowerCase() === vocabularyConstants.PROPERTY_DATA_OBJECT_TYPE_TABLE) {
				generateTermForAllModifierForAssociation(dataObject, assoc, visitedAssociationsArr, termExpression, attrInfo, termContext,
					vocabularyId);
			}
			//Generate a Term with NOT modifier for DO Association Attribute.
			generateTermForNotModifierForAssociation(dataObject, visitedAssociationsArr, termExpression, attrInfo, termContext, isCollection,
				vocabularyId);
			//Generate a Term with CURRENT modifier for DO Association Attribute.
			generateTermForCurrentModifierForAssociation(dataObject, assoc, visitedAssociationsArr, termExpression, attrInfo, termContext,
				isCollection, vocabularyId);
		}

		//Generates Terms for a DO Association.
		function generateTermsForDOAssociation(dataObject, visitedAssociationsArr, assocInfo, vocabularyFullName, isSoFarCollection,
			vocabularyId, dbConn, dependenciesList) {
			var i, j;
			var attrArr;
			var attrInfo;
			var assocArr;
			...
			if (!assocWasVisited(visitedAssociationsArr, assocInfo)) {
				...
				generateTermForCurrentModifierForAssociation(dataObject, assocInfo, visitedAssociationsArr, termExpression, null, null, isCollection,
					vocabularyId);
				...
				for (i = 0, j = assocArr.length; i < j; i++) {
					generateTermsForDOAssociation(dataObject, visitedAssociationsArr, assocArr[i], vocabularyFullName, isCollection, vocabularyId,
						dbConn, dependenciesList);
				}
		...
		function generateTermForAllModifier(dataObject, attrInfo, termExpression, vocabularyId) {
			...
			if (attrInfo !== null || attrInfo.hasOwnProperty("name")) {
				attName = attrInfo.name;
				termDescription.description = attName + vocabularyConstants.TERM_OF_STRING + vocabularyConstants.TERM_ALL_STRING + pluralize(
					dataObjectName);
				//Generate a corresponding FriendlyTerm:
				if (byDescModeIsOn) {
		...
		function generateTermForCurrentModifier(dataObject, attrInfo, termExpression, vocabularyId) {
			...
			if (attrInfo !== null || attrInfo.hasOwnProperty("name")) {
				...
				if (attDatatype === vocabularyConstants.BUSINESS_DATA_TYPE_BOOLEAN) {
					...
					termDescription.description = vocabularyConstants.TERM_CURRENT_STRING + dataObjectName + vocabularyConstants.TERM_IS_STRING +
						attName;
					//Generate a corresponding FriendlyTerm:
					if (byDescModeIsOn) {
					...
					modifiers = createModifiersObj(false, true, true);
					termDescription.description = vocabularyConstants.TERM_CURRENT_STRING + dataObjectName + vocabularyConstants.TERM_IS_NOT_STRING +
						attName;
					//Generate a corresponding FriendlyTerm:
					if (byDescModeIsOn) {
					...
					modifiers = createModifiersObj(false, false, true);
					termDescription.description = attName + vocabularyConstants.TERM_OF_STRING + vocabularyConstants.TERM_CURRENT_STRING +
						dataObjectName;
					//Generate a corresponding FriendlyTerm:
					if (byDescModeIsOn) {
		...
		function generateTermsForDOAttribute(startDO, attrInfo, termExpression, vocabularyId) {
            if (attrInfo.businessDataType !== vocabularyConstants.BUSINESS_DATA_TYPE_GEOMETRY) {
            	//Generate a Term for the boolean attribute.    For example: Player is whale.
				...
				if (attrInfo.businessDataType === vocabularyConstants.BUSINESS_DATA_TYPE_BOOLEAN) {
					...
					var type = startDO.type;
					if (!type) {
						// default case: Allow ALL modifier terms generation
						generateTermForAllModifier(startDO, attrInfo, termExpression, vocabularyId);
					} else if (type.toLowerCase() === vocabularyConstants.PROPERTY_DATA_OBJECT_TYPE_TABLE) {
						// Allow All modifier if Type is table
						generateTermForAllModifier(startDO, attrInfo, termExpression, vocabularyId);
					}
				}
	        }
		...
		function generateTerms(dataObjectArrContent, vocabularyFullName, vocabularyId, dbConn, isPrivate, resourceID) {
			...
			for (i = 0, j = dataObjectArrContent.length; i < j; ++i) {
				if(dataObjectArrContent[i].type !== vocabularyConstants.PROPERTY_DATA_OBJECT_TYPE_ELEMENT){
			...
			if (writeDependecies) {
				dependency = dependencyUtilsLib.createDependenciesInVocabulary(dependencyConstantsLib.CATEGORY_VOCA_TERMS, vocabularyFullName,
					dependenciesList, isPrivate, dbConn);
