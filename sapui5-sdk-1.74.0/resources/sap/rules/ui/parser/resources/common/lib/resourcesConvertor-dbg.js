...
sap.rules.ui.parser.resources.common.lib.resourcesConvertor.lib = (function() {
	...
	function convertToDataObjectAttribute(attrObj, convertedAttributesArr, parentDOname, validationUtilsLib, hasCPValSource, cpVocabId) {
		...
		if (hasCPValSource && attrObj.hasOwnProperty(oDataHandlerLib.CP_HAS_VALUE_SOURCE) && attrObj[oDataHandlerLib.CP_HAS_VALUE_SOURCE] === true) {
			newConvertedAttrObj.valueList = cpVocabId + "_" + attrObj[oDataHandlerLib.CP_DATOBJECT_ID] + "_" + attrObj[oDataHandlerLib.PROPERTY_NAME_ID];
			validationUtilsLib.validateValueHelpBusinessDataType(newConvertedAttrObj.valueList, businessDataType);
		}
	...
	function convertoDataCPVH(oDataVL, convertedVocaJson, validationUtilsLib) {
		var newValueHelp = {};
		var cpVocabId = getOdataPropertyValue(oDataVL, oDataHandlerLib.CP_VOCABULARY_ID);
		var cpDataobjectId = getOdataPropertyValue(oDataVL, oDataHandlerLib.CP_DATOBJECT_ID);
		var cpAttrId = getOdataPropertyValue(oDataVL, oDataHandlerLib.CP_ATTRIBUTE_ID);
		var cpValueHelpId  = cpVocabId + "_" + cpDataobjectId + "_" + cpAttrId;
		var businessDataType = validationUtilsLib.getValueHelpBusinessDataType(cpValueHelpId );
		newValueHelp.businessDataType = businessDataType;
		newValueHelp.metadata = {};
	    newValueHelp.metadata.vocabularyId = cpVocabId;
	    newValueHelp.metadata.attributeId = cpAttrId;
	    newValueHelp.metadata.dataObjectId = cpDataobjectId;
		newValueHelp.metadata[oDataHandlerLib.CP_HAS_VALUE_SOURCE] = true;
		var sourceType = getOdataPropertyValue(oDataVL, oDataHandlerLib.CP_VALUE_SOURCE_TYPE);
        if (sourceType === oDataHandlerLib.CP_VALUE_SOURCE_TYPE_STATIC) {
            newValueHelp.metadata.serviceURL = "/Enumerations"; 
            newValueHelp.metadata.entitySet = "Enumerations";
        }else{
            newValueHelp.metadata.serviceURL = "/ExternalValues"; 
            newValueHelp.metadata.entitySet = "ExternalValues";
        } 
		convertedVocaJson.valueLists[cpValueHelpId] = newValueHelp;
	...
	function convertoDataDO(oDataVocaDOarr, oDataDO, convertedVocaJson, vocaId, validationUtilsLib, hasCPValSource, cpVocabId) {
		...
		if (propName) {
			newHRF_Entity.name = getOdataPropertyValue(oDataDO, oDataHandlerLib.PROPERTY_NAME_NAME);
			// New type added to DataObject to Restrict Terms generation based on DataObject type
			propType = getOdataPropertyValue(oDataDO, oDataHandlerLib.PROPERTY_NAME_TYPE);
			if (propType) {
				newHRF_Entity.type = propType;
			}
			if (oDataDO[propName] === oDataHandlerLib.TYPE_DO) {
				...
				if (propName) {
					...
					attributesArr.forEach(function(attrObj) {
						if (hasCPValSource) {
							convertToDataObjectAttribute(attrObj, newHRF_Entity.attributes, newHRF_Entity.name, validationUtilsLib, hasCPValSource,
								cpVocabId);
						} else {
	...
	var convertVocabularyODataToInternalModel = function(oDataObj, termModes) {
		...
		propName = getOdataPropName(oDataObj, oDataHandlerLib.CP_VALUE_SOURCE);
		if (propName) {
			hasCPValSource = true;
		}
		...
		if (propName) {
			oDataObj[propName].forEach(function(oDataDO) {
				if (hasCPValSource) {
					convertoDataDO(oDataObj[propName], oDataDO, convertedVocaJson, oDataObj.id, validationUtilsLib, hasCPValSource, oDataDO.VocabularyId);
				} else {
		...
		if(hasCPValSource){
			...
			if (propName && Array.isArray(oDataObj[propName])) {
				oDataObj[propName].forEach(function(oDataVH) {
					convertoDataCPVH(oDataVH, convertedVocaJson, validationUtilsLib);
				});
			}
		}
		//voca rules
