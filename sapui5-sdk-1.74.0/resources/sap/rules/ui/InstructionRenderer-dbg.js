sap.ui.define([
	...
	"sap/rules/ui/providers/ValueHelpDialogProvider",
    ...
    TimePicker, ValueHelpProvider, ValueHelpDialogProvider, LocaleData, Constants) {
    "use strict";
    ...
    return Control.extend("sap.rules.ui.InstructionRenderer", {
        ...
        _createValueListControl: function(instruction, nextCreateFunction, localPath, fullPath) {
			var that = this;
            this.valueHelpCtrl = new Input({
                showValueHelp: true,
				id: "valueHelpCtrl",
                valueHelpRequest: function() {
                    if (that.valueHelpCtrl.oValueHelpDialogProvider) {
                        that.valueHelpCtrl.oValueHelpDialogProvider._onInitialise();
                    } else {
						ValueHelpDialogProvider._createCpValueHelp(instruction, 0, false, false);
					}
                },
                value: {
            ...
            if (oValueListAnnotation) {
                this.valueHelpCtrl.oValueHelpDialogProvider = new ValueHelpProvider({
                ...
                if (oPopover instanceof sap.m.Popover) {
                    this.valueHelpCtrl.oValueHelpDialogProvider._popover = oPopover;
                }
            }
            
            
            this.valueHelpCtrl.addStyleClass(this.TOKENS_GAP_CLASS);

            return {
                controls: [this.valueHelpCtrl],
                needLineBreak: false
            };
        },
	    ...
	    if (model) {
                newValue = model.getProperty(localPath + this.TEXT);
                if (model.oData && model.oData.instructions) {
                    var instructions = model.oData.instructions;
                    for (var instruction in instructions) {
                        businessDataType = instructions[instruction].businessDataType;
                    }
            ...
            if (businessDataType === Constants.STRING || businessDataType === Constants.DATE_BUSINESS_TYPE || businessDataType === Constants.TIMESTAMP_BUSINESS_TYPE || businessDataType === "Time") {
				if (businessDataType === Constants.DATE_BUSINESS_TYPE) {
					newValue = this.oFormatDate.format(this.oFormatDate.parse(newValue));
				}
