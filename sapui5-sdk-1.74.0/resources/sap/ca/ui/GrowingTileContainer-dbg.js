 ...
 */
sap.m.ScrollContainer.extend("sap.ca.ui.GrowingTileContainer", /** @lends sap.ca.ui.GrowingTileContainer.prototype */ { metadata : {
    ...
    if (oBindingInfo) {
        if (this._iItemCount == 0 || sChangeReason == sap.ui.model.ChangeReason.Filter) {
            if (this.getGrowing()) {
                ...
                this._iItemCount = oBindingInfo.binding.getLength();
            }
        }
...
// remove check for length >0
        ...
        if (jQuery.sap.domById(sListId) != null) {
            ...
            if (aContexts.diff) {
                ...
                for (var i = 0, l = aContexts.diff.length; i < l; i++) {
                    ...
                    if (aContexts.diff[i].type === "delete") {
                        this.deleteListItem(aContexts.diff[i].index);
                    }
            ...
            } else if( !aContexts.dataRequested) {
 ...
 */
sap.ca.ui.GrowingTileContainer.prototype.deleteListItem = function (nIndex) {
    if (nIndex != null) {
        this._iRenderedDataItems--;
        jQuery(jQuery(this._getItemsContainer()).children()[nIndex]).remove();
    }
    return this;
};
