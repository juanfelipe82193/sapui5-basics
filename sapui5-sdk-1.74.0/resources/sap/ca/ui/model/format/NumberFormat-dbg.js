 ...
 *
 *
 * @deprecated Since version 1.28. This control has been made available in sap.ui.core. Please use {@link sap.ui.core.format.NumberFormat} instead! This control will not be supported anymore.
 */
...
sap.ca.ui.model.format.NumberFormat.prototype.format_short = function (oValue) {
	...
	function getShortFormat(value) {
		...
		if (shortCldrFormats) {
			...
			var shortCldrFormat = shortCldrFormats[key.toString() + "-other"];
			shortFormat = {};
...
sap.ca.ui.model.format.NumberFormat.prototype.parse = function(sValue) {
	...
	// During input, we can't really expect user to type non-breaking spaces  "\u00a0"
	// so when grouping separator is a non-breaking space (russian and french for example),
	// we replace all typed whitespaces with non-breaking spaces.
	if (formatter.oFormatOptions.groupingSeparator == "\u00a0") {
		sValue = sValue.replace(/ /g,formatter.oFormatOptions.groupingSeparator);
	}
 ...
 *
 * @param {string} sValue the string containing a value
...
sap.ca.ui.model.format.NumberFormat.prototype.format_percentage = function(sValue) {
	...
	if (!percentCldrFormats){
		...
		sdLocale = new sap.ui.core.Locale("en");
