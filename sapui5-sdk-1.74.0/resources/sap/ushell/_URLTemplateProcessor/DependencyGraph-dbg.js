...
/**
 * @fileOverview Methods to understand in what order template parameters shall
 * be resolved.
 *
 *
 * Because values of URL Template Parameters can refer to the value of other
 * parameters, a proper order for finding the value of parameters must be
 * established (e.g., by resolving parameters without a depending parameters
 * first).  The proper order of evaluation can be determined by
 * creating and visiting a graph that represents parameters and depending parameters
 * as nodes. This graph can be also used to detect cyclic references in the
...
sap.ui.define([
    ...
    function getDependencyResolutionOrder (oDependencyGraphReadOnly) {
        var oDependencyGraph = fnClone(oDependencyGraphReadOnly);
