...
sap.ui.define([
    ...
    "sap/base/Log"
], function (mobileLibrary, cache, jQuery, Log) {
    "use strict";
    ...
    sap.ui.controller("tiles.indicatorDualComparison.DualComparison", {
                ...
                if (!cachedValue) {
                    ...
                    this.comparisionChartODataRef = finalQuery.model.read(finalQuery.uri, null, null, true, function (data) {
                        if (data && data.results && data.results.length) {
                            ...
                            } cache.setKpivalueById(that.oConfig.TILE_PROPERTIES.id, that.writeData);
                            fnSuccess.call(that, that.oConfig.TILE_PROPERTIES.FINALVALUE);
                        } else if (data.results.length === 0) {
                            that.oConfig.TILE_PROPERTIES.FINALVALUE = data;
                            that.writeData.data = data; cache.setKpivalueById(that.oConfig.TILE_PROPERTIES.id, that.writeData);
                            fnSuccess.call(that, that.oConfig.TILE_PROPERTIES.FINALVALUE);
                        } else {
        ...
        flowWithDesignTimeCall: function () {
            var that = this;
            try {
                var evaluationData = cache.getEvaluationById(this.oConfig.EVALUATION.ID);
                if (evaluationData) {
                    ...
                    sap.ushell.components.tiles.indicatorTileUtils.util.getFilterFromRunTimeService(this.oConfig, function (filter) {
                        that.oConfig.EVALUATION_FILTERS = filter; cache.setEvaluationById(that.oConfig.TILE_PROPERTIES.id, that.oConfig);
                        that.flowWithoutDesignTimeCall();
                    });
                }
            } catch (e) {
                    ...
                    this.oChip.configuration.getParameterValueAsString("tileConfiguration"), this.oChip.preview.isEnabled(), function (config) {
                        ...
                        if (that.oChip.preview.isEnabled()) {
                            ...
                            that.oDualComparisonView.oGenericTile.attachPress(function () {
                                sap.ushell.components.tiles.indicatorTileUtils.util.abortPendingODataCalls(that.comparisionChartODataRef); cache.setKpivalueById(that.oConfig.TILE_PROPERTIES.id, null);
