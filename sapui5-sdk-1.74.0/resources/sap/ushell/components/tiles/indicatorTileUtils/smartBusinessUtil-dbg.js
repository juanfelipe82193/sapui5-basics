    ...
    sap.ushell.components.tiles.indicatorTileUtils.util = (function (global, $) {
        ...
        return {
            ...
            isCacheValid: function (chipId, chipLastUpdatedOn, cacheMaxAge, cacheMaxAgeUnit, tilePressed) {
                var cachedValue = cache.getKpivalueById(chipId);
                if (!cachedValue) {
                    return false;
            ...
            getFrontendCache: function (oConfig, oTileApi) {
                var that = this;
                var oldDeferred = cache.getFrontEndCacheDeferredObject(oTileApi.url.getApplicationSystem());
                var deferred;
                if (oldDeferred) {
                ...
                var cachedKpiValue = cache.getKpivalueById(chipId);
                if (cachedKpiValue) {
                    oldDeferred.resolve(true);
                } else if (oldDeferred) {
                    return oldDeferred.promise();
                } else {
                    cache.setFrontEndCacheDefferedObject(oTileApi.url.getApplicationSystem(), deferred);
                                ...
                                frontendModel.read(cacheQueriUri, null, null, true, function (data) {
                                    if (data && data.results && data.results instanceof Array && data.results.length) {
                                        jQuery.each(data.results, function (index, cachedData) {
                                            cache.setKpivalueById(cachedData.ChipId, cachedData);
                                        });
                                    }
                                    if (cache.getKpivalueById(chipId)) {
                                        isCached = true;
                                    }
            ...
            getHanaClient: function () {
                ...
                var success = function (d, s, x) {
                    sessionClient = d.sessionClient; cache.setCacheHanaClient(sessionClient);
                ...
                var error = function (jqXHR, textStatus, errorThrown) {
                    ...
                    Log.error("session client call failed"); cache.setCacheHanaClient(null);
                ...
                var cacheHanaClient = cache.getCacheHanaClient();
