...
sap.ui.define([
    ...
    "sap/base/Log"
], function (generic, cache, mobileLibrary, jQuery, Log) {
    "use strict";
                    ...
                    if (that.kpiValueFetchDeferred) {
                        ...
                        this.comparisionChartODataRef = finalQuery.model.read(finalQuery.uri, null, null, true, function (data) {
                            ...
                            if (data && data.results && data.results.length) {
                                ...
                                that.oConfig.TILE_PROPERTIES.FINALVALUE = data;
                                if (cache.getKpivalueById(that.oConfig.TILE_PROPERTIES.id)) {
                                    writeData = cache.getKpivalueById(that.oConfig.TILE_PROPERTIES.id);
                                    writeData.data = data;
                                } else {
                                    writeData.data = data;
                                } cache.setKpivalueById(that.oConfig.TILE_PROPERTIES.id, writeData);
                                fnSuccess.call(that, that.oConfig.TILE_PROPERTIES.FINALVALUE);
                                that.setNoData();
                            } else {
