...
sap.ui.define([], function () {
    ...
    return function JsSearch(options) {
        ...
        if (!options.algorithm || !options.algorithm.id || !Array.isArray(options.algorithm.options) ||
            ...
            var toBeReturned = {
                'contains-ranked': {
                    ...
                    compileSearchTerm: function (searchTerm) {
                        ...
                        var st = {
                            original: searchTerm.normalize ? searchTerm.normalize('NFC') : searchTerm, // original string (unicode normalized)
                            parsed: [],
                        ...
                        /** @param {mapTypeFunction} f */ // with same type as type
                        ...
                        //
                        st.parsed = mapType(st.parsed, 'character', /** @param {Character} c */ function (c, i, pre, succ) {
                            var r = c.cOriginal;

                            if (r === '*') { // * wildcard  -  may be treated specially in next steps
                                r = '[^\\s]*?';
                            } else if (r === '?') { // ? wildcard  -  may be treated specially in next steps
                                r = '[^\\s]';
                            } else if (r.match(/\\[*?"\\]/)) { //  \* \? \\
                            } else {
                                r = r.replace(/[\[\]\/\{\}\(\)\+\.\\\^\$\|]/, "\\$&");
                        ...
                        mapType(st.parsed, 'word', /** @param {Word} w */ function (w) {
                            ...
                            for (var i = 0; i < w.children.length; i++) {
                                ...
                                if (i < w.children.length - 1) { // if not last character
                            ...
                            var last = '';
                            if (c.cOriginal === '*') { // if * at end, highlight till end of word
                        ...
                        var compiledRegExps = st.parsed.map(function (s) {
                            ...
                            if (s.type === 'phrase') {
                                ...
                                if (!s.children.length) { // phrase consisted just of * wildcards
                                    ret.push({
                                        r: '^',
                                        score: opt[0],
                                    ...
                                    mapType(s.children, 'word',
                                        ...
                                        function (word, i, pre, succ) {
                                            ...
                                            } else if (lastCharacterOriginal(pre).match(regExps.Alpha_atBegin) && firstCharacterOriginal(succ).match(regExps.Alpha_atBegin)) { //  letter * letter
                                                middle += '(?:[^a]|[^a].*?[^a])';
                                            } else if (lastCharacterOriginal(pre).match(regExps.Num_atEnd) && firstCharacterOriginal(succ).match(regExps.Num_atEnd)) { //  number * number
                                                middle += '(?:[^0]|[^0].*?[^0])';
                                            } else if (lastCharacterOriginal(pre).match(regExps.Alpha_atBegin) && firstCharacterOriginal(succ).match(regExps.Num_atEnd)) { //  letter * number
                                                middle += '.*?';
                                            } else if (lastCharacterOriginal(pre).match(regExps.Num_atEnd) && firstCharacterOriginal(succ).match(regExps.Alpha_atBegin)) { //  number * letter
                                                middle += '.*?';
                                            } else if (lastCharacterOriginal(pre) === '*' && succ && firstCharacterOriginal(succ).match(regExps.Alpha_atBegin)) { //  xxx* * letter
                                                middle += '.*?';
                                            } else if (lastCharacterOriginal(pre) === '*' && succ && firstCharacterOriginal(succ).match(regExps.Num_atEnd)) { //  xxx* * letter
                                                middle += '.*?';
                                            } else {
                                                middle += '(?:[^a0]|[^a0].*?[^a0])';
                                            }
                                ...
                                if (s.wOriginal.match('^\\*+$')) { // is * wildcard or **
                                    containsJustSimpleWords = false;
                                    ret.push({
                                        r: '^',
                                        score: opt[0],
                    ...
                    compileTestTerm: function (testTerm) {
                        ...
                        tt.original = testTerm.normalize ? testTerm.normalize('NFC') : testTerm; // original string (unicode normalized)
