    ...
    CollisionModule.prototype = {
        ...
        removeTilePlaceholder: function (oGroup, bAnimateRemoveTilePlaceholder) {
            ...
            if (bAnimateRemoveTilePlaceholder) { // we have single tile in last row
                ...
                this.collidedLinkAreas = [];
                jQuery(this.currentGroup.getDomRef()).find(".sapUshellTilesContainer-sortable").animate({ height: height }, 300);
            }
        },
        ...
        reorderTilesInDom: function () {
            ...
            if (this.endArea !== "links") {
                ...
                if (Config.last("/core/home/gridContainer")) {
                    if (jqTargetGroup[0].firstElementChild.children.length) {
                        jqTargetGroup[0].firstElementChild.insertBefore(jsSelectedTile, jqGroupTiles[destTileIndex]);
                    } else {
                        jqTargetGroup[0].firstElementChild.appendChild(jsSelectedTile);
                    }
                } else {
        ...
        layoutStartCallback: function (element) {
            ...
            if (Config.last("/core/home/gridContainer") && this.startGroup.getParent().getId() !== "dashboardGroups") {
                // this is only the gird not the grid container
                this.startGroup = this.currentGroup = this.startGroup.getParent();
            }
            this.groupSwitched = false;
                    ...
                    if (innerContainerElements[j] && this._isElementCollideByGivenCordinates(innerContainerElements[j], moveX, moveY)) {
                        ...
                        for (var idx = this.groupsList.length - 1; idx >= i; idx--) {
                            ...
                            if (!this.groupsList[idx].groupHasTiles()) {
                                var jQLastGroupExpanded = this.groupsList[idx].$();
                                jQLastGroupExpanded.removeClass("sapUshellEnableTransition");
                                jQLastGroupExpanded.find(".sapUshellInner").removeAttr("style");
                            }
                        }
                        return oGroup;
                    }
        ...
        _getTabBarHoverItem: function (moveX, aTabBarItemsLocation) {
            ...
            iLeftTabBarOffset = jQuery(".sapUshellAnchorItem:not(.sapUshellShellHidden)").eq(0).offset().left;
