...
(function () {
    ...
    var fnDefine = (!window.sap || !sap.ui || !sap.ui.define)
        ? function (aVoidDeps, fnCallback) { return fnCallback(); }
        : sap.ui.define;

    return fnDefine([], function () {
        var _oHub = {
        ...
        function log (sMessage, sDetails, fnCausedBy) {
            if (jQuery && jQuery.sap && jQuery.sap.log && jQuery.sap.log.error) {
                jQuery.sap.log.error(sMessage, sDetails, "sap.ushell.EventHub");
                return;
            }

            if (window.console) {
                var aArgs = [sMessage, sDetails];
                if (fnCausedBy) {
                    aArgs.push("error caused by:");
                    aArgs.push(fnCausedBy);
                }
                window.console.error.apply(window.console, aArgs);
            }
        }
         ...
         */
        function emit (oHub, sEvent, oData) {
