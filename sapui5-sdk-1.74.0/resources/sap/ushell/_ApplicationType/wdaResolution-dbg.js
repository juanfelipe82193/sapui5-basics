...
sap.ui.define([
...
], function (URI, oApplicationTypeUtils, oSystemAlias, jQuery, ObjectPath) {
    "use strict";
    ...
    function buildWdaURI (sApplicationId, sConfigId, bIsWDAInCompatibilityMode, oOtherParameters, fnExternalSystemAliasResolver) {
        ...
        return oSystemAlias.spliceSapSystemIntoURI(oURI, oSystemAlias.LOCAL_SYSTEM_ALIAS, sSapSystem, sSapSystemDataSrc, "WDA", oSystemAlias.SYSTEM_ALIAS_SEMANTICS.apply, fnExternalSystemAliasResolver);
    }
    ...
    function constructWDAResolutionResult (oMatchingTarget, sBaseUrl, fnExternalSystemAliasResolver) {
        ...
        return Promise.all([
            (new Promise(function (fnDone, fnReject) {
                oSystemAlias.spliceSapSystemIntoURI(oWDAURI, oInboundResolutionResult.systemAlias, sSapSystem, sSapSystemDataSrc, "WDA", oInboundResolutionResult.systemAliasSemantics || oSystemAlias.SYSTEM_ALIAS_SEMANTICS.applied, fnExternalSystemAliasResolver)
                    .fail(fnReject)
                    .done(fnDone);
            })),
    ...
    function resolveEasyAccessMenuIntentWDA (oIntent, oMatchingTarget, fnExternalSystemAliasResolver) {
        var sConfId,
            sAppId,
            oOtherParams,
            bCompatibilityMode = true; // For easy access menus product decided to generate compatible URLs for now.

        sAppId = oIntent.params["sap-ui2-wd-app-id"][0];
        sConfId = (ObjectPath.get("params.sap-ui2-wd-conf-id", oIntent) || [])[0];
        ...
        return new Promise(function (fnResolve, fnReject) {
            buildWdaURI(sAppId, sConfId, bCompatibilityMode, oOtherParams /* may include sap-system and/or sap-system-src */, fnExternalSystemAliasResolver)
                .done(function (oURI) {
