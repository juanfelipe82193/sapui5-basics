...
sap.ui.define([
    ...
    "sap/ui/core/Icon"
], function (resources, encodeXML, Icon) {
    "use strict";
    ...
    TileBaseRenderer.highlight = function (aHighlightTerms, sText) {
        ...
        if (aHighlightTerms && aHighlightTerms.length && aHighlightTerms.length > 0) {
            for (i = 0; i < aHighlightTerms.length; i = i + 1) {
                ...
                regexpHighlightTerm = new RegExp("(" + encodeXML(aHighlightTerms[i]).replace(/([.*+?^=!:${}()|[\]/\\])/g, "\\$1") + ")", "gi");
    ...
    TileBaseRenderer.render = function (oRm, oControl) {
        ...
        oRm.writeAccessibilityState(oControl, {
            label: translationBundle.getText("tileTypeListItem") + translationBundle.getText("TileDetails_lable") + translationBundle.getText("TileTitle_lable") + oControl.getTitle()
        });
        oRm.write(">");
