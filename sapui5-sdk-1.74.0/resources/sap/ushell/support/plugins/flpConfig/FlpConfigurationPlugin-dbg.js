 ...
 *
 * Adds a plugin to the SAPUI5 Diagnsotics tool to show the startup configuration (window["sap-ushell-config"]) of the
 * Fiori launchpad in a tree table seperated by IconTabFilters for the first level properties of the configuration object.
...
sap.ui.define([
    ...
    var that = this,
        ol18nModel,
        flpConfigPlugin = Plugin.extend("sap.ushell.support.plugins.flpConfig.FlpConfigurationPlugin", {
            constructor: function (oSupportStub) {
                // get i18n files and create model
                ol18nModel = new ResourceModel({ bundleName: "sap/ushell/support/plugins/flpConfig/i18n/FlpConfigurationPlugin" });

                // base constructor
                Plugin.apply(this, ["sapUiFlpConfigurationPlugin",
                    ol18nModel.getResourceBundle().getText("PLUGIN_NAME"), oSupportStub]);
    ...
    flpConfigPlugin.prototype.init = function (oSupportStub) {
        ...
        if (this.runsAsToolPlugin()) {
            ...
            // delay call to get FlpConfig and build UI because of asynchronos event handling
            // event to receive data is not accessable immediately
            setTimeout(function () {
                that.requestData();
            }, 500);
            ...
            oRenderManager.write("<div id='" + this.getId() + "'</div>").flush(this.$().get(0));
    ...
    flpConfigPlugin.prototype.onsapUiFlpConfigurationPluginReceiveData = function (oEvent) {
        ...
        // now build UI
        sap.ui.getCore().loadLibraries(["sap.ui.table"], { async: true }).then(function () {
            sap.ui.require(["sap/ui/table/TreeTable", "sap/ui/table/Column", "sap/ui/table/library"], function (TreeTable, Column, tableLibrary) {
                ...
                that.getFlpConfigurationPluginUI(null, TreeTable, Column, SelectionMode);
            });
        });
    };
    ...
    function _onPressExpand (/*oEvent*/) {
        var iHierarchyCount = 0,
            sSelectedKey = sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").getSelectedKey(),
            oFlpConfig = sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").getModel().getData();
        ...
        if (Array.isArray(oFlpConfig[sSelectedKey])) {
            ...
            if (iHierarchyCount > 0) {
                ...
                sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TreeTable" + sSelectedKey).expandToLevel(iHierarchyCount - 1);
            } else {
                MessageBox.error(new Label({
                    text: sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar")
    ...
    function _onPressCollapse (/*oEvent*/) {
        var sSelectedKey = sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").getSelectedKey();
        sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TreeTable" + sSelectedKey).collapseAll();
    }

    // handles search event for input field
    flpConfigPlugin.prototype.onPressSearchNewModel = function (oEvent) {
        var aPathsForOneProperty, aPropertyIntTmp, aPropertyIntCopy, aAllProperties, aProperty, aPropertyInt, oPropertyTmp, sSelectedKey, oValueForPath,
            aMissingValues = [],
            oTabBarFilter = {},
            sPath = "",
            oFlpConfigSearch = {},
            oLevelOneHierarchyPaths = {},
            oFlpConfig = sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").getModel("Full"),
        ...
        if (sSearchQuery) {
            ...
            sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").setModel(new JSONModel(oFlpConfigSearch));
            sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").setModel(new JSONModel(oFlpConfig), "Full");
            ...
            Object.keys(oTabBarFilter).forEach(function (sLevelOneProp) {
                ...
                // minus 1 because last hierarchy level does not need to expanded but counts because its a hierarchy level as well
                sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TreeTable" + sLevelOneProp).expandToLevel(iHierarchyCount - 1);
                // set numbers of properties in UI
                sap.ui.getCore().byId(sLevelOneProp).setCount(_getPropertyCount(oFlpConfigSearch[sLevelOneProp], sLevelOneProp, 0));
            });
        } else {
            // set numbers of properties in UI
            Object.keys(oTabBarFilter).forEach(function (sLevelOneProp) {
                sap.ui.getCore().byId(sLevelOneProp).setCount(_getPropertyCount(oFlpConfig[sLevelOneProp], sLevelOneProp, 0));
            });
            sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar").setModel(new JSONModel(oFlpConfig));
        }
    };
    ...
    function _searchForProperties (oCurrentObject, sProperty, sPath, aPathsFound, sSearchQuery) {
        oCurrentObject.forEach(function (element, index) {
            ...
            if (element[sProperty]) {
                ...
                // check if propertyname contains searchquery
    ...
    function _createNewConfigJSON (oCurrentObject, sPropertyKey) {
        ...
        return Object.keys(oCurrentObject).map(function (sPropertyName) {
            ...
            if (typeof (oCurrentObject[sPropertyName]) === "object" && Object.keys(oCurrentObject[sPropertyName]).length !== 0) {
                oResult[sPropertyKey] = _createNewConfigJSON(oCurrentObject[sPropertyName], sPropertyKey);
                return oResult;
            }
            if (!isEmptyObject(oCurrentObject[sPropertyName]) || typeof (oCurrentObject[sPropertyName]) !== "object") {
                oResult.value = oCurrentObject[sPropertyName];
            } else {
                oResult.value = "";
            }
            return oResult;
     ...
     *   - it prepares the data for being sent by the plugin framwork
     */
    ...
    // build new config JSON with structure usable for databinding
    ...
    flpConfigPlugin.prototype.getFlpConfigurationPluginUI = function (foo, TreeTable, Column, SelectionMode) {
        ...
        // create config object for databinding in tree table
        ...
        // if someone refreshs the diagnostics tool, the event will be triggered for every refresh
        // so do not build a new UI or set any model in case everything is already there
        if (!sap.ui.getCore().byId("sap-ui-support-flpConfigurationPlugin-TabBar")) {
            // place an error message on UI if there is no config
            if (!oFlpConfigNew || isEmptyObject(oFlpConfigNew)) {
                new MessageStrip({
                    text: ol18nModel.getResourceBundle().getText("ERROR_MESSAGE"),
                    type: "Warning",
                    showIcon: true,
                ...
                oTabBar.setModel(ol18nModel, "i18n");
