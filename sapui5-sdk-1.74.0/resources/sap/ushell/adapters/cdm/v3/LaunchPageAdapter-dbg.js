...
sap.ui.define([
    ...
    function LaunchPageAdapter (oUnused, sParameter, oAdapterConfiguration) {
        ...
        this._getCatalogTileView = function (oCatalogTile) {
            ...
            if (typeof oCatalogTile.tileResolutionResult !== "object") {
                oResolvedTile = this._mResolvedTiles[oCatalogTile.id];
            } else {
                oResolvedTile = oCatalogTile;
            }

            return this._getTileUiComponentContainerSync(oCatalogTile, oResolvedTile, true);
        };
            ...
            oCdmSiteService.getSite()
                .done(function (oSite) {
                    ...
                    oCdmSiteService.save()
                        ...
                        .fail(function (sError) {
                            jQuery.sap.log.error(sGenericErrorMessage + sError);
                            oDeferred.reject(sGenericErrorMessage + sError);
                        });
                    return undefined;
                })
                ...
                var aCatalogTiles = ((oCatalog.payload && oCatalog.payload.viz) || [])
                    .reduce(function (aReturnedCatalogTiles, sVisualizationId) {
                        ...
                        if (sAppId) {
                            ...
                            if (!that._isStartableInbound(oInboundResult.inbound) || that._isHiddenInbound(oInboundResult.inbound)) {
                                return aReturnedCatalogTiles;
                            }
        ...
        function updateTileComposition (oTile, oParameters, oTarget) {
            ...
            if (oParameters.serviceRefreshInterval || oParameters.serviceRefreshInterval === 0) {
        ...
        this._getSiteData = function () {
            return sap.ushell.Container.getService("CommonDataModel").getSite();
        };
    ...
    LaunchPageAdapter.prototype = Object.create(AdapterBase.prototype);
