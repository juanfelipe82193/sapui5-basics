...
sap.ui.define([
    ...
    var oLogger = jQuery.sap.log.getLogger("sap/ushell/adapters/cdm/LaunchPageAdapter");
    var oLogLevel = jQuery.sap.log.Level;
    ...
    AdapterBase.prototype._ensureLoaded = function () {
        ...
        this._getSiteData()
            ...
            .fail(function (sErrorMessage0) {
                oLogger.error("Delivering hompage groups failed - " + sErrorMessage0);
    ...
    AdapterBase.prototype._resolveTileByVizId = function (oTile, oSite) {
        ...
        if (!sVisualizationId) {
            return reject(oLogLevel.ERROR, "Cannot resolve tile '" + oTile.id + "': vizId must be specified");
        }

        oVisualization = oReadVisualization.get(oSite, sVisualizationId);
        if (!oVisualization) {
            return reject(oLogLevel.ERROR, "Cannot resolve tile '" + oTile.id + "': no visualization found for vizId '" + sVisualizationId + "'");
        }
        ...
        // Resovle tile via app id in visualization target
        ...
        if (sAppId) {
            ...
            if (!oConfig) {
                return reject(oLogLevel.INFO, "Tile '" + oTile.id + "' filtered from result: Neither target app nor configuration given in visualization '" + sVisualizationId + "'");
            }
    ...
    AdapterBase.prototype.getTileIndicatorDataSource = function (oTile) {
        ...
        if (oTile.indicatorDataSource) {
            oResult.indicatorDataSource = oUshellUtils.clone(oTile.indicatorDataSource);
            if (oTile.dataSource) {
                oResult.dataSource = oUshellUtils.clone(oTile.dataSource);
            }
        ...
        if (oResolutionResult.indicatorDataSource) {
            ...
            oResult.indicatorDataSource = oUshellUtils.clone(oResolutionResult.indicatorDataSource);
            ...
            if (oResolutionResult.indicatorDataSource.hasOwnProperty("dataSource")) {
                ...
                if (oDataSources && oDataSources.hasOwnProperty(sDataSourceName)) {
                    oResult.dataSource = oUshellUtils.clone(oDataSources[sDataSourceName]);
                } else {
                    jQuery.sap.log.warning("datasource referenced but not found for tile: " + oResolvedTile.tileIntent);
                }
            }
    ...
    AdapterBase.prototype.getCardManifest = function (oGroupCard) {
        var oResolutionResult,
            oResolvedCard = this._mResolvedTiles[oGroupCard.id];

        oResolutionResult = oResolvedCard.tileResolutionResult;
     ...
     * component for catalog tiles.
     * Helper function for _getCatalogTileView
     * function. It works synchronously. It is deprecated. Use asynchronous version instead.
     *
     *
     * @param {object} oTile
     *  Ccatalog tile
     ...
     * @private
     */
    AdapterBase.prototype._getTileUiComponentContainerSync = function (oTile, oResolvedTile, bIsCatalogTile) {
        var that = this,
            oResolutionResult,
            oTileComponentData,
            sNavigationMode,
        ...
        if (typeof oResolutionResult.tileComponentLoadInfo === "object" &&
        ...
        if (oConfig.name) {
            ...
            oCompContainer = new sap.ui.core.ComponentContainer({ // TODO: pending dependency migration
    ...
    AdapterBase.prototype._getTileUiComponentContainer = function (oTile, oResolvedTile, bIsCatalogTile) {
        ...
        if (oResolvedTile.tileResolutionResult.isPlatformVisualization) {
            // Let the platform specific logic create the view for this visualization
            sap.ushell.Container.getService("VisualizationDataProvider").getVisualizationView(oTile.vizId).then(function (oView) {
    ...
    AdapterBase.prototype._createTileComponentProperties = function (oTileComponentData, oTileComponentLoadInfo) {
        ...
        if (!oTileComponentLoadInfo) {
            return oTileComponentProperties;
        }

        if (isEmptyObject(oTileComponentLoadInfo)) {
    ...
    AdapterBase.prototype.isSiteSupported = function (oSite) {
        ...
        if (!oSite._version ||
            Version(oSite._version).compareTo(this.getCdmVersionsSupported().min) < 0 ||
            Version(oSite._version).compareTo(this.getCdmVersionsSupported().max) > 0) {
            jQuery.sap.log.fatal("Invalid CDM site version: Only version 3.0.0 is supported");
            return false;
        }
        return true;
    };
