...
// eslint-disable-complexity no-alert
...
sap.ui.define([
    ...
    function transform (sUri, sUriParent, sUriBase, sUriNewBase) {
        ...
        if (!isRelative(oUriBase) && !isRelative(oUriNewBase)) {
            ...
            var oUriNewBaseToBaseRelative = directoryPath(oUriBase).relativeTo(directoryPath(oUriNewBase));
            var oUriAbsolute;
    ...
    // Determines the directory from a uri and adds a trailing '/'
    function directoryPath ( oUri ) {
        return URI.joinPaths(oUri.directory(), '/');
    }
