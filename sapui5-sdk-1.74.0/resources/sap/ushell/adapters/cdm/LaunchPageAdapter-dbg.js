...
sap.ui.define([
    ...
    "use strict";
    /* eslint-disable consistent-this */ // TODO: remove eslint-disable
    /* global window Promise hasher */
    ...
    function LaunchPageAdapter (oUnused, sParameter, oAdapterConfiguration) {
        ...
        // https://sapui5.netweaver.ondemand.com/sdk/#docs/api/symbols/Log.html#.error
        ...
        this._getTileFromHashFromSite = function (oSite, sIntent) {
            var oCstrService = sap.ushell.Container.getService("ClientSideTargetResolution");
            var aInboundsFromSite = oUtilsCdm.formatSite(oSite);
        ...
        this._assureLoaded = function () {
            ...
            oCDMSiteService.getSite()
                ...
                .fail(function (sErrorMessage0) {
                    Log.error("Delivering hompage groups failed - " + sErrorMessage0);
                    ...
                    .done(function (oSite) {
                        oReadHomePageUtils.getGroupsArrayFromSite(oSite).forEach(function (oGroup) {
                            oModifyHomePageUtils.setGroupVisibility(oGroup,
                                aHiddenGroupIds && // group is not in list of hidden groups -> visiblity === true
        ...
        this.resetGroup = function (oGroup) {
            ...
            if (typeof oGroup === "object" && oReadHomePageUtils.getGroupId(oGroup)) {
                ...
                oCdmSiteService.getSite()
                    .done(function (oSite) {
                        jQuery.extend(true, aSiteGroupsBackup, oReadHomePageUtils.getGroupsArrayFromSite(oSite));
        ...
        this._getTileUiComponentContainer = function (oTile, oResolvedTile, bIsCatalogTile) {
            ...
            var _customTileCreateCompContainer = function () {
            ...
            if (bIsCustomTile) {
                oEventHub.once("CoreResourcesComplementLoaded")
                    .do(function () {
                        _customTileCreateCompContainer()
        ...
        /**
         * Creates the component data needed to inst±antiate tiles
         *
         * @param {object} oTile
        ...
        this._addTileToSite = function (oPersonalizedSite, oGroup, oNewTile, oCdmSiteService) {
            ...
            var that = this,
                oDeferred = new jQuery.Deferred(),
                oIntent = sap.ushell.Container.getService("URLParsing").parseShellHash(oNewTile.properties.targetURL),
                oTileToBeAdded = {
                ...
                var aCatalogTiles = ((oCatalog.payload && oCatalog.payload.appDescriptors) || [])
                    .reduce(function (aReturnedCatalogTiles, oAppID) {
                        ...
                        if (that._isStartableInbound(oFirstInbound) && !that._isHiddenInbound(oFirstInbound)) {
                            ...
                            oInbound.tileResolutionResult.navigationMode = navigationMode.computeNavigationModeForHomepageTiles(sApplicationType, sAdditionalInformation, bIsApplicationTypeConfiguredInPlace);
         ...
         *  Catalog tile
         * @returns {object}
         *  returns an object of a given catalog tile
         */
            ...
            if (oTile && oTile.id && oSettingsView) {
                ...
                oCdmSrvc.getSite()
                    .done(function (oSite) {
                        ...
                        } else if (that._mResolvedTiles[oTile.id].linkTileControl) {
        ...
        this.getTileActions = function (oTile) {
            ...
            if (this._isGroupTile(oTile) && !this._isFailedGroupTile(oTile)) {
                ...
                oTileSettingsAction = utilsRT.getTileSettingsAction(oModel, this._onTileSettingsSave.bind(this, oTile), this.getTileType(oTile));
