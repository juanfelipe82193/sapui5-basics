    ...
    function UsageAnalytics (oContainerInterface, sParameter, oServiceProperties) {
        ...
        this.setTrackUsageAnalytics = function (bState) {
            var deferred = jQuery.Deferred(),
                ...
                currentUserTracking = bState,
                oldUserTracking = sap.ushell.Container.getUser().getTrackUsageAnalytics();

            try {
                var userInfoService = sap.ushell.Container.getService("UserInfo");
                if (oldUserTracking !== currentUserTracking) { //only if there was a change we would like to save it

                    sap.ushell.Container.getUser().setTrackUsageAnalytics(currentUserTracking);
                    oUserPreferencesPromise = userInfoService.updateUserPreferences(sap.ushell.Container.getUser());

                    oUserPreferencesPromise.done(function () {
                        sap.ushell.Container.getUser().resetChangedProperty("trackUsageAnalytics");
                        if (!bInitialized && currentUserTracking) {
                            this.start();
                        } else if (bInitialized && currentUserTracking === false) {
                            window.swa.disable();
                        } else if (bInitialized && currentUserTracking) {
                            window.swa.enable();
                        }
                        deferred.resolve();
                    }.bind(this));
                    ...
                    oUserPreferencesPromise.fail(function (sErrorMessage) {
                        sap.ushell.Container.getUser().setTrackUsageAnalytics(oldUserTracking);
                        sap.ushell.Container.getUser().resetChangedProperty("trackUsageAnalytics");
                ...
                deferred.reject("Getting UserInfo service failed.");
            }
        ...
        this.showLegalPopup = function () {
            sap.ui.require(["sap/m/Text", "sap/m/Dialog", "sap/m/Button"], function (Text, Dialog, Button) {
                var dialog =
                    new Dialog("agreementMessageBox", {
         ...
         */
        this.start = function () {
            sap.ui.getCore().getEventBus().publish("sap.ushell.services.UsageAnalytics", "usageAnalyticsStarted");
            this._initUsageAnalyticsLogging();
            bInitialized = true;
            //setting the laguages for all requests
            window.swa.custom2 = { ref: sap.ui.getCore().getConfiguration().getLanguage() };
        };
