...
sap.ui.define([
    ...
    "sap/ushell/Config"
], function (ContentExtensionAdapterFactory, jQuery, resources, AppStatePersistencyMethod, Config) {
    "use strict";

    /**
     * This method MUST be called by the Unified Shell's container only, others MUST call <code>sap.ushell.Container.getService("LaunchPage")</code>.
    ...
    function LaunchPage (oAdapter/*, oContainerInterface*/) {
        ...
        oAdaptersPromise.then(function (oAdapters) {
            jQuery.extend(this.oAdapters, oAdapters);
        }.bind(this));
         ...
         *
         * @param {boolean} bGetAll if the filter should be applied
         *
         * @returns {Promise<object>} A promise that resolves to the list of groups
         * @private
         */
                ...
                this.getDefaultGroup().then(function (oDefaultGroup) {
                    if (aGroups.length > 0) {
                        aGroups = aGroups.filter(function (group) {
                            if (bGetAll !== undefined && bGetAll === true) {
                    ...
                    jQuery.sap.log.error("getGroupsForBookmarks - getDefaultGroup - failed: " + oError.message);
                });
            }.bind(this), function (oError) {
                jQuery.sap.log.error("getGroupsForBookmarks - getGroups - failed: " + oError.message);
            });
        ...
        this.getGroupById = function (sGroupId) {
            ...
            this.getGroups().then(function (aGroups) {
                aGroups = jQuery.grep(
                    aGroups,
                    function (entry) {
                        return that.getGroupId(entry) === sGroupId;
                    }
                );

                Deferred.resolve((aGroups && aGroups.length > 0 ? aGroups[0] : undefined));
            });
         ...
         * @returns {Array} The group tiles array
         * @public
         ...
         * @returns {Array} The array of link tiles
         * @public
        ...
        this.addTile = function (oCatalogTile, oGroup) {
            var oPromise = this._getAdapter(oGroup.contentProvider).addTile(oCatalogTile, oGroup),
                that = this;
            oPromise.done(function () {
                that.changeURLStatesToPersistent(that.getCatalogTileTargetURL(oCatalogTile));
            });
            oPromise.fail(function () {
        ...
        this.removeTile = function (oGroup, oTile, iIndex) {
            var oPromise = this._getAdapter(oGroup.contentProvider).removeTile(oGroup, oTile, iIndex),
                that = this;
            oPromise.done(function () {
                that.deleteURLStatesPersistentData(that.getCatalogTileTargetURL(oTile));
            });
            oPromise.fail(function () {
        ...
        this.moveTile = function (oTile, iSourceIndex, iTargetIndex, oSourceGroup, oTargetGroup, sNewTileType) {
            ...
            oPromise.fail(function () {
                jQuery.sap.log.error("Fail to move Tile: " + that.getTileId(oTile));
            });
            return oPromise;
        };
        ...
        this.setTileVisible = function (oTile, bNewVisible) {
            var sAdapterName = oTile && oTile.contentProvider;
            return this._getAdapter(sAdapterName).setTileVisible(oTile, bNewVisible);
        };
        ...
        this.getCatalogTileView = function (oCatalogTile) {
            var sAdapterName = oCatalogTile && oCatalogTile.contentProvider;
            return this._getAdapter(sAdapterName).getCatalogTileView(oCatalogTile);
        };
         ...
         * @param {object} [oGroup] Optional reference to the group the bookmark tile should be added to. If not given, the default group is used.
        ...
        this.addBookmark = function (oParameters, oGroup) {
            ...
            if (oGroup && this.isGroupLocked(oGroup)) {
                ...
                sAdapterName = oGroup && oGroup.contentProvider;
                oPromise = this._getAdapter(sAdapterName).addBookmark(oParameters, oGroup);
                oPromise.done(function () {
                    that.changeURLStatesToPersistent(oParameters.url);
                });
                oPromise.fail(function () {
        ...
        this.countBookmarks = function (sUrl) {
            ...
            var oPromise = this._getAdapter().countBookmarks(sUrl);
            oPromise.fail(function () {
        ...
        this.deleteBookmarks = function (sUrl) {
            ...
            var oPromise = this._getAdapter().deleteBookmarks(sUrl);
            oPromise.done(function () {
                that.deleteURLStatesPersistentData(sUrl);
            });
            oPromise.fail(function () {
        ...
        this.updateBookmarks = function (sUrl, oParameters) {
            ...
            var oPromise = this._getAdapter().updateBookmarks(sUrl, oParameters);
            oPromise.fail(function () {
        ...
        this.onCatalogTileAdded = function (sTileId) {
            return this._getAdapter().onCatalogTileAdded(sTileId);
        };
            ...
            if (sURL && sURL.length > 0) {
                try {
                    ...
                    if (sXStateKey !== undefined) {
                        oAppState.makeStatePersistent(sXStateKey, AppStatePersistencyMethod.PersonalState);
                    }
                    if (sIStateKey !== undefined) {
                        oAppState.makeStatePersistent(sIStateKey, AppStatePersistencyMethod.PersonalState);
                    }
                } catch (e) {
                    jQuery.sap.log.error(
                        "error in converting transiant state to personal persistent when bookmark is added",
                        e,
                        "sap.ushell.services.LaunchPage");
                }
            }
         ...
         * @private
         */
        this.deleteURLStatesPersistentData = function (sURL) {
            var oAppState = sap.ushell.Container.getService("AppState");
            ...
            if (sURL && sURL.length > 0) {
                try {
                    ...
                    if (sXStateKey !== undefined || sIStateKey !== undefined) {
                        ...
                        // URL is apears in other tiles. Only if the result iz zero (because the current tile
                        // was already deleted) we can go ahead and delete also the states data
                        this.countBookmarks(sURL).done(function (iCount) {
                            if (iCount === 0) {
                                if (sXStateKey !== undefined) {
                                    oAppState.deleteAppState(sXStateKey);
                                }
                                if (sIStateKey !== undefined) {
                                    oAppState.deleteAppState(sIStateKey);
                                }
                            }
                        });
                    }
                    ...
                    jQuery.sap.log.error(
                        "error in deleting persistent state when bookmark is deleted",
                        e,
                        "sap.ushell.services.LaunchPage");
                }
            }
