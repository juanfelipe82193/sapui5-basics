...
sap.ui.define([
     ...
     * @param {integer} iMaxLength maximum number of entries in the list
     * @param {function} fnEquals used to decide whether an entry should be updated instead of inserting a new one
     * @param {function} fnCompare uesed to sort the list
     * @param {function} fnLoad called to load recent list from backend
     * @param {function} fnSave called to save current list into backend
     * @constructor
     */
    function RecentList (iMaxLength, fnEquals, fnCompare, fnLoad, fnSave) {
        ...
        this.newItem = function (oItem) {
            var oDeferred = new jQuery.Deferred(),
                iTimestampNow = +new Date(), // timestamp: thanks to http://stackoverflow.com/a/221297
                bAlreadyIn;
        ...
        var iMaximumDays = 30,
            ...
            fnUpdateIfAlreadyIn = function (oItem, iTimestampNow) {
                return oRecentActivities.recentUsageArray.some(function (oRecentEntry) {
                    ...
                    if (fnEquals(oRecentEntry.oItem, oItem)) {
                        /*
                        in case both items considered as equal (by fnEquals function),
        ...
        this.newItem = function (oItem) {
            ...
            fnLoad().done(function (aLoadedRecents) {
                ...
                fnSave(oRecentActivities).done(function (data) {
                    EventHub.emit("newUserRecentsItem", oRecentActivities);
                    oDeferred.resolve(data);
                }).fail(function () {
                    oDeferred.reject();
                });
            ...
            fnLoad().done(function (aLoadedRecents) {
                ...
                if (aURL.length > 0) {
                    ...
                    for (activityIndex = 0; activityIndex < oRecentActivities.recentUsageArray.length; activityIndex++) {
                        if (!(oRecentActivities.recentUsageArray[activityIndex].oItem.url[0] === "#")) {
                            urlItem = oRecentActivities.recentUsageArray[activityIndex];
     ...
     *
     * @param {function} fnLoad called to load current list from backend
     * @param {function} fnSave called to save current list into backend
     */
    ...
    function UserRecents () {
        ...
        /**
         * Add the given activity item.
         * Adds the action to the LRU list of activities.
         *
         * @param {object} oActionItem the actionItem  <code>sTerm</code>
         * @returns {object[]} the updated LRU list
         * @since 1.32.0
         * @public
         ...
         *
         * @returns {object[]} a list of frequently used activities
         * @since 1.42.0
         * @public
        ...
        /**
         * Notification that the given datasources has just been used.
         * Adds the search to the LRU list of datasources.
         *
         * @param {object} oDataSource the datasource identified by the string parameter <code>objectName.value</code>
         * @returns {object[]} the updated LRU list
         * @since 1.19.0
         * @public
         ...
         * Adds the search to the LRU list of searches.
         *
         * @param {object} oSearchItem the searchItem identified by the string parameter <code>sTerm</code>
         * @returns {object[]} the updated LRU list
         * @since 1.15.0
         * @public
        ...
        /**
         * Notification that the given app has just been used.
         * Adds the app to the LRU list of apps.
         *
         * @param {object} oAppItem the searchItem identified by the string parameter <code>id</code>
         * @returns {object[]} the updated LRU list
         * @since 1.15.0
         * @public
         ...
         */
        this.addAppUsage = function (hash) {
            var relevantHash = utils.getBasicHash(hash);
        ...
        oRecentSearches = new RecentList(10,
            function (x, y) {
                var compare = false;
        ...
        oRecentDataSources = new RecentList(6,
            function (x, y) {
        ...
        oRecentActivity = new RecentActivity(500,
            function (x, y) {
                ...
                } else if (x.appType === appType.APP || y.appType === appType.APP) {
