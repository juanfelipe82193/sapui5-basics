...
sap.ui.define([
...
], function (utils, personalizationUtils, WindowAdapter, jQuery) {
    "use strict";
    ...
    function WindowAdapterContainer (sContainerKey, oScope, oBackendContainer) {
    ...
    WindowAdapterContainer.prototype.load = function () {
        ...
        if (WindowAdapter.prototype.data[this._sContainerKey]) {
            //load data from window
            this._oItemMap.entries = personalizationUtils.clone(WindowAdapter.prototype.data[this._sContainerKey]);
            ...
            this._oBackendContainer.load().done(function () {
                ...
                WindowAdapter.prototype.data[that._sContainerKey] = personalizationUtils.clone(that._oItemMap.entries);
                oDeferred.resolve();
            }).fail(function (sMsg) {
                oDeferred.reject(sMsg);
            });
        } else {
            WindowAdapter.prototype.data[this._sContainerKey] = {};
            oDeferred.resolve();
        }
    ...
    WindowAdapterContainer.prototype.save = function () {
        var oDeferred = new jQuery.Deferred();
        WindowAdapter.prototype.data[this._sContainerKey] = personalizationUtils.clone(this._oItemMap.entries);
        if (this._oBackendContainer) {
