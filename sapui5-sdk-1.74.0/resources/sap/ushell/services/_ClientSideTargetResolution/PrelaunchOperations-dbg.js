 ...
 * @fileOverview
 *
 * <p>PrelaunchOperations are executed just before launching the application
 * (post parameter mapping and app state merging) if the
 * <code>sap-prelaunch-operations</code> parameter is specified as a technical
 * parameter.</p>
 *
 * @private
 *
...
sap.ui.define([
     ...
     * @returns {Promise} Resolves to an app state object
     * @since 1.69.0
     * @private
     */
    function _getAppState(sAppStateKey) {
        ...
        return new Promise(function (fnResolve, fnReject) {
            sap.ushell.Container.getService("AppState").getAppState(sAppStateKey).done(function (oAppState) {
                fnResolve(oAppState);
     ...
     * @param {array} aDefaultedParameterNames The defaulted parameter names
     *
     * @returns {Promise} Resolves to the matching object
     *
     * @since 1.69.0
     * @private
     */
    ...
    /**
     * Executes the merge operation. The parameters need to be merged can come from the selection variant
     * or the startup parameters. If the merge operation is executed successfully, an object which indicates
     * the modification of the startup paramters, the selection variant and the defaulted parameter names is returned.
    ...
    function _executeMerge(oOperation, oSelectionVariant, oStartupParameters) {
        ...
        // If intent parameters are present in the URL these must have the same value defined in the selection variant
        // in the sap-xapp-state. When no such match occurs, the merge operation should be aborted
    ...
    /**
     * Returns true if the split operaion is valid and false if it is not valid.
     * In the following situations, the split operation is not valid. 1.The source in the operation does not exist
     * in the selection variant. 2.One or more targets in the operation already exist in the selection variant.
     * 3.One or more targets in the operation already exist in the startup paramters.
     *
     * @param {object} oOperation The prelaunch operaion
     * @param {object} oSelectionVariant The selection variant
     * @param {object} oStartupParameters The startup parameters
     *
    ...
    /**
     * Executes the split operation and updates paramters in the startup paramters and the selection variant. The split
     * operation and the select vairant are validated. If they are not valid, an empty object is returned. If they
     * are valid, the split operation is executed and an object which indicates the modification of the startup paramters,
     * the selection variant and the defaulted parameter names is returned. The split operation does not modify the
     * defaulted parameter names
     *
    ...
    /**
     * Executes the delete operation and updates paramters in the startup paramters, the selection variant and the
     * defaulted parameter names.Returns an object which indicates the modification of the startup paramters, the
     * selection variant and the defaulted parameter names.
     *
     * @param {object} oOperation The prelaunch operation
     * @param {object} oSelectionVariant The selection variant
     * @param {object} oStartupParameters The startup parameters
     * @param {array} aDefaultedParameterNames The default parameter names
     *
    ...
    /**
     * Parses the prelaunch operations into an array in json format. Returns the array or an empty array if
     * the prelaunch operations is an emtpy string or cannot be parsed.
     *
     * @param {string} sPrelaunchOperations The sap-prelaunch-operations
     *
     * @returns {array} Prelaunch operations
     * @since 1.69.0
     * @private
     */
    ...
    /**
     * Validates prelaunch operations. Returns the array of the operations or null if there are invalid operaions.
     *
     * @param {array} aPrelaunchOperations The prelaunch operations
     *
     * @returns {array} Prelaunch operations
     * @since 1.69.0
     * @private
     ...
     * @param {string} sPrelaunchOperations The sap-prelaunch-operations
     *
     * @returns {array} Valid prelaunch operations
     * @since 1.69.0
     * @private
     */
    ...
    /**
     * Executes the prelaunch operations which are split, merge and delete. Returns an object which indicates
     * the modification of the seletion variant and the start up parameters. When the debug mode is enabled,
     * output the log messages in the console.
     *
    ...
    function _executePrelaunchOperations(oSelectionVariant, aPrelaunchOperations, oStartupParameters, aDefaultedParameterNames) {
        var aDebugLog = oCSTRUtils.isDebugEnabled() ? [] : null,
    ...
    /**
     * Executes the sap-prelaunch-operations on the matching target. The sap-prelaunch-operations
     * is passed as a string and needs to be parsed and validated before being excuted. When the
     * prelaunch operations are valid, they are executed and the matching target is updated
     * according to the exection results. When the prelaunch operations are not valid or the mathcing
     * target is not changed, the original matching target is returned.
     *
     * @param {object} oMatchingTarget The matching target
     * @param {string} sPrelaunchOperations The sap-prelaunch-operations
     *
     * @returns {Promise} Resolves to an object of the matching target
     * @since 1.69.0
     * @private
     */
    function executePrelaunchOperations(oMatchingTarget, sPrelaunchOperations) {
        ...
        return _getAppState(sSapXappState).then(function (oAppState) {
            ...
            var oAppStateData = oAppState ? oAppState.getData() : {},
                oClonedStartupParameters = fnClone(oStartupParameters),
                aClonedDefaultedParameterNames = fnClone(aDefaultedParameterNames),
                oSelectionVariant;
            ...
            var oPrelaunchOperationsResult = _executePrelaunchOperations(oSelectionVariant, aPrelaunchOperations, oClonedStartupParameters, aClonedDefaultedParameterNames);

            return _updateMatchingTarget(oPrelaunchOperationsResult, oMatchingTarget, oSelectionVariant, oClonedStartupParameters, aClonedDefaultedParameterNames);
        });
    }
