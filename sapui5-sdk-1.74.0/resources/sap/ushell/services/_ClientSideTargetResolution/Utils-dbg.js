...
"sap/base/Log"
], function (jQuery, Log, oVirtualInbounds, oFormatter) {
    "use strict";

    /**
     * When multiple parameters are renamed to the same parameter via renameTo,
     * the parameter that was explicitly passed in the intent must have
     * priority, all others should be removed.
     *
     * This method finds parameters that are dominated by a non-defaulted
     * (note, NOT necessarily required) parameter.
     *
     ...
     * @return {object}
     *   An object with dominated default parameters that must be deleted:
     *   <code>{
     ...
     *
     * @return {object}
     *   The object filtered based on the filter function. This is a new
     *   object, but the values assigned to the keys are not cloned from the
     *   original object.
     *
     * @private
     */
    ...
    function isDebugEnabled () {
        var iCurrentLogLevel = Log.getLevel();
        if (iCurrentLogLevel >= Log.Level.DEBUG) {
            return true;
        }
        return false;
    }
     ...
     *
     * @param {array} aParamsToExtract
     * An array of Parameters passed to determin what parameters are ment to be extracted
     *
     * @param {object} oWithParameters
     * The object the parameters should be extracted from
     *
     * @return {object}
