...
sap.ui.define([
     ...
     *
     * @param {oServiceConfiguration} oServiceConfiguration a Service configuration
     * @constructor
    ...
    function NavTargetResolution (oAdapter, oContainerInterface, sParameters, oServiceConfiguration) {
         ...
         * @return {jQuery.Deferred.Promise} a jQuery promise that resolves to an object representing the resolution result
         * @private
         * @since 1.34.0
         */
        ...
        this.expandCompactHash = function (sHashFragment) {
            ...
            if (oParsedShellHash && oParsedShellHash.params && oParsedShellHash.params["sap-intent-param"]) {
                sap.ushell.Container.getService("AppState").getAppState(oParsedShellHash.params["sap-intent-param"][0])
        ...
        this._getSapSystem = function (sHashFragment, oResolutionResult) {
            ...
            if (oResolutionResult && oResolutionResult.url) {
                sSystem = (new UriParameters(oResolutionResult.url)).get("sap-system");
                if (sSystem) {
                    return sSystem;
                }
            }
            if (aWDAGUIAppType.indexOf(oResolutionResult.applicationType) >= 0 && sHashFragment && sHashFragment.substring(1)) {
                sSystem = (new UriParameters(sHashFragment.substring(1))).get("sap-system");
                if (sSystem) {
                    return sSystem;
                }
            }
        ...
        this.resolveTarget = function (oArgs) {
            var sHashFragment,
                oArgsClone = utils.clone(oArgs),
                oDeferred = new jQuery.Deferred();
            utils.addTime("resolveTarget");
            Measurement.average("sap.ushell.navigation.resolveTarget");

            utils.storeSapSystemToLocalStorage(oArgsClone);
            sHashFragment = sap.ushell.Container.getService("URLParsing").constructShellHash(oArgsClone);
            sap.ushell.Container.getService("NavTargetResolution").resolveHashFragment("#" + sHashFragment).done(function (oResolved) {
                Measurement.end("sap.ushell.navigation.resolveTarget");
                oDeferred.resolve({
                    url: oResolved.url,
                    text: oResolved.text,
                    externalNavigationMode: oResolved.externalNavigationMode
                });
            }).fail(function (sMsg) {
        ...
        this.getLinks = function (oArgs) {
            ...
            // outcome determins whether the process of getting links delegates to ClientSideTargetresolution or navtarget resolution adapter.
            ...
            if (fnResolverBound) {
                ...
                if (oResolverResult.isGetSemanticObjectLinksCall) {
                    ...
                    oShellNavigation.hrefForExternal(oHrefForExternalArg, true, oComponent, true).done(function (oResVerboseCompacted) {
                        ...
                        fnResolverBound(
                            sSemanticObject,
                            oMaybeCompactedParameters, // may contain sap-intent-param (but server does nothing with it)
                            bIgnoreFormFactor)
         ...
         *
         * @returns {jQuery.Deferred.promise} A promise that resolves with an array of strings
         ...
         * @returns {jQuery.Deferred.Promise} a promise that is guaranteed to resolve to an array with the same
         *   structure as the array returned from getSemanticObjectLinks, but with shortened intents.
         *   NOTE: if it's not possible to shorten an intent in the input array, a warning is logged on the console and the unshortened intent is kept.
         * @private
         * @since 1.32.0
         */
         ...
         * @returns {object} A <code>jQuery.Deferred</code> object's promise which is resolved with a map
         ...
         *
         * The resolver must be JavaScipt object with a string property name, and two functions
         ...
         *   b) Allows to launch an application via a specific ( client side coded ) Target #Shell-runStandaloneApp
            ...
            resolveHashFragment: function (sHashFragment) {
                ...
                function localURL (sUrl) {
                    ...
                    var sPathNameUrl = (new URI(that.parseUrl(sUrl).href)).normalizePathname().pathname(),
                        oRunStandaloneAppFolderWhitelist = ObjectPath.create("runStandaloneAppFolderWhitelist", oServiceConfig),
                        sElement;
                ...
                function getURLParameter (sKey) {
                    return (new UriParameters(window.location.href)).get(sKey);
                }
