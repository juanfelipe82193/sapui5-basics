...
sap.ui.define([
...
], function (PersonalizationProcessor, ObjectPath, extend, jQuery, Log, isPlainObject) {
                    ...
                    .done(function (oPers) {
                        that._oPersonalizationProcessor.mixinPersonalization(oSite, oPers)
                            .done(function (oPersonalizedSite) {
                                ...
                                that._oPersonalizedSite = that._ensureGroupsOrder(that._oPersonalizedSite);
                                ...
                                that._oPersonalizedSite = that._ensureStandardVizTypesPresent(that._oPersonalizedSite);
     ...
     *
     */
    CommonDataModel.prototype._getPage = function (sPageId) {
        return new Promise(function (resolve, reject) {
            this._oAdapter._getPage(sPageId).then(function (oSite) {
                oSite = extend({}, oSite);
                oSite = this._ensureCompleteSite(oSite);
                oSite = this._ensureGroupsOrder(oSite);

                oSite = this._ensureStandardVizTypesPresent(oSite);
                resolve(oSite);
            }.bind(this)).catch(function (sMessage) {
                    reject(sMessage);
                }
            );
        }.bind(this));
        ...
        this._oPersonalizationProcessor.extractPersonalization(this._oPersonalizedSite, this._oOriginalSite)
            .done(function (oExtractedPersonalization) {
                if (oExtractedPersonalization) {
                    that._oAdapter.setPersonalization(oExtractedPersonalization)
                        .done(function () {
                            oDeferred.resolve();
                        })
