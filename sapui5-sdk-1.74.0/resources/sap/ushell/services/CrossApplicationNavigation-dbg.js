...
sap.ui.define([
    ...
    /**
     * The Unified Shell's CrossApplicationNavigation service,
    ...
    function CrossApplicationNavigation (oContainerInterface, sParameters, oServiceConf) {
         ...
         * parameter object <code>oTarget</code> used in the methods {@link #hrefForExternal()} and {@link #toExternal()}.
         * The system is only added if the current application specifies it and <code>oTarget</code> does not already contain this parameter.
         *
        ...
        function getTargetWithCurrentSystem (vTarget, oComponent) {
            ...
            var sNextNavMode;
            if (oComponent) {
                // Take sap-system and sap-ushell-next-navmode parameters from the component
                ...
                // Take sap-system and sap-ushell-next-navmode parameters from the current application
                ...
                if (oResolution && oResolution["sap-system"]) {
                    ...
                    sSystem = new UriParameters(oResolution.url).get("sap-system");
                }
                if (oResolution && oResolution["sap-ushell-next-navmode"]) {
                    sNextNavMode = oResolution["sap-ushell-next-navmode"];
                } else if (oResolution && oResolution.url) {
                    sNextNavMode = new UriParameters(oResolution.url).get("sap-ushell-next-navmode");
                }
            }
         ...
         * object <code>oTarget</code> used in the methods {@link #hrefForExternal()} and {@link #toExternal()}.
         ...
         * @return {variant} The intent with the given <code>sInnerAppRoute</code> parameter.
         * @private
         */
        this._injectInnerAppRoute = function (vIntent, sInnerAppRoute) {
            ...
            if (Object.prototype.toString.apply(vIntent) === "[object Object]") {
                sShellHash = ObjectPath.get("target.shellHash", vIntent);
                if (typeof sShellHash === "string") {
        ...
        * @returns {string} the href for the specified parameters as an *external* shell hash;
        ...
        */
        this.hrefForExternal = function (oArgs, oComponent, bAsync) {
            ...
            if (sap.ushell && sap.ushell.Container && typeof sap.ushell.Container.getService === "function" && sap.ushell.Container.getService("ShellNavigation")) {
                ...
                oArgsClone = merge({}, oArgs);
                oExtraction = this._extractInnerAppRoute(oArgsClone);

                vIntentNoAppRoute = oExtraction.intent;
                ...
                oArgsClone = oUtils._injectStickyParameters({
                    args: oArgsClone,
         ...
         * @returns {object} promise the success handler of the resolve promise get an expanded shell hash as first argument
         * @public
        ...
        /**
         * Attempts to use the browser history to navigate to the previous app.<p>
         * A navigation to the Fiori Launchpad Home is performed in case this method is called on a first navigation.
         ...
         * especially on mobile devices where "back" is the previous inner app state iff these are put into the history!</p>
         *
         * @public
         ...
         *   <code>
         *   {
         *     target : { semanticObject : "AnObject", action: "action", context  : "AB7F3C" },
         *     params : { A : "B", c : "e" }
         *   }
         *   </code>
         *   or
         ...
         *
         * Since version 1.56 this API accepts a sap-xapp-state-data parameter that can be used to launch and application with certain data,
         * for example:
         *   <pre>
         *   {
         *     target : { semanticObject : "AnObject", action: "action" },
         ...
         * Note that the application parameter length (including SemanticObject/Action) shall not exceed 512 bytes when serialized as UTF-8
         *
         * @param {object} [oComponent] an optional SAP UI5 Component,
         * @since 1.15.0
         * @public
        ...
        this.getPrimaryIntent = function (sSemanticObject, mParameters) {
            var oQuery = {},
                fnSortPredicate,
                rgxDisplayFactSheetAction = /^#\w+-displayFactSheet(?:$|\?.)/;
         ...
         * @returns {object} A <code>jQuery.Deferred</code> object's promise which is resolved with
        ...
        this.getSemanticObjectLinks = function (sSemanticObject, mParameters, bIgnoreFormFactor, oComponent, sAppStateKey, bCompactIntents) {
            ...
            mParametersPlusSapSystem = oUtils._injectStickyParameters({
            ...
            aExpandedIntents = utils.invokeUnfoldingArrayArguments(oSrv.getLinks.bind(oSrv), [vArgs]);
         ...
         * @returns {jQuery.Deferred.promise} A promise that resolves with an array of links objects containing (at least)
        ...
        this._getLinks = function (oNominalArgs) {
            ...
            oNominalArgsClone = oUtils._injectStickyParameters({
         ...
         * @param {object} [oComponent] the root component of the application
         * @returns {object} A <code>jQuery.Deferred</code> object's promise which is resolved with a map containing
         ...
         * It is functionally equivalent to {@link isIntentSupported} but accepts the same interface as {@link toExternal}/ {@link hrefForExternal}.
         *
         * @param {object[]} aIntents the intents to be checked with object being instances the oArgs object of toExternal, hrefForExternal etc.
         *   e.g.
         *   <code>
         *   {
         *     target: {
         ...
         *   <code>{ target: { shellHash: "SO-36&jumper=postman" } }</code>
         * @param {object} [oComponent] the root component of the application
         * @returns {object} A <code>jQuery.Deferred</code> object's promise which is resolved with an array (!) of objects representing
         *   whether the intent is supported or not objects with a property <code>supported</code> of type <code>boolean</code>.<br/> representing
         *   Example:
         *
         * aIntents:
         *   <pre>
         ...
         * @returns {object} A <code>jQuery.Deferred</code> object's promise which is resolved if the URL is supported and rejected if not.
         *   The promise does not return parameters.
         * @since 1.30.0
         * @private
         */
        ...
        /**
         * Creates an empty app state object which act as a parameter container for cross app navigation.
         *
         * @param {object} oAppComponent - a UI5 component used as context for the app state
         * @param {boolean} bTransient true if the appstate should not persisted on the backend.
         ...
         */
        this.createEmptyAppState = function (oAppComponent, bTransient, iPersistencyMethod, oPersistencySettings) {
            if (!oAppStateService) {
            ...
            return oAppStateService.createEmptyAppState(oAppComponent, bTransient, iPersistencyMethod, oPersistencySettings);
        };
         ...
         * @return {object} promise object returning the data of an AppState object, or an empty <code>{}</code> javascript
         ...
         *
         * @private internal usage(exposure to WebDypnro ABAP)
         */
         ...
         * @returns {object} a jQuery.Deferred. Returns a promise,
        ...
        this._processShellHashWithParams = function (oArgs) {
            ...
            if (oArgs && oArgs.processParams === true && oArgs.target && oArgs.target.shellHash && oArgs.params) {
                ...
                oHash = oUrlParsing.parseShellHash(oArgs.target.shellHash);
        ...
        this.makeStatePersistent = function (sKey, iPersistencyMethod, oPersistencySettings) {
            ...
            return oAppStateService.makeStatePersistent(sKey, iPersistencyMethod, oPersistencySettings);
        };
    }
