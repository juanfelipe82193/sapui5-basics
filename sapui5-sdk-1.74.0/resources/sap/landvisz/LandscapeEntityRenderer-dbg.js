...
sap.ui.define([
	...
	/**
	 * LandscapeEntity renderer.
	 * @namespace
	 */
	var LandscapeEntityRenderer = {};

	LandscapeEntityRenderer.render = function(oRm, oControl) {
		...
		if (!this.initializationDone) {
			...
			if (oControl.viewType == ViewType.DEPENDENCY_VIEW
					|| oControl.viewType == ViewType.SOLUTION_VIEW)
			...
			if (oControl.getDataContainers() && oControl.getDataContainers().length > 0) {
				...
				for ( var i = 0; i < containers.length; i++) {
					...
					if (hasHeader == false) {
						oControl.smvContainer = new SingleDataContainer(
	...
	LandscapeEntityRenderer._renderModelingStatus = function(oRm,
			oControl) {
		var renderSize = oControl.getRenderingSize();
		oRm.write("<div");
		...
		if (oControl.entityMaximized != true) {
			...
			else if (renderSize == EntityCSSSize.Medium)
				oRm.addClass("sapLandviszStatusMediumSize");
			else if (renderSize == EntityCSSSize.Large)
	...
	LandscapeEntityRenderer._renderActionBar = function(oRm, oControl) {
		var renderSize = oControl.getRenderingSize();
		...
		if (oControl.entityMaximized != true) {
			...
			if (oControl.expVisible == true){
				if(renderSize == EntityCSSSize.Regular){
	...
	LandscapeEntityRenderer._renderSystemEntity = function(oRm,
			oControl) {
		...
		oRm.writeClasses();
		oRm.write(">");
		if(oControl.getType()==LandscapeObject.Track){
	...
	LandscapeEntityRenderer._renderTrack = function(oRm,oControl) {
	...
	LandscapeEntityRenderer._renderIdentificationBar = function(oRm,
			oControl) {
	...
	LandscapeEntityRenderer._renderDataEntityRegion = function(oRm,oControl) {
	...
	//finding properties.
	LandscapeEntityRenderer._findContainerProperties = function(key,
			oControl, oRm) {
	...
	LandscapeEntityRenderer._findContainerAggregationProperties = function(
			key, oControl, oRm) {
	...
	LandscapeEntityRenderer._renderSingleContainerData = function(oRm,
			oControl) {
		var container = oControl.getDataContainers();
		var continerId = oControl.getId();
		oControl.dataContainer = new DataContainer(continerId
	...
	LandscapeEntityRenderer._setPropertiesAggregation = function(oRm,
		...
		} else if (properties instanceof Control) {
	...
	LandscapeEntityRenderer._renderContainerData = function(oRm,
			oControl) {
		...
		for ( var i = 0; i < oControl.propertyHeaders.length; i++) {
			...
			oControl.dataContainer = new DataContainer(
	...
	LandscapeEntityRenderer._makeOtherContainerUnselected = function(
			oControl, index) {
	...
	LandscapeEntityRenderer._renderTabContainers = function(oRm,
			oControl) {
		...
		if (oControl.expVisible == false
			...
			else if(oControl.getRenderingSize() ==  EntityCSSSize.Regular)
				oControl.oVLayoutProperties
				.addStyleClass("sapLandviszRegularSizesPropertySectionSmv");
			else if(oControl.getRenderingSize() ==  EntityCSSSize.RegularSmall)
				oControl.oVLayoutProperties
		...
		if(oControl.expVisible == false && oControl.getRenderingSize() ==  EntityCSSSize.RegularSmall){
			oControl.oVLayoutProperties
			.removeStyleClass("sapLandviszRegularSizesPropertySectionSmv");
		}

		if(oControl.explodeViewClosed == true && oControl.getRenderingSize() ==  EntityCSSSize.RegularSmall && oControl.oHeadersLayout.getContent().length > 3){
			oControl.oVLayoutProperties
			.removeStyleClass("sapLandviszRegularSmallSizesPropertySectionSmv");
			oControl.oVLayoutProperties
			.addStyleClass("sapLandviszRegularSmallSizesPropertySection");
		}
		if(oControl.explodeViewClosed == true && oControl.getRenderingSize() ==  EntityCSSSize.Small && oControl.oHeadersLayout.getContent().length > 3){
			oControl.oVLayoutProperties
			.removeStyleClass("sapLandviszSmallSizesPropertySectionSmv");
			oControl.oVLayoutProperties
			.addStyleClass("sapLandviszSmallSizesPropertySection");
		}
		if(oControl.expVisible == true && oControl.getRenderingSize() ==  EntityCSSSize.RegularSmall){
			oControl.oVLayoutProperties
			.addStyleClass("sapLandviszRegularSmallSizesPropertySectionSmv");
		}
	...
	LandscapeEntityRenderer._setVisibleTabs = function(oRm, oControl) {
	...
	LandscapeEntityRenderer._enableNextAndPrevious = function(oRm,
	...
	LandscapeEntityRenderer._renderContainerAggregation = function(
			oRm, oControl) {
	...
	LandscapeEntityRenderer._setContainerRenderingSize = function(
			oControl) {
	...
	LandscapeEntityRenderer._initilizeDataContainer = function(oRm,
			oControl, index) {
	...
	LandscapeEntityRenderer._createMiniNavigation = function(oRm,
		...
		if (hasNavigation == false)
			oControl.headerBtn = new HeaderList();
	...
	LandscapeEntityRenderer._selectMiniNavigationButton = function(
			oControl, oRm, value) {
	...
	LandscapeEntityRenderer._selectNavigationHeader = function(
			oControl, oRm, value) {
	...
	LandscapeEntityRenderer._renderDataContainerProperties = function(
		...
		} else if (oProperties instanceof Control) {
	...
	LandscapeEntityRenderer._renderContainerProperty = function(
			oControl, oProperty, id) {

		if (oProperty instanceof LinearRowField
				|| oProperty instanceof NestedRowField
				|| oProperty instanceof TreeField
				|| oProperty instanceof LongTextField)
	...
	LandscapeEntityRenderer._renderEntityAction = function(oRm,
			oControl) {
	...
	LandscapeEntityRenderer._renderExplodeView = function(oRm,
			oControl) {
		this.hideUnselectedHeaders(oRm, oControl);
	};

	LandscapeEntityRenderer.hideUnselectedHeaders = function(oRm,
			oControl) {
	...
	LandscapeEntityRenderer._renderUnselectedHeaderPopup = function(
			oRm, oControl) {
		...
		var renderSize = oControl.getRenderingSize();
		if (renderSize == EntityCSSSize.RegularSmall)
	...
	LandscapeEntityRenderer._resetHeaderData = function(oRm, oControl) {
	...
	LandscapeEntityRenderer._resetHeaderNavigation = function(oRm,
			oControl) {
		var containers = oControl.getDataContainers();
		for ( var i = 0; i < containers.length; i++) {
			if (i != oControl.selectedIndex) {
	...
	LandscapeEntityRenderer._checkAllPopupClosed = function(oControl,
			currentDailog) {
	...
	LandscapeEntityRenderer._renderCustomActionRegion = function(oRm,
			oControl) {
	...
	LandscapeEntityRenderer._renderOverlayContainer = function(oRm,
			oControl) {
		...
		if(headerContent){
			for ( var i = 0; i < headerContent.length; i++) {
				if (headerContent[i] instanceof LinearRowField) {
				...
				} else if (headerContent[i] instanceof NestedRowField) {
					nestedRow = new NestedRowField();
	...
	return LandscapeEntityRenderer;
