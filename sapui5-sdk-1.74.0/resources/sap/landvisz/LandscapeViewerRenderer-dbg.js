...
sap.ui.define([
...
], function(landviszLibrary, Connector, Connection, DeploymentType, lvsvg, OptionEntity, ResponsiveFlowLayout, ResponsiveFlowLayoutData) {
	"use strict";
	...
	/**
	 * LandscapeViewer renderer.
	 * @namespace
	 */
	var LandscapeViewerRenderer = {};

	LandscapeViewerRenderer.render = function(oRm, oControl) {
		...
		if (!this.initializationDone) {
			...
			if (oControl.getVisibleDependency() == DependencyVisibility.BOTH) {
				oControl.networkViewVisible = true;
				oControl.boxViewVisible = true;
			} else if (oControl.getVisibleDependency() == DependencyVisibility.NETWORK) {
				oControl.networkViewVisible = true;
				oControl.boxViewVisible = false;
			} else if (oControl.getVisibleDependency() == DependencyVisibility.BOX) {
			...
			if (oControl.getViewType() == ViewType.SOLUTION_VIEW) {
				...
				if (oControl.getViewType() == ViewType.SELECTION_VIEW)
					this._renderSelectionView(oRm, oControl);
				else if (oControl.getViewType() == ViewType.DEPENDENCY_VIEW) {
					this._renderDependencyView(oRm, oControl);
				}

				if (oControl.getViewType() == ViewType.DEPENDENCY_VIEW)
					this._renderNavigationPanel(oRm, oControl);
			}
			oRm.write("</div>");
		}

		if (oControl.currentView == DependencyType.BOX_VIEW) {
		...
		if (optionsEntities.length > 0) {
			for ( var i = 0; i < optionsEntities.length; i++)
				if (optionsEntities[i] instanceof OptionEntity)
	...
	LandscapeViewerRenderer._renderSolutionView = function(oRm,oControl) {
			...
			if(oControl.getSolutionType() == SolutionType.DEPLOYMENT_VIEW)
	...
	LandscapeViewerRenderer._renderComponentViewHeader = function(oRm,
				oControl) {
			...
			if (oControl.firstTime == true) {
				oControl.compViewBtn.attachPress(function(oEvent) {
									...
									oControl.currentSolutionView = SolutionType.COMPONENT_VIEW;
	...
	LandscapeViewerRenderer._renderDeploymentViewHeader = function(
				oRm, oControl) {
			...
			if (oControl.firstTime == true) {
				oControl.depViewBtn.attachPress(function(oEvent) {
									oControl.currentSolutionView = SolutionType.DEPLOYMENT_VIEW;
	...
	LandscapeViewerRenderer._renderSolutionViewer = function(oRm,oControl) {
	...
	//	       oRm.addStyle("height", "-o-calc(100% - 35px)");
			...
			// Bug fix to remove the top empty area in Component View
			if(oControl.getSolutionType() == SolutionType.COMPONENT_VIEW)
				this._renderOptions(oRm, oControl);
			oRm.write("</div>");
	};


	LandscapeViewerRenderer._renderPlugContent = function(oRm,oControl) {
	...
	LandscapeViewerRenderer._renderSolutionContainer = function(oRm,
				oControl) {
			...
			var connectedLayout;
			if (options.length >0 && options[0].getType() == OptionType.VIEW)
			...
			if (oControl.getSolutionOptions().length >0 && oControl.getSolutionType() == SolutionType.COMPONENT_VIEW )
	...
	LandscapeViewerRenderer._renderSolution = function(oRm, oControl) {
	...
	//       connectedLayout = oControl.connection.getBoxViewConnectedNodesLayout(connectors);
			...
			if (connectedLayout) {
				...
				var boxContainer = oControl.$("solutionInnerContainer");
				...
				for ( var i = 0; i < systems.length; i++) {
						...
						if (connectedLayout[entityID]) {
								...
								systems[i].viewType = ViewType.SOLUTION_VIEW;
				...
				for ( var i = 0; i < connectionEntities.length; i++) {
						...
						connectionEntities[i].viewType = ViewType.SOLUTION_VIEW;
			...
			var navigationContainer = oControl.$("navigation");
			var navigator = oControl.$("navigation_navigator");
	...
	LandscapeViewerRenderer._renderSolutionDeploymentType = function(
				oRm, oControl) {
			...
			for ( var j = 1; j < optionsEntities.length; j++) {
				...
				for ( var k = 0; k < optionSources.length; k++) {
						...
						if (null != entity
									&& entity.getComponentType() == ComponentType.notDefined)
								continue;
	...
	LandscapeViewerRenderer._getDesiredSystem = function(oControl, id) {
	...
	LandscapeViewerRenderer._renderDeploymentType = function(oRm,
				oControl, count, left, type, tooltip, selected, srcEntityId) {

			var oDepType = new DeploymentType();
			oDepType.left = left;
	...
	LandscapeViewerRenderer._renderOptions = function(oRm, oControl) {
			var options = oControl.getSolutionOptions();
			if(options.length <=0)
				return;
			...
			if (oControl.getSolutionOptionType() == OptionType.VIEW)
				oRm.addClass("sapLandviszOptionContainer");
			else if (oControl.getSolutionOptionType() == OptionType.ENTITY)
	...
	LandscapeViewerRenderer._createOptions = function(oRm, oControl) {
			if (oControl.getSolutionOptionType() == OptionType.VIEW) {
	...
	LandscapeViewerRenderer._createEntityOptions = function(oRm, oControl) {
			var options = oControl.getSolutionOptions();
			var optionsEntities;
			var systems = oControl.getSystems();
			var srcLeft;
	...
	LandscapeViewerRenderer._renderViewOptions = function(oRm, oControl, optionType) {
			var options = oControl.getSolutionOptions();
			...
			for ( var i = 0; i < optionsEntities.length; i++) {
				...
				optionsEntities[i].attachEvent("optionSelected", function(event) {
						...
						for ( var j = 0; j < optionsEntities.length; j++) {
								optionEntity = optionsEntities[j].$();
								optionEntity.css({
									"background-color" : "transparent",
									"color" : "#00669c"
								});
	...
	LandscapeViewerRenderer._makeOptionVisible = function(oRm,
				oControl, event) {
			var optionBtn = event.getSource();

			if (optionBtn.optionOn == OptionType.VIEW) {
				this._makeViewOptionVisible(oRm, oControl, optionBtn);
			} else if (optionBtn.optionOn == OptionType.ENTITY) {
	...
	LandscapeViewerRenderer._makeViewOptionVisible = function(oRm,
				oControl, optionBtn) {
			...
			var depContainer = oControl.$("deploymentTypeContainer");
			...
			for ( var x = 0; x < deploymentTypes.length; x++) {
				deploymentType = jQuery(deploymentTypes[x]);
				deploymentType.css({
						"display" : "none"
				});
			}
						...
						function() {
							...
							for ( var i = 0; i < connectionEntities.length; i++) {
								...
								if (connectedLayout[connectionId]) {
									...
									var connectionRow = jQuery(document.getElementById(connectionEntities[i].getId()
											+ "connectionRow"));
	...
	LandscapeViewerRenderer._makeEntityOptionVisible = function(oRm,
				oControl, optionBtn) {
	...
	LandscapeViewerRenderer._renderNavigationPath = function(oRm,
				oControl) {
	...
	LandscapeViewerRenderer._renderSelectionView = function(oRm,
				oControl) {
			...
			oRm.writeStyles();
			oRm.write(">");
			systems[0].viewType = ViewType.SELECTION_VIEW;
	...
	LandscapeViewerRenderer._renderDependencyView = function(oRm,
				oControl) {
	...
	//     oRm.write("top:" + 26 + "px;");
			...
			if (oControl.currentView != ""
						&& oControl.currentView == DependencyType.NETWORK_VIEW)
				this._renderNetworkView(oRm, oControl)
			else if (oControl.currentView != ""
						&& oControl.currentView == DependencyType.BOX_VIEW)
				this._renderBoxView(oRm, oControl);
			else if (oControl.getVisibleDependency() == DependencyVisibility.BOTH) {
				if (oControl.getDefaultDependencyView() == DependencyType.NETWORK_VIEW)
						this._renderNetworkView(oRm, oControl);
				if (oControl.getDefaultDependencyView() == DependencyType.BOX_VIEW)
						this._renderBoxView(oRm, oControl);
			} else if (oControl.getVisibleDependency() == DependencyVisibility.NETWORK)
				this._renderNetworkView(oRm, oControl);
			else if (oControl.getVisibleDependency() == DependencyVisibility.BOX)
	...
	LandscapeViewerRenderer._renderCloseButton = function(oRm,
				oControl) {
	...
	LandscapeViewerRenderer._renderNetworkViewHeader = function(oRm,
				oControl) {
			...
			if (type == ViewType.SELECTION_VIEW) {
				oControl.networkViewBtn.setEnabled(false);
			} else if (type == ViewType.DEPENDENCY_VIEW) {
	...
	LandscapeViewerRenderer._renderBoxViewHeader = function(oRm,
				oControl) {
	...
	LandscapeViewerRenderer._renderTitle = function(oRm, oControl) {
			...
			oRm.write(">");
			oRm.writeEscaped(containerheaderText);
			oRm.write("</div>");
	};

	LandscapeViewerRenderer._renderNetworkView = function(oRm,
				oControl) {
			this.renderAllEntities(oRm, oControl, true);
	};

	// renders entities for network view
	LandscapeViewerRenderer.renderAllEntities = function(oRm,
				oControl, isRendering) {

			var connectors = oControl.getConnectors();
			oControl.currentView = DependencyType.NETWORK_VIEW;

			var boxContainer = oControl.$("boxViewContainer");
			if (boxContainer)
			...
			if (boxLevels) {
				for ( var i = 0; i < boxLevels.length; i++) {
						var boxLabel = oControl.$("boxViewLabel" + i);
						if (boxLabel)
			...
			if(oControl.getConnectionLine() == ConnectionLine.Line)
			...
			for ( var i = 0; i < systems.length; i++) {
				...
				systems[i].left = leftPos;
				systems[i].top = topPos;
				systems[i].viewType = ViewType.DEPENDENCY_VIEW;
				if (isRendering == true)
						oRm.renderControl(systems[i]);
				else
	...
	LandscapeViewerRenderer._renderBoxContainer = function(oRm,
				oControl) {
	...
	LandscapeViewerRenderer._renderBoxViewLabels = function(oRm,
				oControl) {
	...
	LandscapeViewerRenderer._renderNavigationPanel = function(oRm,
				oControl) {
	...
	LandscapeViewerRenderer._renderBoxView = function(oRm, oControl) {

			this.renderAllBoxViewEntities(oRm, oControl, true);
	};

	// renders all entities for box view
	LandscapeViewerRenderer.renderAllBoxViewEntities = function(oRm,
				oControl, isRendering) {
			...
			if (connectedLayout) {
				...
				for ( var i = 0; i < connectionEntities.length; i++) {
						...
						if (oControl.getVisibleDependency() == DependencyVisibility.BOTH
									&& oControl.getDefaultDependencyView() == DependencyType.NETWORK_VIEW) {
									...
									function() {
											if (boxLevels) {
													for ( var i = 0; i < boxLevels.length; i++) {
														...
														var boxLavelLabel = oControl.$("boxViewLabel" + i + "Level");
	...
	LandscapeViewerRenderer._moveEntityToPosition = function(leftpos,
				toppos, entityId) {

			var entity = jQuery(document.getElementById(entityId));
	...
	LandscapeViewerRenderer._animateConnectionEntityContainer = function(
			...
			var connectionDiv = jQuery(document.getElementById(connectionDivID));
	...
	return LandscapeViewerRenderer;
