...
sap.ui.define([
...
], function(landviszLibrary, jQuery) {
	"use strict";
	...
	Connection.hideConnections = function() {
		if (this.svgForConnections) {
			jQuery(this.svgForConnections.canvas).hide();
		}
	}

	Connection.getConnectedNodesLayoutTrack= function(connections) {
	...
	Connection.assignIndexToConnectedNodes= function(node,key) {
	...
	Connection.assignTargetIndex= function(nodename) {
	...
	Connection.assignSourceIndex= function(nodename) {
	...
	Connection.getAllTargets= function(sourceConnection) {
	...
	Connection.getAllSources= function(sourceConnection) {
	...
	Connection.getConnectedNodesLayout = function(connections) {
	...
	Connection.getAllConnections = function() {
	...
	Connection.parseSingleConnection = function(
			connectionData) {
		var properties = {};

		jQuery(connectionData).children('DATA').each(function() {
			var property = jQuery(this).attr('property');
			var value = typeof jQuery(this).attr('value') === "string" ? jQuery(this).attr('value').trim() : jQuery(this).attr('value') != null ? String(jQuery(this).attr('value')).trim() : "";
	...
	Connection.getSourceTargetConnections = function(
			sourceConnection) {
		var sourceMatrixCol = [];
	...
	Connection.getTargetSourceConnections = function(
			targetConnection) {
		var targetMatrixCol = [];
	...
	Connection.findConnectedNodes = function(nodeName) {
		var node = this.connectionMatrix[nodeName];
	...
	Connection.inTopPosition = function(row, col) {
		if (this.topPostions[row]) {
	...
	Connection.renderConnections = function(systems,connectionEntities,type) {
		for ( var i = 0; i < this.allConnections.length; i++) {
	...
	Connection.getEntity = function(systems,connectionEntities,connectorID){
	...
	Connection.drawConnection = function(clvConnection,type) {
	...
	Connection.getX = function(entity) {
	...
	Connection.getY = function(entity) {
	...
	Connection.getTargetXMiddle = function(targetEntity) {
	...
	Connection.getTargetYCenter = function(targetEntity) {
	...
	Connection.showConnections = function() {
		if (this.svgForConnections) {
			jQuery(this.svgForConnections.canvas).show();
		}
	}

	Connection.destroyConnections = function() {
		if (this.svgForConnections) {
	...
	Connection.getConnectedNodes = function(nodeName) {
		var connectedNodes = [];
		for ( var i = 0; i < this.allConnections.length; i++) {
	...
	Connection.deinitialize = function() {
		if (this.initialized) {
			if (this.svgForConnections) {
				this.svgForConnections.canvas.remove();
				this.svgForConnections = null;
	...
	Connection.getBoxViewConnectedNodesLayout = function (connections) {
