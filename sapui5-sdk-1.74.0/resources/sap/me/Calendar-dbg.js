...
sap.ui.define([
	...
	// shortcut for sap.me.CalendarEventType
	var CalendarEventType = library.CalendarEventType;

	// shortcut for sap.me.CalendarSelectionMode
	var CalendarSelectionMode = library.CalendarSelectionMode;

	// shortcut for sap.me.CalendarDesign
	var CalendarDesign = library.CalendarDesign;
	...
	var Calendar = Control.extend("sap.me.Calendar", /** @lends sap.me.Calendar.prototype */ { metadata : {
		...
		properties : {
			 ...
			 */
			design : {type : "sap.me.CalendarDesign", group : "Appearance", defaultValue : CalendarDesign.Approval},

			/**
			 * Indicates the design of the calendar (mainly colors)
			 */
			selectionMode : {type : "sap.me.CalendarSelectionMode", group : "Behavior", defaultValue : CalendarSelectionMode.SINGLE},
	...
	Calendar.prototype.setEnableMultiselection = function (bEnable) {
		// no rerender needed
		return this.setProperty("selectionMode", bEnable ? CalendarSelectionMode.MULTIPLE : CalendarSelectionMode.SINGLE, true);
	};

	Calendar.prototype.getEnableMultiselection = function () {
		return this.getProperty("selectionMode") == CalendarSelectionMode.MULTIPLE;
	};
	...
	Calendar.prototype._gestureSelect = function (oEvent) {
		...
		if ($date) { // handle tap-like event
			...
			if (!this.getEnableMultiselection()) {
				if (this.getSelectionMode() == CalendarSelectionMode.SINGLE) {
					this.unselectAllDates();
				} else if (this.getSelectedDates().length > 1) {
					this.unselectAllDates();
				}
			}
			if ((this.getSelectionMode() == CalendarSelectionMode.RANGE) && (this.getSelectedDates().length == 1)) {
	...
	Calendar.prototype.removeTypesOfAllDates = function () {
		this._removeStyleClassOfAllDates("sapMeCalendar" + CalendarEventType.Type00);
		this._removeStyleClassOfAllDates("sapMeCalendar" + CalendarEventType.Type01);
		this._removeStyleClassOfAllDates("sapMeCalendar" + CalendarEventType.Type04);
		this._removeStyleClassOfAllDates("sapMeCalendar" + CalendarEventType.Type06);
		this._removeStyleClassOfAllDates("sapMeCalendar" + CalendarEventType.Type07);
		this._removeStyleClassOfAllDates("sapMeCalendar" + CalendarEventType.Type10);
	};
	...
	Calendar.prototype._getDatesWithStyleClass = function (sState) {
		...
		var sDate;
		for (sDate in this._oDatesClasses) {
			if (this._oDatesClasses[sDate] && this._oDatesClasses[sDate].indexOf(sState) !== -1) {
				aDates.push(sDate);
			}
		}
		return aDates;
	...
	Calendar.prototype._getCalendarFirstDate = function () {
		...
		if (this.getSingleRow()) {
			...
			Log.error("You cannot use this method in a non single row calendar, returning current date.");
		}
