 ...
 */
sap.ui.define(['sap/ui/base/Object', 'sap/ui/core/date/UniversalDate', "sap/base/Log"],
	function (BaseObject, UniversalDate, Log) {
		"use strict";
		 ...
		 * @private
		 */
		var theCalendarDate = BaseObject.extend("sap.me.CalendarDate", {
		...
		theCalendarDate.getMonthFromString = function (strMonth) {
			...
			return aMonths.indexOf(strMonth);
		};
		...
		theCalendarDate.parseFromToDateString = function (fromToDateString, bThrowOnParseError) {
			...
			if (aResult !== null && aResult.length === 4 && aResult[0] === fromToDateString) {
				...
				parsedDate = theCalendarDate.createDate(parseInt(aResult[3]), theCalendarDate.getMonthFromString(aResult[1]), parseInt(aResult[2]));
			} else {
				Log.warning("The provided string does not match the toDateString format: " + fromToDateString);
