...
sap.ui.define([
    ...
    function fnGetManifestByStructure() {
        var $XMLView = jQuery(".sapUiXMLView"),				// jQuery object: all XML views in current application
            $OVPWrapper = jQuery(".ovpWrapper"),			// jQuery object: OVP wrapper, only set in OVP applications
        ...
        if ($OVPWrapper && $OVPWrapper.length && $OVPWrapper.length > 0) {
            // OVP
            $XMLView = $OVPWrapper.closest(".sapUiXMLView"); // TODO: replace 'closest' for IE11 compatibility, use e.g. jQuery.parents or jQuery.parent instead
            if ($XMLView
                && $XMLView.length
                && $XMLView.length > 0
                && $XMLView[0]
    ...
    function fnGetComponentIDByStructure() {
        ...
        if (oManifest && CommonMethods.hasObjectContent(oManifest) && oManifest["sap.app"] && oManifest["sap.app"].id) {
            $XMLView = jQuery(".sapUiXMLView");				// jQuery object: all XML views in current application
            $OVPWrapper = jQuery(".ovpWrapper");				// jQuery object: OVP wrapper, only set in OVP applications
            if ($OVPWrapper && $OVPWrapper.length && $OVPWrapper.length > 0) {
                $XMLView = $OVPWrapper.closest(".sapUiXMLView"); // TODO: replace 'closest' for IE11 compatibility, use e.g. jQuery.parents or jQuery.parent instead
            }
    ...
    function fnGetManifestPath(sAppId) {
        ...
        // sAppIdForURL is created by replacing all the '.' to '/' to support sap.ui.require.toUrl API
        var sAppIdForURL = sAppId.replace(/\./g, "/");
        return sap.ui.require.toUrl(sAppIdForURL, "/manifest.json");
    }
     ...
     *
     * @param {string} sManifestPath absolute URL to manifest.json, can be received by using public method sap.ovp.support.lib.CommonChecks.getManifestURL()
     ...
     *
     * @param {sap.ovp.template.support.lib.CommonChecks.mFloorplans} sFloorplan floorplan
     ...
     *
     * @see sap.ovp.support.lib.CommonChecks.mFloorplans
     * @param {sap.ovp.support.lib.CommonChecks.mFloorplans} sFloorplan floorplan
