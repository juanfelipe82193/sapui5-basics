sap.ui.define([
    ...
    "sap/m/MessageToast",
    "sap/m/MessageBox",
    "sap/ovp/support/lib/Documentation",
    "sap/ui/thirdparty/jquery",
    "sap/base/Log"
], function (Controller, MessageToast, MessageBox, Documentation, jQuery, Log) {
    "use strict";
    ...
    function fnOnCopyDataPlain() {
        ...
        var sContent = "*************+Extracted by SAP Fiori Elements Diagnostics Plugin [FEDiagnosticOVP]+*************" + sNewLine;
    ...
    function fnOnCopyDataHTML() {
        ...
        var oProperties = oData.properties,						// properties to display in table
            $Anchor = document.getElementById(this.getView().byId("CopyDataHTML").getId()),
            oAnchor = $Anchor[0];

        var sContent = sOpening + "*************+Extracted by SAP Fiori Elements Diagnostics Plugin [FEDiagnosticOVP]+*************" + sSuffix;
        sContent += sPrefix + "Extracted on " + new Date().toUTCString() + sSuffix;
        sContent += sPrefix + "Host: <a href=\"" + oData.url + "\">" + oData.origin + "</a>" + sSuffix;
        sContent += sPrefix + "Application status: " + oData.status + sSuffix;
        if (oData.statusMessage) {
            sContent += sPrefix + "Notice: " + oData.statusMessage + sSuffix;
        }
        sContent += sPrefix + "Documentation: " + "<a href=\"" + Documentation.getDocuURL() + "\">" + Documentation.getDocuURL() + "</a>" + sSuffix;
        sContent += sNewLine;
        ...
        for (var i = 0, len = oProperties.length; i < len; i++) {
            if (oProperties[i].type === "string") {
                ...
                sContent += sPrefix + oProperties[i].name + ": <a href=\"" + oProperties[i].target + "\">" + oProperties[i].value + "</a>" + sSuffix;
            } else if (oProperties[i].type === "group") {
        ...
        // copy content to clipboard
        $Anchor.empty();
        jQuery(sContent).appendTo(oAnchor);
        try {
    ...
    function fnApplyCustomLayout() {
        var $GroupHeaders = jQuery(".diagnosticPropertiesGroupHeaderContent");
        if ($GroupHeaders.length === 0) {
            Log.debug("No headers found which should be customized");
            return;
        }
        var $GroupHeaderElement, $CellElement, $Cell;
        for (var i = 0; i < $GroupHeaders.length; i++) {
            $GroupHeaderElement = jQuery($GroupHeaders[i]);
            $CellElement = $GroupHeaderElement.parents("td:first");
            $Cell = jQuery("#" + $CellElement[0].id);
            // apply custom class, attribute colspan=2 and remove next cell
            $Cell.addClass("diagnosticPropertiesGroupHeader");
            $Cell.attr("colspan", 2);
            $Cell.next().remove();
        }
    }
    ...
    function fnShowDataRefreshed() {
        ...
        if (jQuery.sap.fioriElementsPluginID) {
            var $View = document.getElementById(jQuery.sap.fioriElementsPluginID);
            if ($View && $View.parent() && !$View.parent().hasClass("sapUiSupportHidden")) {
                MessageToast.show("Data refreshed");
            }
        }
    }

    return Controller.extend("sap.ovp.support.DiagnosticsTool.controller.DiagnosticsTool", {
        onInit: fnOnInit,
