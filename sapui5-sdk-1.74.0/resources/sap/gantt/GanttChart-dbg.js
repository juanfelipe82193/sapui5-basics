...
sap.ui.define([
	...
	"./drawer/ExpandedBackground",
	"./misc/AxisOrdinal",
	"./misc/Format",
	"./eventHandler/AutoScrollHandler",
	"./eventHandler/MouseWheelHandler",
	"./eventHandler/TimePeriodZoomHandler",
	"./axistime/ProportionZoomStrategy",
	"./config/TimeHorizon",
	"./misc/ShapeSelectionModel",
	"./misc/ShapeManager",
	"./eventHandler/ShapeResizeHandler",
	"./eventHandler/ShapeHoverHandler",
	"./eventHandler/TimePopoverHandler",
	"./eventHandler/BirdEyeHandler",
	// 3rd party lib
...
function (
	 ...
	 * @constructor
	 * @public
	 * @deprecated Since version 1.64. {@link sap.gantt.simple.GanttChartWithTable} should be used instead.
	 * @alias sap.gantt.GanttChart
	 */
	var GanttChart = GanttChartBase.extend("sap.gantt.GanttChart", /** @lends sap.gantt.GanttChart.prototype */ {
		metadata: {
			designtime: "sap/gantt/designtime/GanttChart.designtime",
			aggregations: {
	...
	GanttChart.prototype.init = function () {
		...
		// Init exposed TreeTable properties in GanttChart
		...
		this._oColumn = new sap.ui.table.Column({
			template: new sap.ui.core.HTML({
				content: "<div tabIndex = \" 0 \" style=\" opacity : 0 \">" + oRb.getText("XCEL_GANTT_CELL") + "</div>"
			})
		});
		...
		//init shape hover handler
		this._oShapeHoverHandler = new ShapeHoverHandler(this);

		this._oTimePopoverHandler = new TimePopoverHandler(this);

		//init bird eye operation handler
		this._oBirdEyeHandler = new BirdEyeHandler(this);
	...
	GanttChart.prototype._onRedrawRequest = function(oEvent){
		...
		if (sReasonCode !== "totalHorizonUpdated" && oValueBeforeChange) {
			this._oLastVisibleHorizon = oValueBeforeChange;

			if (sReasonCode !== "syncVisibleHorizon"){
				...
				this.fireEvent("_visibleHorizonUpdate", {reasonCode: sReasonCode, visibleHorizon: oVisibleHorizon, visibleWidth: iCurrentVisibleWidth, originEvent: oOriginEvent});
			}
		}
	...
	GanttChart.prototype.setTimeAxis = function (oTimeAxis) {
		...
		if (oTimeAxis) {
			...
			this.setAxisTimeStrategy(new ProportionZoomStrategy({
				totalHorizon: oTimeAxis.getPlanHorizon().clone(),
				visibleHorizon: oTimeAxis.getInitHorizon().clone(),
	...
	GanttChart.prototype.setTableProperties = function (oProperties) {
		if (oProperties.hasOwnProperty("selectionBehavior")) {
			if (oProperties.selectionBehavior !== sap.ui.table.SelectionBehavior.RowOnly) {
				delete oProperties.selectionBehavior;
				jQuery.sap.log.warning("Change GanttChart's TreeTable selectionBehavior is not allowed!");
			}
		}
	...
	GanttChart.prototype.setShapes = function (aShapes) {
		...
		if (this.getAxisTimeStrategy().getAxisTime() && aShapes && aShapes.length > 0) {
			this._shapesInited = false;
			this._ensureShapesCreated(function () {
	...
	GanttChart.prototype._bindAggregation = function (sName, oBindingInfo) {
		if (sName == "rows" && oBindingInfo){
			...
			return GanttChartBase.prototype._bindAggregation.apply(this, arguments);
		}
	};
	...
	GanttChart.prototype._detachEvents = function () {
		...
		$svgCtn.unbind("contextmenu", this._onContextMenu);
		// unbind mouse events
		...
		var sGanttKeyDown = "keydown." + this.getId();
		jQuery(document.body).unbind(sGanttKeyDown);
	...
	GanttChart.prototype._attachEvents = function () {
		...
		$svgCtn.bind("contextmenu", this._onContextMenu);
		...
		var sGanttKeyDown = "keydown." + this.getId();
		jQuery(document.body).bind(sGanttKeyDown, this._onKeyDown.bind(this));
		...
		this.attachEvent("_autoScrolling", this._onAutoScrolling, this);
	};

	GanttChart.prototype._onAutoScrolling = function (oEvent){
		if (this._oTimePeriodZoomHandler.isActive()){
			this._oTimePeriodZoomHandler.handleAutoScrolling(oEvent);
		}

		this._oShapeResizeHandler._handleShapeResizingWhenAutoScroll();
	};
	...
	GanttChart.prototype.syncMouseWheelZoom = function (oEventData){
		this._oMouseWheelHandler.handleEvent(oEventData.originEvent, true);
	};

	GanttChart.prototype.handleExpandChartChange = function(bExpanded, aChartSchemes, aSelectedIndices) {
		return this._ensureShapesCreated(function () {
			aSelectedIndices = aSelectedIndices ? aSelectedIndices : this._getSelectionHandler().getSelectedIndices();
	...
	GanttChart.prototype._getRowSpanWithData = function (oSelectedData, aExpandChartSchemes) {
		...
		for (var key = 0; key < aExpandChartSchemes.length; key++) {
			...
			if (this._oObjectTypesConfigMap[oSelectedData[sRowTypeName]] && this._oObjectTypesConfigMap[oSelectedData[sRowTypeName]].getExpandedChartSchemeKeys().length > 0) {
				...
				if (jQuery.inArray(sExpandScheme, aValidSchems) > -1) {
		...
		if (this._oChartSchemesConfigMap[sScheme]) {
			...
			jQuery.each(aShapesForChartScheme, function (iKey, oVal) {
				if (mShapeConfig[oVal]) {
					...
					if ((aModeKeys && aModeKeys.length > 0 && jQuery.inArray(sMode, aModeKeys) > -1) || aModeKeys.length == 0 || sMode == sap.gantt.config.DEFAULT_MODE_KEY) {
	...
	GanttChart.prototype.getAllSelections = function () {
		...
		return {"rows": selectedRows, "shapes": selectedShapes, "relationships": selectedRelationships};
	};

	GanttChart.prototype.getSelectedRows = function () {
		var aSelectedIndices = this._getSelectionHandler().getSelectedIndices();
		var aSelectedRowData = this._getDrawingData(undefined, aSelectedIndices);

		var aSelectedRows = [];
		for (var i = 0; i < aSelectedRowData.length; i++) {
			//filter out the expanded rows(this method should just collect the selected main rows, rather than
			//expanded rows)
			if (aSelectedRowData[i] && !aSelectedRowData[i].parentId) {
				aSelectedRows.push(aSelectedRowData[i]);
			}
		}
	...
	GanttChart.prototype.selectRowsAndShapes = function(aRowId, bExclusive) {
		...
		this._oShapeSelection.selectUnderlyingTableRows(aRowId, this._oTT, bExclusive);
	...
	/*
	 * If Device.os.name is mac, we support the meta key i.e. 'Command' for multiple shapes selection.
	 */
	...
	GanttChart.prototype._syncTTSelection = function(event){
		...
		var iIndex =  aVisibleRows[iRowIndex].getIndex();
		var oSelectionHandler = this._getSelectionHandler();
		// legacy selection plugin has the getSelectionMode function private (starts with _)
		var sSelectionMode = typeof oSelectionHandler.getSelectionMode === "function" ? oSelectionHandler.getSelectionMode() : oSelectionHandler._getSelectionMode();
		if (iIndex >= 0 && sSelectionMode !== sap.ui.table.SelectionMode.None) {
			if (sSelectionMode === sap.ui.table.SelectionMode.Single) {
				if (!oSelectionHandler.isIndexSelected(iIndex)) {
					oSelectionHandler.setSelectedIndex(iIndex);
				} else {
					oSelectionHandler.clearSelection();
				}
			} else {
				...
				if (bShift) {
					...
					var iSelectedIndex = oSelectionHandler.getSelectedIndex();
					if (iSelectedIndex >= 0) {
						if (iSelectedIndex < iIndex) {
							oSelectionHandler.addSelectionInterval(iSelectedIndex, iIndex);
						} else {
							oSelectionHandler.addSelectionInterval(iIndex, iSelectedIndex);
						}
					} else {
						oSelectionHandler.setSelectedIndex(iIndex);
					}
				} else if (!oSelectionHandler.isIndexSelected(iIndex)) {
					if (bCtrl) {
						oSelectionHandler.addSelectionInterval(iIndex, iIndex);
					} else {
						oSelectionHandler.setSelectedIndex(iIndex);
					}
				} else if (bCtrl) {
					oSelectionHandler.removeSelectionInterval(iIndex, iIndex);
				} else if (oSelectionHandler.getSelectedIndices().length > 1) {
					oSelectionHandler.setSelectedIndex(iIndex);
				}
			}
		}
		...
		if (oSourceShapeData && bEnableDnD) {
			...
			if (rowInfo) {
				...
				if (oEvent.target.getAttribute("x") && oEvent.target.getAttribute("width")) {
					...
					shapeY = y;
					shapeHeight = rowInfo.rowHeight / 2;
				} else {
					shapeX = x;
					shapeWidth = 1;
					shapeY = y;
				...
				var oDragStartPoint = {"x": x, "y": y, "shapeX": shapeX, "shapeWidth": shapeWidth, "shapeY": shapeY, "shapeHeight": shapeHeight};
				...
				if (this.getEnableShapeTimeDisplay()){
					this._oTimePopoverHandler.handleTimePopover(oEvent);
				}
			}
		}
	};
	...
	GanttChart.prototype._getDraggingDomObjs = function (oEvent) {
		...
		if (currentNode.parentElement) {
			oParentDom = currentNode.parentElement;
		} else if (currentNode.parentNode) {
			oParentDom = currentNode.parentNode;
		}
		if (oParentDom && oParentDom.getAttribute("class")) {
			var sClasses = oParentDom.getAttribute("class");
			var sTopShapeClass = sClasses.split(" ")[0];
			if (sap.ui.getCore().byId(sTopShapeClass) instanceof sap.gantt.shape.Group){
				result.groupDom = oParentDom;
				result.contentDoms = oParentDom.childNodes;
			}
		} else {
			result.groupDom = currentNode;
			result.contentDoms = [currentNode];
		}
		return result;
	};
	...
	GanttChart.prototype._handleShapeDragging = function (oEvent) {
		...
		if (dx > 3 || dy > 3) {
			...
			if (this._oDraggingData === undefined) {
				...
				var iShapeY = this._oDraggingData.dragStartPoint.shapeY;
				var iStartMouseX = this._oDraggingData.dragStartPoint.x;
				var iStartMouseY = this._oDraggingData.dragStartPoint.y;

				var svgContentWidth = 12;
				var svgContentHeight = 10;
				...
				if (aDragDiv.empty() || dragDiv === null) {
					...
					if (Device.browser.safari) {
						svgContentWidth = this._oDraggingData.dragStartPoint.shapeWidth;
						svgContentHeight = this._oDraggingData.dragStartPoint.shapeHeight;
					}

					var yCountText = svgContentHeight / 2;
					//set 'width' 'height' of the svg is to ensure the content in the groupDom can be visible in the svg wrapper
					var svgAttr = 'width="' + (svgContentWidth + 5) + '" ' + 'height="' + (svgContentHeight + 10) + '"';

					var oClonedDom = jQuery(this._oDraggingData.groupDom).clone();
					this._resetShadowRelativePosition(oClonedDom);

					//wrap the groupDom with a svg
					var sDragSvgHtml = '<svg ' + svgAttr + '>' + this._getOuterHTML(oClonedDom) + "</svg>";

					jQuery("#dragDropShadow").append(sDragSvgHtml);
					var oDragSvg = dragDiv.select("svg");
					oDragSvg.select("g").attr("opacity", 0.5);
					oDragSvg.append("text")
					.attr("x", svgContentWidth / 2 - fontSize / 2)
					.attr("y", yCountText + 4)
					.attr("fill", "blue")
				...
				var iCurrentX;
				var iMousePageX = this._getMouseXPos(oEvent);
				...
				var iMousePageY = oEvent.pageY;
				var iCurrentY = oEvent.pageY;//calculate current Y on the align of shape&row
				iCurrentY = iShapeY + iMousePageY - iStartMouseY;
				var shadowLeft = iCurrentX + "px";
				var shadowTop = iCurrentY + "px";
				var shadowWidth = svgContentWidth + 5 + "px";
				var shadowHeight = svgContentHeight + 5 + "px";

				d3.select("#dragDropShadow")
					.style({
						'left': shadowLeft,
						'top': shadowTop,
						'width': shadowWidth,
						'height': shadowHeight});
				jQuery(document.body).addClass("sapGanttDraggingCursor sapGanttDraggingOverflow");
				jQuery("svg").addClass("sapGanttDraggingCursor");
	...
	GanttChart.prototype._resetShadowRelativePosition =  function (oClonedDom, relativeX, relativeY) {
		var that = this;
		if (oClonedDom.attr("x") !== undefined) {
			if (!relativeX) {
				oClonedDom.attr("x", 0);
				oClonedDom.attr("y", 2);
			}
		...
		var aChildNodes = oClonedDom.children();
		if (aChildNodes && aChildNodes.length > 0) {
			if (!relativeX) {
				aChildNodes = aChildNodes.filter(function(index, oNode) {
					var oJQueryNode = jQuery(oNode);
					if (oJQueryNode.is('text')) { // skip text type
						return false;
					}
					if (oJQueryNode.attr("x") == undefined || oJQueryNode.attr("y") == undefined) {
						return false;
					}
					return true;
				});
				var iNodeX, iNodeY;
				aChildNodes.each(function(index, oNode){
					var oJQueryNode = jQuery(oNode);
					var x = oJQueryNode.attr("x");
					var y = oJQueryNode.attr("y") - 2;
					if (!iNodeX) {
						iNodeX = x;
					} else {
						iNodeX = x < iNodeX ? x : iNodeX;
					}
					if (!iNodeY) {
						iNodeY = y;
					} else {
						iNodeY = y < iNodeY ? y : iNodeY;
					}
				});
				if (!iNodeX || !iNodeY) {
					return;
				...
				aChildNodes.each(function(index, oNode) {
					var oJQueryNode = jQuery(oNode);
					oJQueryNode.attr("x", oJQueryNode.attr("x") - iNodeX);
					oJQueryNode.attr("y", oJQueryNode.attr("y") - iNodeY);
					that._resetShadowRelativePosition(oJQueryNode, iNodeX, iNodeY);
				});
			} else {
				aChildNodes.each(function(index, oNode) {
					var oJQueryNode = jQuery(oNode);
					if (oJQueryNode.attr("x") !== undefined) {
						oJQueryNode.attr("x", oJQueryNode.attr("x") - relativeX);
						oJQueryNode.attr("y", oJQueryNode.attr("y") - relativeY);
					}
					that._resetShadowRelativePosition(oJQueryNode, relativeX, relativeY);
				});
			}
	...
	GanttChart.prototype._handleShapeDragEnd = function (oEvent) {
		...
		jQuery("svg").removeClass("sapGanttDraggingCursor");
		jQuery(document.body).removeClass("sapGanttDraggingCursor sapGanttDraggingOverflow");
	...
	GanttChart.prototype.setDraggingData = function (oDraggingSource) {
		...
		if (this._oDraggingData !== undefined) {
			...
			jQuery(document.body).bind("mousemove.shapeDragDrop", this._oTimePopoverHandler._handleTimePopoverDragging.bind(this._oTimePopoverHandler));
			jQuery(document).bind("mouseup.shapeDragDrop", this._oTimePopoverHandler._handleTimePopoverEnd.bind(this._oTimePopoverHandler));

			this._setEventStatus("dragEnter");
		}
	};

	GanttChart.prototype.setTimePopoverData = function(oTimePopoverData) {
		this._oTimePopoverHandler.setCurrentPopoverData(oTimePopoverData);
	};
	...
	GanttChart.prototype._onSvgMouseLeave = function (oEvent) {
		...
		if (oEvent.target && (oEvent.target.id === this.getId() + "-svg-ctn"
			...
			/*
			 * Mouse move quickly may cause quick view open at the boundary shape.
			 * So invoke handler on mouse leave to remove delayed call.
			 */
			this._oShapeHoverHandler.handleShapeHover(null, oEvent);
		}
	};
	...
	GanttChart.prototype._onContextMenu = function (oEvent) {
	...
	GanttChart.prototype._onSvgDoubleClick = function (oEvent) {
		if (!this._bDragging && !this._oShapeResizeHandler.getIsResizing()
			&& !oEvent.ctrlKey && !oEvent.shiftKey
			&& !(Device.os.name === Device.os.OS.MACINTOSH && oEvent.metaKey)){
			var param = this._composeParameterForClickPosition(oEvent);
	...
	GanttChart.prototype._handleHover = function (oEvent) {
		...
		var oShapeData = d3.select(oEvent.target).data()[0];
		if (oShapeData) {
			var pointX = this._getMouseXPos(oEvent) - jQuery(this.getDomSelectorById("svg")).offset().left || oEvent.offsetX;
		...
		this._oShapeHoverHandler.handleShapeHover(oShapeData, oEvent);
		if (iRowIndex  > -1 &&
	...
	GanttChart.prototype._handleDragLeave = function (oEvent) {
		...
		this.fireChartDragLeave({
			...
			timePopoverData: this._oTimePopoverHandler.getCurrentPopoverData()
		});
	};
	...
	GanttChart.prototype._handleSelectionChange = function (oEvent) {
		...
		if (oShapeData !== undefined && this.isShapeSelectable(oShapeData, sClassId)) {
			...
			if (!(this.getParent() instanceof sap.gantt.GanttChartWithTable) || (this.getParent().getTableProperties().selectionBehavior !== sap.ui.table.SelectionBehavior.RowSelector)) {
				var param = this._composeParameterForClickPosition(oEvent);
	...
	GanttChart.prototype._onHSbScroll = function (oEvent) {
		if (this._initHorizonApplied) {
			...
			if (!this._bSuppressSetVisibleHorizon){
				...
				this._updateVisibleHorizon(oTimeHorizon, "horizontalScroll");
			}
			this._bSuppressSetVisibleHorizon = false;
	...
	GanttChart.prototype._getScrollRateByScrollLeft = function (iScrollLeft){
		...
		if (nScrollWidth !== nVisibleWidth){
			//ie, edge, ff, safari and cr are different in scroll bar handling in RTL mode. All below comments describe the behavior in RTL mode, not LTR mode
			//ie/edge: The scrollLeft() of scroll bar returns 0 if the scroll bar indicator is at the most right side.
			...
			//ie/edge: we doesn't use the value of scrollLeft() directly, because in ie, scrollLeft() returns the hidden part at right of scroll bar, do the conversion for ie and edge.
			...
			if (Core.getConfiguration().getRTL() === true && (Device.browser.msie || Device.browser.edge)) {
	...
	GanttChart.prototype._getHSBScrollLeftByScrollRate = function (fScrollRate){
		...
		if (Core.getConfiguration().getRTL() && (Device.browser.msie || Device.browser.edge)) {
	 ...
	 * @private
	 */
	GanttChart.prototype.syncVisibleHorizon = function (oTimeHorizon, iVisibleWidth, bKeepStartTime){
		...
		this._updateVisibleHorizon(oTargetVisibleHorizon, "syncVisibleHorizon");
	};
	...
	/**
	 * If parameter "sBirdEyeRange" is
	 * <code>sap.gantt.config.BirdEyeRange.AllRows</code>,
	 * GanttChart will do birdEye on total data set. Be careful that this
	 * may cause a low performance when the total data set is huge.
	 * @private
	 */
	GanttChart.prototype._doBirdEye = function (sBirdEyeRange) {
		this._oBirdEyeHandler.doBirdEye(sBirdEyeRange);
	};

	/**
	 * Returns visibleHorizon of current view based on the bird eye range
	 * @param {string} sBirdEyeRange Data range for bird eye. value can be <code>sap.gantt.config.BirdEyeRange.VisibleRows</code>
	 * or <code>sap.gantt.config.BirdEyeRange.AllRows</code>. if this parameter is not provided, then the second parameter should be provided.
	 * @param {number} iRowIndex row index, the largestHorizon will be calculated based on the shapes on the specified row. If the first parameter
	 * is provided, this row index will be ignored.
	 *
	 * @private
	 */
	GanttChart.prototype.getLargestHorizonByDataRange = function (sBirdEyeRange, iRowIndex, bIgnoreBuffer) {
		var aAllShapeInstances = this._oShapeManager.getAllShapeInstances();
		var aRowData = [];
		if (aAllShapeInstances && aAllShapeInstances.length > 0) {
			if (sBirdEyeRange === sap.gantt.config.BirdEyeRange.AllRows) {
				aRowData = aRowData.concat(this.getAllRowData());
			} else if (sBirdEyeRange === sap.gantt.config.BirdEyeRange.VisibleRows){
				aRowData = aRowData.concat(this._aFilteredRowData);
			} else if (!sBirdEyeRange && iRowIndex >= 0 && this._aFilteredRowData.length > iRowIndex){
				aRowData = aRowData.concat(this._aFilteredRowData[iRowIndex]);
			}
			this._oShapeManager.collectDataForAllShapeInstances(aRowData, this._oObjectTypesConfigMap, this._oChartSchemesConfigMap);
		}

		return this._oBirdEyeHandler.calculateLargestVisibleHorizon(aAllShapeInstances, bIgnoreBuffer);
	};

	/**
	 * GanttChart will do birdEye on the specified row.
	 * The row index starts from 0 and indicates the position in all rows, instead of visible rows
	 * @public
	 */
	GanttChart.prototype.doBirdEyeOnRow = function (iRowIndex) {
		var iFirstVisibleRow = this._getTTFirstRenderingRowIndex();
		iRowIndex = iRowIndex - iFirstVisibleRow;
		this._oBirdEyeHandler.doBirdEye(null, iRowIndex);
	};
	...
	GanttChart.prototype.jumpToPosition = function(value) {
		if (value instanceof Date) {
			...
			this._updateVisibleHorizon(this.getAxisTimeStrategy().getTotalHorizon(), "jumpToPosition");
		}
	};

	GanttChart.prototype._updateVisibleHorizon = function(oTimeHorizon, sReasonCode){
		var oAxisTimeStrategy = this.getAxisTimeStrategy();
		var nVisibleWidth = this.getVisibleWidth();
	...
	GanttChart.prototype._syncSvgHeightWithTT = function () {
		...
		$svg.height(jQuery(this._oTT.$().find(".sapUiTableCtrlRowScroll:not(.sapUiTableCHT)")).height());
	};
	...
	GanttChart.prototype._getDrawingData = function(aRange, aSelectedIndexs) {
		...
		var aRowIndexes = [],
			iBoundry;
		if (aSelectedIndexs) {
			aRowIndexes = aSelectedIndexs;
		} else {
			for (i = aRange[0]; i <= aRange[1]; i++ ){
				aRowIndexes.push(i);
			}
		}
		iBoundry = aRowIndexes.length;
		if (oBinding !== undefined){
			var oModel = oBinding.getModel();
			var aShapeDataNames = this.getShapeDataNames();
			for (i = 0; i < iBoundry; i++) {
				var oContext = this._oTT.getContextByIndex(aRowIndexes[i]);
				if (!oContext) {
					continue;
				} else {
	...
	GanttChart.prototype._prepareVerticalDrawingRange = function() {
		...
		var nFirstVisibleRow = this._getTTFirstRenderingRowIndex();
	...
	GanttChart.prototype.getVisibleWidth = function () {
		...
		var nVsbWidth = this.getTTVsbDom() != null ? this.getTTVsbDom().getBoundingClientRect().width : 0;
		var nVisibleWidth = $svgCtn.width() - (Core.getConfiguration().getRTL() ? 0 : nVsbWidth);
	...
	GanttChart.prototype._drawHeader = function () {
		...
		var oScreenWidth = window.screen.width;
		var aLabelList = this.getAxisTime().getTickTimeIntervalLabel(
				oAxisTimeStrategy.getTimeLineOption(), null, [0, this._oStatusSet.nWidth >= oScreenWidth ? this._oStatusSet.nWidth : oScreenWidth]);
	...
	GanttChart.prototype._drawNowLine = function () {
		...
		if (this.getEnableNowLine()) {
			this._oNowlineDrawer.drawSvg($GanttChartSvg, $GanttChartHeader, this.getNowLineInUTC());
		} else {
...
//		this._collectSelectedDataPerShapeKey();
	...
	GanttChart.prototype._ensureAxisTimeCreated = function () {
		var oAxisTime = this.getAxisTimeStrategy().getAxisTime();
		if (!oAxisTime) {
				this._createAxisTime(this.getAxisTimeStrategy());
				oAxisTime = this.getAxisTimeStrategy().getAxisTime();
		}
	};

	/**
	 * Tries to initiate shapes synchronously. Throws exception in case it fails.
	 * @private
	 */
	GanttChart.prototype._ensureShapesCreatedSync = function () {
		this._ensureAxisTimeCreated();
		if (!this._shapesInited) {
			var aShapes = this.getProperty("shapes");
			if (aShapes && aShapes.length > 0) {
				this._oShapeManager.instantiateShapesSync(aShapes);
			}
			this._shapesInited = true;
		}
	};

	GanttChart.prototype._ensureShapesCreated = function (fnCallback) {
		fnCallback = fnCallback || function () {};
		this._ensureAxisTimeCreated();

		if (!this._shapesInited) {
			// In most cases all objects are going to be loaded. There is not reason to introduce this async behaviour
			// all over the code. So we will fallback to async only in case it's needed and cross the fingers that
			// nothing goes wrong. The code is so relaying on sync behaviour, that a proper refactoring is not possible.
			var aShapes = this.getProperty("shapes");
			if (aShapes && aShapes.length > 0) {
				try {
					this._oShapeManager.instantiateShapesSync(aShapes);
					...
					this._oShapeManager.instantiateShapes(aShapes).then(function () {
						this._shapesInited = true;
						fnCallback();
	...
	GanttChart.prototype._getTTFirstRenderingRowIndex = function () {
		if (this._oTT.getRows().length > 0){
			return this._oTT.getRows()[0].getIndex();
		}
	...
	// for MS IE, outerHTML is not supported on svg element
	GanttChart.prototype._getOuterHTML = function (oDom) {
		if (Device.browser.msie) {
			return jQuery('<div>').append(oDom).html();
		}
		return oDom.prop("outerHTML");
	};
