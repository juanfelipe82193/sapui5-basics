...
sap.ui.define([
	...
	var ShapeResizeHandler = BaseObject.extend("sap.gantt.eventHandler.ShapeResizeHandler", {
		constructor : function (oChart) {
			...
			this._DEFAULT_RESIZE_CURSOR_SHOWING_THRESHOLD = 1;
	...
	ShapeResizeHandler.prototype.checkShapeResizable = function (oShapeData, sTarget, sPointX) {
		if (oShapeData && sTarget && sPointX) {
			...
			if (!sClassId || sClassId.indexOf("__") < 0) {
				return;
			}
			if (this._oShapeManager.isShapeResizable(oShapeData, sClassId)) {
				...
				if (hoverPointDistance <= this._DEFAULT_RESIZE_CURSOR_SHOWING_THRESHOLD && !this._oSourceChart._bDragging) {
	...
	ShapeResizeHandler.prototype._handleShapeResizingWhenAutoScroll = function () {
		if (this.getIsResizing()) {
			this._updateResizingShapeData(this._oLastEvent);
		}
	};
