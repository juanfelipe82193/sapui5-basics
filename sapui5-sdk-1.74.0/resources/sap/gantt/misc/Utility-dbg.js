 ...
 */
sap.ui.define(["sap/gantt/misc/Format", "sap/gantt/config/TimeHorizon", "sap/base/Log"], function (Format, TimeHorizon, Log) {
	"use strict";
	...
	Utility.findSapUiSizeClass = function (oControl) {
		...
		var sSizeCSSClassNames = ".sapUiSizeCompact,.sapUiSizeCondensed,.sapUiSizeCozy";
		if (!$rootDiv || $rootDiv.length === 0) {
			$sizeDef = jQuery(document.body).find(sSizeCSSClassNames);
		} else {
			$sizeDef = $rootDiv.closest(sSizeCSSClassNames);
		}
