...
sap.ui.define([
	 ...
	 *
	 * @param {array} aElementArray Element array corresponding to an ordinal axis.
	...
	var AxisOrdinal = function (aElementArray, aScaleArray, nViewBandWidth, nViewRangeStart,
	                            nViewRangeEnd, nZoomRate, nZoomOrigin, nPadding) {
		this.elementArray = aElementArray;
		this.scaleArray = aScaleArray;

		// padding, zoomRate and zoomOrigin are NOT Using any more
		this.padding = Utility.assign(nPadding, 0);
		this.zoomRate = Utility.assign(nZoomRate, 1);
		this.zoomOrigin = Utility.assign(nZoomOrigin, 0);

		this.viewRangeStart = Utility.assign(nViewRangeStart, 0);
	...
	AxisOrdinal.prototype._createScale = function(){
		if (typeof (this.viewBandWidth) !== "undefined"){
			...
			this.scale.domain(this._calculateInternalValuesByElements())
				.rangeBands(this._calculateViewRangeByViewBandWidth());
		} else if (typeof (this.viewRangeEnd) !== "undefined"){
			this.viewBandWidth = undefined;
			this.scale.domain(this._calculateInternalValuesByElements())
				.rangeBands([this.viewRangeStart, this.viewRangeEnd]);
		} else {
