...
//			this.mSelectedShapes.shapes.splice(iIndex, 1);
	...
	ShapeSelectionModel.prototype.deselectRelationship = function(sRelationshipUid) {
		...
		var bUpdated = jQuery.each(this.aSelectedRelationships, function (iIndex, relationship) {
			if (relationship && relationship.uid === sRelationshipUid) {
	...
	ShapeSelectionModel.prototype.selectUnderlyingTableRows = function (aIds, oTable, bExclusive) {
		...
		var oTableSelectionHandler = this._getSelectionHandler(oTable);
		var aTableSelectedIndices = oTableSelectionHandler.getSelectedIndices();
		if (bExclusive && aTableSelectedIndices.length > 0) {
			oTableSelectionHandler.clearSelection();
		}
		...
		for (var iIndex = 0; iIndex < aRowDatum.length; iIndex++) {
			...
			if (sSelectionMode === sap.gantt.SelectionMode.Multiple ||
				...
				oTableSelectionHandler.addSelectionInterval(oRowInfo.rowIndex, oRowInfo.rowIndex);
			} else {
				oTableSelectionHandler.setSelectedIndex(oRowInfo.rowIndex);
			}
	...
	ShapeSelectionModel.prototype.deselectUnderlyingTableRows = function (aIds, oTable) {
		if (!aIds || aIds.length === 0) {
			...
			this._getSelectionHandler(oTable).clearSelection();
			return;
		}
		...
		for (var iIndex = 0; iIndex < aRowDatum.length; iIndex++) {
			var oRowInfo = aRowDatum[iIndex];
			if (oRowInfo) {
				this._getSelectionHandler(oTable).removeSelectionInterval(oRowInfo.rowIndex, oRowInfo.rowIndex);
			}
		}
	};
	...
	ShapeSelectionModel.prototype._getGanttChart = function() {
		 ...
		 * @param {sap.ui.table.Table} oTable Table to get the selection handler from.
		 * @returns {sap.ui.table.Table|sap.ui.table.plugins.SelectionPlugin} Control which has standard APIs for table selection.
		 * @private
		 */
		ShapeSelectionModel.prototype._getSelectionHandler = function (oTable) {
			return oTable._oSelectionPlugin || oTable;
		};
