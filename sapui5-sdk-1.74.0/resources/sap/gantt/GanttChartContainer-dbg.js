...
function (
	 ...
	 * @constructor
	 * @public
	 * @deprecated Since version 1.64. {@link sap.gantt.simple.GanttChartContainer} should be used instead.
	...
	var GanttChartContainer = Control.extend("sap.gantt.GanttChartContainer", /** @lends sap.gantt.GanttChartContainer.prototype */ {
		metadata: {
			...
			properties: {
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				toolbarSchemes: {type: "object[]", defaultValue: library.config.DEFAULT_CONTAINER_TOOLBAR_SCHEMES},
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				containerLayouts: {type: "object[]", defaultValue: library.config.DEFAULT_CONTAINER_LAYOUTS},
				 ...
				 */
				containerLayoutKey: {type: "string", defaultValue: library.config.DEFAULT_CONTAINER_SINGLE_LAYOUT_KEY},
	...
	GanttChartContainer.prototype._insertGanttChart = function (oGanttChart, bReadConfig, iIndex) {
		...
		if ( oGanttChart === oFirstGanttChart){
			...
			oGanttChart.getAxisTimeStrategy()._setVisibleHorizon(oFirstGanttChart.getAxisTimeStrategy().getVisibleHorizon());
		}
	...
	GanttChartContainer.prototype._onToolbarBirdEye = function (oEvent) {
		...
		for (var i = 0; i < aGanttCharts.length; i++){
			var oLargestHorizon = aGanttCharts[i].getLargestHorizonByDataRange(sBirdEyeRange, undefined, true);
			if (oLargestHorizon) {
		...
		if (oTargetStartTime && oTargetEndTime) {
			...
			var nVisibleWidth = aGanttCharts[0].getVisibleWidth();
			var timeRangePerPixel = (oTargetEndTime.getTime() - oTargetStartTime.getTime()) / nVisibleWidth;
			var oTargetStartTimeWithBuffer = new Date(oTargetStartTime.getTime() - timeRangePerPixel * 5);
			var oTargetEndTimeWithBuffer = new Date(oTargetEndTime.getTime() + timeRangePerPixel * 5);

			var oTargetVisibleHorizon = new TimeHorizon({
				startTime: oTargetStartTimeWithBuffer,
				endTime: oTargetEndTimeWithBuffer
			});
	...
	GanttChartContainer.prototype._onZoomInfoUpdated = function (oEvent) {
		...
		if (!isNaN(iZoomLevel)){
			this._oToolbar.updateZoomLevel(iZoomLevel);
		}
	};
	...
	GanttChartContainer.prototype._onChartDragEnter = function (oEvent) {
		...
		if (oSourceEvent.button === 0 && oSourceEvent.buttons !== 0 && this._oDraggingSource !== undefined) {
			...
			if (this._oTimePopoverData) {
				oGanttChart.setTimePopoverData(this._oTimePopoverData);
			}
