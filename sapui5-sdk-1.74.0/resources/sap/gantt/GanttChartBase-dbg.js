...
sap.ui.define([
	...
	'sap/ui/core/Control',
	"./misc/Utility"
], function (jQuery, library, Control, Utility) {
	"use strict";
	...
	var GanttChartBase = Control.extend("sap.gantt.GanttChartBase", /** @lends sap.gantt.GanttChartBase.prototype */ {
		metadata: {
			...
			properties: {
				 ...
				 */
				mode: {type: "string", defaultValue: library.config.DEFAULT_MODE_KEY},
				 ...
				 * @deprecated
				 *
				 * Please use <code>tableProperties</code>(e.g. <code>setTableproperties({selectionMode: "None"})</code>)
				 ...
				 */
				selectionMode: {type : "sap.gantt.SelectionMode", defaultValue : library.SelectionMode.MultiWithKeyboard},
				...
				/**
				 * This property control the size of the selection panel. The value will be overwritten by property <code>selectionPanelSize</code>
				 * in <code>sap.gantt.config.ContainerLayout</code> if you are using GanttChartContainer
				 *
				 * In the current library, <code>sap.gantt.GanttChart</code> does not have a selection panel.
				 * <code>sap.gantt.GanttChartWithTable</code> has a selection panel implemented by <code>sap.ui.table.TreeTable</code>.
				 *
				 * @see {sap.gantt.GanttChartContainer#containerLayoutKey}
				 * @see {sap.gantt.GanttChartContainer#containerLayouts}
				 * @see {sap.gantt.config.ContainerLayout#selectionPanelSize}
				 */
				 ...
				 */
				hierarchyKey: {type: "string", defaultValue: library.config.DEFAULT_HIERARCHY_KEY},

				/**
				 * row height of Gantt Chart.
				 * If this property is set, then it will change the Table row height on the left
				 */
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				timeAxis: {type: "object", defaultValue: library.config.DEFAULT_TIME_AXIS},
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				modes: {type: "object[]", defaultValue: library.config.DEFAULT_MODES},
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				toolbarSchemes: {type: "object[]", defaultValue: library.config.DEFAULT_GANTTCHART_TOOLBAR_SCHEMES},
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				hierarchies: {type: "object[]", defaultValue: library.config.DEFAULT_HIERARCHYS},
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				objectTypes: {type: "object[]", defaultValue: library.config.DEFAULT_OBJECT_TYPES},
				 ...
				 * Limitation: It's defined as a property thus can not be used in XML View
				 */
				chartSchemes: {type: "object[]", defaultValue: library.config.DEFAULT_CHART_SCHEMES},
				 ...
				 * 		shapeDataNames: ["task", "order"],
				 *      shapes: [
				 *        new sap.gantt.shape.Shape({
				 *           key: "calendar",
				 *           shapeDataName: "task"
				 ...
				 * or in XMLView
				 * <code>
				 *    &lt;GanttChartWithTable shapeDataNames="task,order"&gt;
				 *    &lt;/GanttChartWithTable&gt;
				 * </code>
				 */
	...
	GanttChartBase.prototype.setTableProperties = function (oProperties) {
		...
		Object.keys(oProperties).forEach(function (sKey) {
			if (this.mDefaultTableProperties.hasOwnProperty(sKey)) {
				...
				var sFunctionName = "set" + sKey[0].toUpperCase() + sKey.slice(1);
				if (sFunctionName === "setSelectionMode") {
					this._getSelectionHandler().setSelectionMode(oProperties[sKey]);
				} else {
					this._oTT[sFunctionName](oProperties[sKey]);
				}
			} else {
	...
	GanttChartBase.prototype.getTableProperties = function () {
		...
		Object.keys(oTableProperties).forEach(function (sKey) {
			if (sKey === "selectionMode") {
				...
				oTableProperties[sKey] = typeof oSelectionHandler.getSelectionMode === "function" ? oSelectionHandler.getSelectionMode() : oSelectionHandler._getSelectionMode();
			} else {
	...
	/**
	 * Getter for the selection handler which is currently being used by the Table.
	 * @returns {sap.ui.table.Table|sap.ui.table.plugins.SelectionPlugin} Control which has standard APIs for table selection.
	 * @private
	 */
	GanttChartBase.prototype._getSelectionHandler = function () {
		return this._oTT._oSelectionPlugin || this._oTT;
	};
