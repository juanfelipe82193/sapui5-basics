...
sap.ui.define(
	...
	function(JSONModel, ODataListBinding, Context, Filter, ManagedObject, ChangeReason, ResourceModel, ODataContextBinding, Log) {
		"use strict";
		 ...
		 * individual state
		 * @alias sap.fe.core.model.DraftModel.EDITSTATE
		 * @readonly
		 * @enum {String}
		 * @private
		 * @sap-restricted
		...
		function getFilterForEditState(sEditState) {
			...
			switch (sEditState) {
				...
				case EDITSTATE.LOCKED:
					sFilter =
						"(IsActiveEntity eq true and SiblingEntity/IsActiveEntity eq null and DraftAdministrativeData/InProcessByUser ne '')";
					break;
				case EDITSTATE.UNSAVED_CHANGES:
					sFilter =
						"(IsActiveEntity eq true and SiblingEntity/IsActiveEntity eq null and DraftAdministrativeData/InProcessByUser eq '')";
					break;
				default:
					//EDITSTATE.ALL
		...
		function executeDraftActivationAction() {
			var oContextWithCache = getContextWithCache(this);

			if (!oContextWithCache.getProperty("IsActiveEntity")) {
		...
		function executeDraftEditAction(preserveChanges) {
			...
			if (oContextWithCache.getProperty("IsActiveEntity")) {
				var oOperation = createOperation(this, arguments[0]); //TODO wait for ABAP support of {$$inheritExpandSelect : true});
				/* Fix arguments */
		...
		function mergeEditStateFilterToFilter(sEditState, sFilter) {
			...
			if (sFilter) {
				...
				aResult = sFilter.match(REGEXFOREDITSTATEFILTER);
				if (Array.isArray(aResult) && aResult[0]) {
					sEditStateFilter = aResult[0];
					sFilter = sFilter.replace(sEditStateFilter, "");
					//Remove first and last paranthesis */
					sFilter = sFilter.substr(1).slice(0, -1);
				}
			}
			//Is there still a filter
			if (sFilter) {
		...
		function _upgrade(oModel) {
				...
				if (sPath) {
					...
					Object.keys(mListBindings).forEach(function(sKey) {
						var oBinding = mListBindings[sKey],
							mParameters = oBinding.mParameters,
							sCurrentFilter = "",
							sEditStateFilter = "",
							aResult = [];
						...
						if (oBinding instanceof ODataListBinding && oBinding.getPath() === sPath) {
							...
							oBinding.reset(sap.ui.model.ChangeReason.Filter);
						}
					});
				}
			});
			...
			oModel.bindList = function(sPath, oContext, vSorters, vFilters, mParameters) {
				...
				if (oEntitySetState) {
					...
					/* enable $count */
					fnOriginal.bindProperty = fnOriginal.bindProperty || oModel.bindProperty.bind(oModel);
					oModel.bindProperty = function(sPath, oContext, mParameters) {
						if (sPath.indexOf(":$count") > -1) {
							var sNamedBinding = sPath.split(":$")[0];
							return fnOriginal.bindProperty("$count", oModel.mNamedBindings[sNamedBinding].getHeaderContext(), mParameters);
						}
						return fnOriginal.bindProperty.apply(this, arguments);
					};
				}
				return oListBinding;
			};
		...
		var DraftModel = {
			...
			EDITSTATE: EDITSTATE,
			_getFilterForEditState: getFilterForEditState
		};
