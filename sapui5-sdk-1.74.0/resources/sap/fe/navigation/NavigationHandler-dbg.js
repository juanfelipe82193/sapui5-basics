...
sap.ui.define(
	...
	function(Error, SelectionVariant, NavLibrary, BaseObject, ResourceModel, UIComponent, HashChanger, jQuery, Log, assert) {
		 ...
		 */
		var NavigationHandler = BaseObject.extend(
			"sap.ui.generic.app.navigation.service.NavigationHandler" /** @lends sap.ui.generic.app.navigation.service.NavigationHandler */,
			{
				metadata: {
					publicMethods: [
						"navigate",
				...
				constructor: function(oController, sParamHandlingMode) {
					...
					if (
						sParamHandlingMode === sap.ui.generic.app.navigation.service.ParamHandlingMode.URLParamWins ||
						sParamHandlingMode === sap.ui.generic.app.navigation.service.ParamHandlingMode.InsertInSelOpt
					) {
					...
					for (var sPropertyName in mSemanticAttributes) {
						if (mSemanticAttributes.hasOwnProperty(sPropertyName)) {
							...
							if (jQuery.type(vSemanticAttributeValue) === "array" || jQuery.type(vSemanticAttributeValue) === "object") {
								...
								jQuery.type(vSemanticAttributeValue) === "number" ||
								jQuery.type(vSemanticAttributeValue) === "boolean"
							) {
							...
							if (vSemanticAttributeValue === undefined) {
								if (
									iSuppressionBehavior & sap.ui.generic.app.navigation.service.SuppressionBehavior.raiseErrorOnUndefined
								) {
									throw new Error("NavigationHandler.INVALID_INPUT");
								} else {
									Log.warning(
