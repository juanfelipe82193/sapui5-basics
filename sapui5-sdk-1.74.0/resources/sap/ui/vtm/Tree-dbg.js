    ...
    function (
        ...
        var Tree = SapUiCoreControl.extend("sap.ui.vtm.Tree", /** @lends sap.ui.vtm.Tree.prototype */ {
            ...
            constructor: function (sId, mSettings) {
                ...
                treeTable.setCollapseRecursive(false);

                treeTable.attachToggleOpenState(this._handleToggleOpenState.bind(this));

                treeTable.attachCellClick(function(e) {
                    if (this.getSelectionMode() == sap.ui.vtm.SelectionMode.MultiToggleWithSingleSelect) {
                        var rowIndex = e.getParameter("rowIndex");
                        var treeItem = this._getItemByRowIndex(rowIndex);
                        if (treeItem) {
                            this._selectedItems = [treeItem];
                            treeTable.setSelectedIndex(rowIndex);
                        } else {
                            // Clear selection by clicking in empty space
                            this._selectedItems = [];
                ...
                treeTable.attachRowSelectionChange(function (event) {
                    ...
                    var addedItems = this._getTreeItemsFromRowIndices(addedItemIndices);
                    var removedItems = this._getTreeItemsFromRowIndices(removedItemIndices);

                    if (userInteraction) {
                        var removedItemIds = removedItems.map(function(treeItem) {
                            return treeItem.id;
                        });

                        var idsEqual = function(a,b) { return a.id == b.id; };
                        var notRemoved = function(treeItem) { return removedItemIds.indexOf(treeItem.id) == -1; };

                        this._selectedItems = SapUiVtmArrayUtilities.union([this._selectedItems.filter(notRemoved), addedItems], idsEqual);
                    }
            ...
            _handleToggleOpenState: function(oEvent) {
                ...
                var treeItem = this.getItemByBindingContextPath(contextPath);
             ...
             * @returns {Promise} A promise that resolves when expansion has completed
             */
            _expandToTreeItem: function(treeItem, scrollIntoView) {
                        ...
                        setTimeout(function() {
                            if (ancestors.length) {
                                while (ancestors.length) {
                                    ...
                                    if (rowIndex === -1) {
                                        // This can happen due to events from unrelated expansions.
                                        return;
                                    } else {
                                        ancestors.shift();
            ...
            expandAncestors: function (treeItem) {
                sap.ui.vtm.measure(this, "expandAncestors", function() {
                    this._expandToTreeItem(treeItem, false);
                }.bind(this));
                return this;
            },
            ...
            scrollIntoView: function (treeItem) {
                sap.ui.vtm.measure(this, "scrollIntoView", function() {
                    this._expandToTreeItem(treeItem, true);
                }.bind(this));
                return this;
            },
            ...
            setSelectedItems: function (items, scrollToView) {
                ...
                if (items.length) {
                    // filter the selection set to only include tree items that currently exist in the tree
                    var treeItemIds = [];
                    this.traverseTree(function(treeItem) {
                        treeItemIds.push(treeItem.id);
                    });

                    items = items.filter(function(treeItem) {
                        return treeItemIds.indexOf(treeItem.id) != -1;
                    });
                }
                ...
                items.forEach(function(item) {
                    if (!chain) {
                        chain = this._expandToTreeItem(item, scrollToView);
                    } else {
                        chain = chain.then(function() {
                            return this._expandToTreeItem(item, scrollToView);
                        }.bind(this));
                    }
                }.bind(this));

                chain.then(selectTreeItems);
            ...
            updateModel: function (forceRecreate) {
                sap.ui.vtm.measure(this, "updateModel", function() {
                    ...
                    this.setSelectedItems(this._selectedItems, false);
            ...
            /**
             * Finds a tree item by binding context path or returns undefined if it is not found.
             * This is typically used to find the corresponding tree item in event handlers
             * for custom column template controls.
             * @public
             * @function
             * @param {string} bindingContextPath The binding context path to look for.
             * @returns {object|undefined} The tree item matching the specified binding context path or undefined if no match was found.
             */
            getItemByBindingContextPath: function (bindingContextPath) {
                return this._getTreeTable().getModel().getProperty(bindingContextPath) || undefined;
            },
            ...
            _getItemByRowIndex: function (index) {
                ...
                return this.getItemByBindingContextPath(path);
            },
            ...
            setSelectionMode: function(value) {
                ...
                case sap.ui.vtm.SelectionMode.MultiToggle:
                    ...
                    break;
                case sap.ui.vtm.SelectionMode.MultiToggleWithSingleSelect:
                    treeTableSelectionMode = sap.ui.table.SelectionMode.MultiToggle;
            ...
            _createColumn: function (column) {
                ...
                this._attachColumnHeaderEvents(column);
                var settings = {
            ...
            /**
             * Attaches event handlers to internal column headers.
             * @private
             * @function
             * @param {sap.ui.vtm.Column} column The column definition.
             * @return {sup.ui.core.Control} The column header control.
             */
            _attachColumnHeaderEvents: function (column) {
                var headerControl = column.getLabelControl();
                if (!headerControl) {
                    return;
                }
                ...
                switch (type) {
                    case sap.ui.vtm.ColumnType.Internal: {
                        ...
                        if (!columnDescriptor) { throw "Column descriptor not specified"; }
                        switch (columnDescriptor) {
                            case sap.ui.vtm.InternalColumnDescriptor.MessageStatus:
                                headerControl.attachPress(function() {
                            ...
                            case sap.ui.vtm.InternalColumnDescriptor.Visibility:
                                headerControl.attachPress(function() {
                                    this.fireVisibilityHeaderIconClicked({
                                        visibility: headerControl.getVisibility(),
                                        control: headerControl
                                    });
                                }.bind(this));
                                break;
                        ...
                        switch (columnDescriptor) {
                            case sap.ui.vtm.InternalColumnDescriptor.MessageStatus:
                            case sap.ui.vtm.InternalColumnDescriptor.Visibility:
                                headerControl.addEventDelegate({
                                    onfocusin: function(event) {
                                        headerControl.getDomRef().blur();
                                    }
                                }, headerControl);
                                break;
                            default:
                        ...
                        switch (columnDescriptor) {
                            ...
                            case sap.ui.vtm.InternalColumnDescriptor.MessageStatus:
                                ...
                                messageStatusColumnTemplate.attachPress(function(oEvent) {
                                    ...
                                    var item = this.getItemByBindingContextPath(sPath);
                            ...
                            case sap.ui.vtm.InternalColumnDescriptor.Visibility:
                                ...
                                var handleChange = function(control) {
                                ...
                                sceneNodeVisibilityColumnTemplate.attachPress(function(event) {
                                    var control = event.getSource();
                                    handleChange(control);
                                });
                                sceneNodeVisibilityColumnTemplate.addEventDelegate({
                                    onfocusin: function(event) {
                                        var core = sap.ui.getCore();
                                        var focusedControlId = core.getCurrentFocusedControlId();
                                        var focusedControl = core.byId(focusedControlId);
                                ...
                                }, sceneNodeVisibilityColumnTemplate);
