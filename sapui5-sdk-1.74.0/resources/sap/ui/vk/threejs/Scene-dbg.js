...
sap.ui.define([
	...
	var Scene = SceneBase.extend("sap.ui.vk.threejs.Scene", /** @lends sap.ui.vk.threejs.Scene.prototype */ {
		...
		constructor: function(scene) {
			...
			this._layersMask = (1 | 0); // three.js visibility layers, the channel 0 is allocated by default
		}
	});
	...
	Scene.prototype._computeBoundingBox = function(visibilityLayers, ignoreDynamicObjects) {
		var boundingBox = new THREE.Box3();
		if (this._scene) {
			this._scene._expandBoundingBox(boundingBox, visibilityLayers, ignoreDynamicObjects);
		}
		return boundingBox;
	};
	...
	Scene.prototype._setSceneBuilder = function(sceneBuilder) {
	 ...
	 * @param {string|string[]} sid The persistent id
	 * @param {string|string[]} sceneId scene id
	 * @returns {boolean} true if assignment is successful, false if persistent id already exists or cannot be assigned for other reasons
	 * @private
	 */
	...
	function createMergedGeometry(node) {
		var geometry = null;
		if (node.isMesh && node.geometry && !box3IsNull(node.geometry.boundingBox) && (node.layers !== node.parent.layers)) {
		...
		for (var i = 0, l = node.children.length; i < l; i++) {
			...
			if (child.isMesh && child.geometry && !box3IsNull(child.geometry.boundingBox) && child.layers === node.layers) {
				if (geometry === null) {
	...
	Scene.prototype._allocateLayersChannel = function() {
		for (var channel = 0; channel < 32; channel++) {
			if ((this._layersMask & (1 << channel | 0)) === 0) {
				this._layersMask |= (1 << channel | 0);
				return channel;
			}
		}
		return -1;
	...
	Scene.prototype._freeLayersChannel = function(channel) {
		this._layersMask &= ~(1 << channel | 0);
	};
