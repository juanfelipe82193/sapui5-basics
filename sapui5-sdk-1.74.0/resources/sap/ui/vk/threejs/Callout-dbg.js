...
sap.ui.define([
	...
	Callout.prototype._update = function(renderer, camera, viewportSize) {
		...
		this._lines.forEach(function(line) {
			...
			if (line.userData.extensionLength > 0 && vertices.length > 2) {
				...
				v1.applyMatrix4(this._billboard.matrixWorld).applyMatrix4(mat.getInverse(line.matrixWorld));
			}
			...
			if (hideLastSegment) {
				...
				v2.applyMatrix4(this._billboard.matrixWorld).applyMatrix4(mat.getInverse(line.matrixWorld));
			}
			...
			if (markMesh !== undefined) {
				...
				if (pos4.w >= nearZ) {
					...
					markMesh.matrix.compose(markMesh.position, markMesh.quaternion, markMesh.scale);
					...
					if (hideFirstSegment || !markMesh.visible) {
						...
						v0.set(markMesh.userData.lineOffset, 0, 0).applyMatrix4(markMesh.matrixWorld).applyMatrix4(mat.getInverse(line.matrixWorld));
					}
				}
			}
	...
	Callout.prototype._createMarkMesh = function(color, lineStyle, headStyle, styleConstant) {
		...
		canvas.width = THREE.Math.ceilPowerOfTwo(width * pixelRatio);
		canvas.height = THREE.Math.ceilPowerOfTwo(height * pixelRatio);
	...
	Callout.prototype.addLeaderLine = function(vertices, targetNode, material, startPointStyle, endPointStyle, styleConstant, extensionLength) {
		...
		polylineMesh.layers = node.layers;
		...
		// var line = new THREE.Line(geometry, material.clone());
		// line.userData.targetNode = targetNode;
		// line.matrixAutoUpdate = false;
		// line.renderOrder = this.getRenderOrder();
		// line.material.transparent = true;
		// line.material.depthTest = this.getDepthTest();
		// node.add(line);
