...
sap.ui.define([
	...
	NodeHierarchy.prototype.getChildren = function(nodeRef, stepIntoClosedNodes) {
		...
		if (stepIntoClosedNodes || !nodeRef.userData.closed) {
			nodeRef.children.forEach(function(child) {
				if (!child.private && (child.geometry === undefined || child.layers !== nodeRef.layers)) {// ignore private and geometry nodes
					children.push(child);
				}
			});
		}
		...
		if (query === undefined || query === null || jQuery.isEmptyObject(query)) {
			...
			this._scene.getSceneRef().children.forEach(function(rootChild) {
				rootChild.traverse(function(obj3D) {
					if (!obj3D.private && (obj3D.geometry === undefined || obj3D.layers !== obj3D.parent.layers)) {// ignore private and geometry nodes
						result.push(obj3D);
					}
				});
			});
			...
			if (searchModeFunction === undefined) {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT8.summary), Messages.VIT8.code, "sap.ui.vk.threejs.NodeHierarchy");
			} else if (!Array.isArray(query.value) && typeof query.value !== "string") {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT6.summary), Messages.VIT6.code, "sap.ui.vk.threejs.NodeHierarchy");
			} else {
	...
	*/
	 ...
	 * @since 1.48.0
	 */
	NodeHierarchy.prototype.createNode = function(parentNode, name, insertBeforeNode, nodeContentType) {
		if (!parentNode) {
		...
		jQuery.sap.assert(parentNode.children[parentNode.children.length - 1] === object3D, getResourceBundle().getText("NODEHIERARCHY_MSG_CREATENODEFAILED"));
		if (insertIndex >= 0) {
	 ...
	 * @param {THREE.Object3D} nodeRef	The reference object of node
	 * @returns {sap.ui.vk.NodeContentType} node content type (Regular or Reference)
	 * @public
	 * @since 1.73.0
	 */
	...
	NodeHierarchy.prototype.createNodeCopy = function(nodeToCopy, parentNode, name, insertBeforeNode) {
		...
		jQuery.sap.assert(parentNode.children[parentNode.children.length - 1] === clone, getResourceBundle().getText("NODEHIERARCHY_MSG_CREATENODEFAILED"));
		if (insertIndex >= 0) {
			parentNode.children.splice(insertIndex, 0, parentNode.children.pop());
		}
