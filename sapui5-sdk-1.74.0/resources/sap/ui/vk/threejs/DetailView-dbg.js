...
sap.ui.define([
		...
		metadata: {
			properties: {
				...
				origin: {
					...
					defaultValue: new THREE.Vector3(0, 0)
				},
				size: {
					type: "any", // THREE.Vector2
					defaultValue: new THREE.Vector3(0.5, 0.5)
				},
				attachmentPoint: {
	...
	DetailView.prototype._createBoxViewport = function(sx, sy, attachmentPoint, shape) {
		...
		geometry.addAttribute("position", new THREE.Float32BufferAttribute(vpPoints, 3));
		geometry.addAttribute("uv", new THREE.Float32BufferAttribute([ 0, 0, 1, 0, 1, 1, 0, 1 ], 2));
	...
	DetailView.prototype._createCircleViewport = function(rx, ry, attachmentPoint, shape) {
		...
		var vertexCount = THREE.Math.clamp(Math.round(ellipsePerimeter(rx, ry) / 24), 32, 256);
	...
	DetailView.prototype._createCircleBorder = function(vpPoints, rx, ry, attachmentPoint, shape) {
		...
		if (attachmentPoint && (shape !== DetailViewShape.Circle && shape !== DetailViewShape.CircleLine)) {
			...
			triPoints[ 0 ] *= l;
			triPoints[ 1 ] *= l;
			triGeometry.addAttribute("position", new THREE.Float32BufferAttribute(triPoints, 3));
	...
	DetailView.prototype._updateGeometry = function(size, viewportSize, camera) {
		...
		if (attachmentPoint) {
			...
			var ax = (pos4.x / pos4.w) * viewportSize.width / viewportSize.height,
				ay = pos4.y / pos4.w;
			aPoint.set(ax - origin.x, ay - origin.y, 0).multiplyScalar(viewportSize.height);
		}
	...
	DetailView.prototype._updateBorderTexture = function(pixelRatio) {
		...
		var textureWidth = THREE.Math.ceilPowerOfTwo(this._borderWidth);
	...
	function setCutawayProjectionMatrix(camera, position, size, viewportSize) {
		var w = size.x / viewportSize.width;
		var h = size.y / viewportSize.height;
		...
		var rect = {};
		rect.left = THREE.Math.lerp(srcRect.left, srcRect.right, x);
		rect.right = THREE.Math.lerp(srcRect.left, srcRect.right, x + w);
		rect.top = THREE.Math.lerp(srcRect.top, srcRect.bottom, 1 - y - h);
		rect.bottom = THREE.Math.lerp(srcRect.top, srcRect.bottom, 1 - y);
	...
	DetailView.prototype._render = function(renderer, camera, scene, boundingBox, eyePointLight) {
		...
		size.copy(this.getSize()).multiplyScalar(viewportSize.height * 0.5);
		size.set(Math.round(size.x) << 1, Math.round(size.y) << 1);

		position.set(origin.x * viewportSize.height / viewportSize.width, origin.y, 0).unproject(camera);

		// calculate screen position
		pos4.copy(position).applyMatrix4(camera.matrixWorldInverse).applyMatrix4(camera.projectionMatrix);
		var sx = ((pos4.x / pos4.w) * 0.5 + 0.5) * viewportSize.width,
			sy = ((pos4.y / pos4.w) * 0.5 + 0.5) * viewportSize.height;

		// set scale
		var scale = pos4.w / (viewportSize.width * camera.projectionMatrix.elements[ 0 ]);
		...
		if (Array.isArray(targetNodes)) {
			targetNodes.forEach(function(node) {
				node.userData.mask = node.layers.mask;
				node.layers.mask &= invMask;
			});
		}

		renderer.setClearColor(this._backgroundColor, 1);
		renderer.render(scene, dvCamera, this._renderTarget, true);
		...
		if (Array.isArray(visibleNodes)) {
			visibleNodes.forEach(function(node) {
				node.layers.mask = node.userData.mask;
			});
		}
		if (Array.isArray(targetNodes)) {
			targetNodes.forEach(function(node) {
		...
		scene.children.forEach(function(root) {
			if (root.userData._vkDynamicObjects) {
				root.userData._vkDynamicObjects.forEach(function(object) {
					if (object.layers.test(camera.layers) && object.isBillboard) {
						object.visible = true; // show screen-space objects
					}
				});
			}
		});
