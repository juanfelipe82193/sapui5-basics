...
function(
	...
	NativeViewport.prototype._handleResize = function(event) {
		...
		if (!event.oldSize || !event.oldSize.width || !event.oldSize.height || this._svgError) {
			this._bestFit();
		} else {
			this._update();
		}
	...
	NativeViewport.prototype._bestFit = function() {
		...
		this._s4BestFit = scale; // saving the scale used for best fit for zoom out limit
		this._s = scale;
		this._x = 0;
		this._y = 0;
		this._update();
	...
	NativeViewport.prototype.loadUrl = function(url, onload, onerror, onprogress, resourceType) {
		...
		if (/^(svg)$/.test(resourceType.toLowerCase())) {
			...
			this._svg.onerror = function() {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT1.summary), Messages.VIT1.code, "sap.ui.vk.NativeViewport");
				this._reset();
				if (onerror) {
					onerror();
				}
			...
			this._img.onerror = function() {
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT2.summary), Messages.VIT2.code, "sap.ui.vk.NativeViewport");
				if (onerror) {
					onerror();
				}
			};
			...
			jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT3.summary), Messages.VIT3.code, "sap.ui.vk.NativeViewport");
			if (onerror) {
				onerror();
			}
		}
