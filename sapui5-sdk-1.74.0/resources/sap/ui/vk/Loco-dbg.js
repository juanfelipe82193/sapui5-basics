...
sap.ui.define([
	...
	function eventToInput(event) {
		...
		if ((event.buttons & 2) && points.length === 1) {// if right button is pressed, then emulate multitouch pan
	...
	Loco.prototype.attachViewportEventHandlers = function(viewport) {
		...
		viewport.ontouchend = function(event) {
			// console.log("up", event.isMarked(), event);
			var input = eventToInput(event);
			this._endGesture(input);
		...
		viewport.ontap = function(event) {
			...
			if (!event.isMarked() && !this._touchMoved) {
				if (this._clickTimer > 0) {
					...
					this._click(eventToInput(event), true); // double tap
				} else {
					this._clickTimer = setTimeout(function(loco) {
						loco._click(eventToInput(event), false); // single tap
					}, 250, this); // double tap timeout
				}
			}
		}.bind(this);
	 ...
	 *
	 * @param {handler} handler The event handler to be added.
	 * @param {priority} priority The priority of the handler.
	 *
	 * @public
	 */
