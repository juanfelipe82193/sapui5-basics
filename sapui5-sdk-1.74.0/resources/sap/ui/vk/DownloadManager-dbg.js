...
sap.ui.define([
		...
		if (this._sourcesToProcess.length > 0 && this._sourcesBeingProcessed.length < this._maxParallelTasks) {
			...
			if (this._authorizationHandler) {
				...
				this._authorizationHandler(source).then(function(token) {
					...
					that._runTask(source, authToken);
	...
	DownloadManager.prototype._runTask = function(source, authToken) {
		var that = this;
		if (typeof source === "string") {
			...
			jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT5.summary), Messages.VIT5.code, "sap.ui.vk.DownloadManager");
		}
