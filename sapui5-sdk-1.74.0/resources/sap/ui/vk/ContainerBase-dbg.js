	...
	ContainerBase.prototype._addToolbarContent = function() {
		...
		this._customButtons.forEach(function(item){
			if (item.visible) {
				...
				if ("overflow" in item) {
					settings.overflow = item.overflow;
				}

				switch (item.type) { // supports "click" and "toggle" types only for now
					case MapContainerButtonType.Click:
						item.button = settings.overflow ? new sap.m.OverflowToolbarButton(settings) : new sap.m.Button(settings);
						break;
					default:
						item.button = settings.overflow ? new sap.m.OverflowToolbarToggleButton(settings) : new sap.m.ToggleButton(settings);
						break;
				}
