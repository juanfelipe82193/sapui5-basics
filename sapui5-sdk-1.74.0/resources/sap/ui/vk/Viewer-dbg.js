...
sap.ui.define([
	...
	function isDvlViewport(vp) {
		...
		return vp instanceof vkLibrary.dvl.Viewport;
	}

	Viewer.prototype.getShowAllHotspots = function() {
		var vp = this.getViewport().getImplementation();
		return isDvlViewport(vp) ? vp.getShowAllHotspots() : this.getProperty("showAllHotspots");
	};
	...
	Viewer.prototype.getHotspotColorABGR = function() {
		var vp = this.getViewport().getImplementation();
		return isDvlViewport(vp) ? vp.getHotspotColorABGR() : this.getProperty("hotspotColorABGR");
	};
	...
	Viewer.prototype.getHotspotColor = function() {
		var vp = this.getViewport().getImplementation();
		return isDvlViewport(vp) ? vp.getHotspotColor() : this.getProperty("hotspotColor");
	};
	...
	Viewer.prototype._instantiateStepNavigation = function() {
		...
		} else if (this._mainScene instanceof vkLibrary.threejs.Scene) {
	...
	Viewer.prototype._instantiateRedlineDesign = function(redlineElements) {
		...
		var vp = this.getViewport().getImplementation();
		if ((vp && vp.getVisible()) || (this.getNativeViewport() && this.getNativeViewport().getVisible())) {
			var activeViewport = vp && vp.getVisible() ? vp : this.getNativeViewport();
	...
	Viewer.prototype.activateRedlineDesign = function(redlineElements) {
		var vp = this.getViewport().getImplementation();
		if (vp instanceof vkLibrary.threejs.Viewport) {
			vp._viewportGestureHandler._activateRedline();
		}
		if (vp instanceof vkLibrary.dvl.Viewport && vp.getViewInfo().camera && vp.getViewInfo().camera.projectionType === "perspective") {
			vp._activateRedline();
		}
		redlineElements = redlineElements || [];
		...
		var onRedlineDesignPanViewport = function(event) {
			...
			vp.queueCommand(function(deltaX, deltaY) {
		...
		var onRedlineDesignZoomViewport = function(event) {
			...
			vp.queueCommand(function(originX, originY, zoomFactor) {
		...
		} else if (vp && isDvlViewport(vp) && vp.getVisible()) {
	...
	Viewer.prototype.destroyRedlineDesign = function() {
		if (this.getRedlineDesign()) {
			var vp = this.getViewport().getImplementation();
			if (vp instanceof vkLibrary.dvl.Viewport) {
	...
	Viewer.prototype._removeProgressIndicator = function(timeout) {
		...
		}.bind(this), timeout); // Timeout to allow hiding of progress indicator in Firefox, and for smoother UX (e.g. bar doesn't dissapear before being able to visually see it at 100%)
	};

	Viewer.prototype._handleContentReplaced = function(event) {
		var content = event.getParameter("newContent");

		if (content && content instanceof vkLibrary.Scene) {
			this._showViewport();
	...
	Viewer.prototype._handleContentChangesStarted = function(event) {
		...
		if (this._componentsState.progressIndicator.defaultEnable) {
			...
			this.getViewport().stopAnimation();
		}
	};
