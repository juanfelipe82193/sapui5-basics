	...
	Viewport.prototype.getViewInfo = function(query) {
		...
		if (effectiveQuery.visibility && this._viewStateManager) {
			...
			if (effectiveQuery.visibility.mode === VisibilityMode.Complete) {
				...
				jQuery.sap.log.warning(getResourceBundle().getText(Messages.VIT32.summary), Messages.VIT32.code, "sap.ui.vk.dvl.Viewport");
			}
		}
	...
	Viewport.prototype.setViewInfo = function(viewInfo, flyToDuration) {
			...
			if (stepVeId || stepIndex >= 0 && procedureIndex >= 0) {
				if (stepVeId) {
					...
					if (stepIndex >= 0 && stepIndex < procedures.procedures[procedureIndex].steps.length) {
						...
						jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT26.summary), Messages.VIT26.code, "sap.ui.vk.dvl.Viewport");
					}
				} else {
					// Unsupported value for procedure index
					jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT27.summary), Messages.VIT27.code, "sap.ui.vk.dvl.Viewport");
				}
				if (stepId) {
		...
		if (viewInfo.camera) {
			...
			if (viewInfo.camera.projectionType === CameraProjectionType.Perspective || viewInfo.camera.projectionType === CameraProjectionType.Orthographic) {
				...
				jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT19.summary), Messages.VIT19.code, "sap.ui.vk.dvl.Viewport");
		...
		if (viewInfo.visibility) {
			...
			switch (viewInfo.visibility.mode) {
				...
				default:
					jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT28.summary), Messages.VIT28.code, "sap.ui.vk.dvl.Viewport");
					break;
			}
		}
	...
	*/
	Viewport.prototype.setSelectionRect = function(rect) {
		if (!rect) {
			...
			this._dvl.Renderer.DrawSelectionRect(rect.x1, rect.y1, rect.x2, rect.y2, this._dvlRendererId);
		}
	};
	...
	Viewport.prototype.resetView = function(resetOptions) {
		...
		if (resetOptions !== undefined && !jQuery.isPlainObject(resetOptions)) {
			jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT31.summary), Messages.VIT31.code, "sap.ui.vk.dvl.Viewport");
		}
	...
	Viewport.prototype.tap = function(x, y, isDoubleClick) {
		if (this._dvlRendererId) {
			...
			if (!isDoubleClick) {
				...
				this._dvl.Renderer.Tap(px, py, false, false, this._dvlRendererId);
			} else if (!this.getFreezeCamera()){
				this._dvl.Renderer.Tap(px, py, true, false, this._dvlRendererId);
			}
		}
		return this;
	};
	...
	Viewport.prototype.rectSelect = function(x1, y1, x2, y2) {
		var nodes = [];
		if (this._dvlRendererId) {
			var result = getJSONObject(this._dvl.Renderer.RectSelect(x1, y1, x2, y2, this._dvlRendererId));
