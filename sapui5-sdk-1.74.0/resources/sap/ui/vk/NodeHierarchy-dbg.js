...
sap.ui.define([
		...
		var compareValuesByPredicate = function(predicate, isCaseSensitive, veFieldValueString, queryFieldValue) {
			...
			switch (predicate) {
				...
				default:
					...
					jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT8.summary), Messages.VIT8.code, "sap.ui.vk.dvl.NodeHierarchy");
			}
			return matchFound;
		};
	...
	NodeHierarchy.prototype.findNodesByMetadata = function(query) {
		...
		if (query === undefined || query === null || jQuery.isEmptyObject(query)) {
			...
			if (query.key === undefined || query.key === null) {
				...
				switch (predicateName) {
					...
					case "startsWith":
						...
						break;
					default:
						jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT9.summary), Messages.VIT9.code, "sap.ui.vk.dvl.NodeHierarchy");
				}
			...
			jQuery.sap.log.error(getResourceBundle().getText(Messages.VIT10.summary), Messages.VIT10.code, "sap.ui.vk.dvl.NodeHierarchy");
		}
	...
	NodeHierarchy.prototype._appendAncestors = function(nodeRefs, skipNodeRefs) {
		...
		var newNodeRefs = [];
		nodeSet.forEach(function(nodeRef) {
			newNodeRefs.push(nodeRef);
		});
		return newNodeRefs;
	};
