...
sap.ui.define([
	...
	AnchorPointToolGizmo.prototype.init = function() {
		...
		function createGizmoArc(axis, color, radius, segments) {
			var geometry = new THREE.TorusBufferGeometry(radius, 1 / 96, 4, segments, Math.PI / 2);
			if (axis === 0) {
		...
		function createTouchArc(axis, radius, segments) {
			var geometry = new THREE.TorusBufferGeometry(radius, 16 / 96, 4, segments, Math.PI / 2);
			if (axis === 0) {
				geometry.rotateY(Math.PI / -2);
			} else if (axis === 1) {
				geometry.rotateX(Math.PI / 2);
	...
	AnchorPointToolGizmo.prototype._setOffset = function(offset, gizmoIndex) {
		if (this._tool.fireEvent("moving", { x: offset.x, y: offset.y, z: offset.z }, true)) {
			...
			var matInv = new THREE.Matrix4().getInverse(this._gizmo.matrixWorld);
			...
			// update line mesh
			this._line.geometry.vertices[ 0 ].copy(offset);
	...
	AnchorPointToolGizmo.prototype.getValue = function() {
		...
		if (this._handleIndex >= 6 && this._handleIndex < 9) { // rotation
			return THREE.Math.radToDeg(this._gizmo.rotation.reorder("YXZ")[ [ "x", "y", "z" ][this._handleIndex - 6] ]);
		}
	...
	AnchorPointToolGizmo.prototype.setValue = function(value) {
		if (this._handleIndex >= 0 && this._handleIndex < 3) { // position
			this.beginGesture();
			var offset = new THREE.Vector3().setFromMatrixColumn(this._gizmo.matrixWorld, this._handleIndex).normalize().multiplyScalar(value - this._lastEditValue);
			this._move(offset);
			this.endGesture();
		} else if (this._handleIndex >= 6 && this._handleIndex < 9) { // rotation
			this.beginGesture();
			var euler = this._gizmo.rotation.clone();
			euler[ [ "x", "y", "z" ][ this._handleIndex - 6 ] ] = THREE.Math.degToRad(value);
			var quat2 = new THREE.Quaternion().setFromEuler(euler);
			var quat = this._gizmo.quaternion.clone().inverse().multiply(quat2);
