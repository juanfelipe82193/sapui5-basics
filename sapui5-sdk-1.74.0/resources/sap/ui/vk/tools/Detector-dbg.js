...
sap.ui.define([
	...
	Detector.prototype.setDetectDistance = function(value) {
		this._detectDistance = parseFloat(value) || 20.0;
	};
		...
		sceneRef.traverse(function(child) {
			if (child.isMesh) {
				if (!this._objArr.find(function(o) { return o.uuid === child.uuid;})) {
					...
					var vArr = this.getVertices(child.geometry);
	...
	Detector.prototype.setSource = function(viewStateManager) {
		var selectedNodes = [];
		...
		if (selectedNodes.length > 0) {
			...
			if (obj.children.length === 0) {
				...
				this._isGroup = true;
				this._sourceObjects = obj.children.filter(function(c) {return c.type === "Mesh";})[0];
			}
	...
	Detector.prototype.setTarget = function() {
		...
		if (this._sourceObjects) {
			...
			this._targetObjects.forEach(function(to) {
				...
				toBox.setFromObject(toBoxHelper);
	...
	Detector.prototype.getVertices = function(geometry) {
		var i, l, vertices = [];
	...
	Detector.prototype.snapMove = function() {
		...
		if (this._finalSnaps.length === 2 && this._sourceObjects && Math.abs(this._finalDistance - this._intersectDistance) > 0.1) {
			if (!this._collidedUuid && this._finalDistance >= this._intersectDistance) {
					...
					option.gizmo._gizmoOffset.add(this._direction.clone().multiplyScalar(0.1));
					option.gizmo._move(option.gizmo._gizmoOffset);
			} else {
				this._intersectDistance = Number.NEGATIVE_INFINITY;
				this._collidedUuid = null;
				this._isMoved = false;
	...
	Detector.prototype.snapScale = function() {
		var option = this._op;
		// scale source obj
		if (!option.gizmo._gizmoScale.equals(new THREE.Vector3(1, 1, 1))
			&& this._finalSnaps.length === 2
			&& this._sourceObjects
			&& Math.abs(this._finalDistance - this._intersectDistance) > 0.1) {
			if (!this._collidedUuid && this._finalDistance >= this._intersectDistance) {
				option.gizmo._tool._handler._gesture = false;
				var gsArray = option.gizmo._gizmoScale.toArray();
				gsArray.forEach(function(s, i) {
					if (s > 1) {
						option.gizmo._gizmoScale.setComponent(i, s * 1.01);
					}
				});
				option.gizmo._scale(option.gizmo._gizmoScale);
			} else {
				this._intersectDistance = Number.NEGATIVE_INFINITY;
				this._collidedUuid = null;
				this._isMoved = false;
	...
	Detector.prototype.detectMove = function() {
		// for move
		this._direction.copy(this._op.gizmo._gizmoOffset).normalize();
	...
	Detector.prototype.detectScale = function() {
		// for scale
		if (this._op.gizmo._gizmoScale.toArray().find(function(s) { return s <= 1; })) {
		...
		var bSphere = 0.0, sphereRadius = 0.0;
		sphereRadius = this._sourceObjects.geometry.boundingSphere.clone().applyMatrix4(this._sourceObjects.matrixWorld).radius;
		bSphere =  new THREE.Sphere(srcWorldPos, sphereRadius + this._detectDistance);
		var interObjArr = [];
	...
	Detector.prototype.detectRotate = function() {
		...
		if (this._op.gizmo._gizmoRotation.toArray()[axisIndex] > 0) {
		...
		function GJK(shape1, shape2) {
			...
			for (var i = 0; i < MAX_ITERATIONS; ++i) {
				...
				if (n === 2) {
					...
					// need a tetrahedron to eclose the origin
					n = 3;
					continue;
				}
