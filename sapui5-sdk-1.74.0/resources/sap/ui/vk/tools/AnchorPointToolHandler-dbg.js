	...
	AnchorPointToolHandler.prototype.move = function(event) {
		if (this._gesture) {
			...
			if (this._handleIndex < 3) {// axis
				...
				var angle2 = this._getMouseAngle();

				if (Math.abs(this._prevDeltaAngle) > Math.PI / 4) {
					if (angle2 - angle1 > this._prevDeltaAngle + Math.PI) {
						angle2 -= Math.PI * 2;
					} else if (angle2 - angle1 < this._prevDeltaAngle - Math.PI) {
						angle2 += Math.PI * 2;
					}
				}
				this._prevDeltaAngle = (angle2 - angle1) % (Math.PI * 2);
