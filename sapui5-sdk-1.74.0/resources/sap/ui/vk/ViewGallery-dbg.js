...
sap.ui.define([
	...
	var ViewGallery = Control.extend("sap.ui.vk.ViewGallery", /** @lends sap.ui.vk.ViewGallery.prototype */ {
		metadata: {
			...
			properties: {
				...
				/**
				 * Sets the transparency level of the viewgallery. Range is 0-1, with 1 being 100% opaque and 0 being 0% opaque
				 */
				transparency: {
					type: "float",
					defaultValue: 1
	...
	ViewGallery.prototype._getViewManager = function() {
		var viewManagerId = this.getViewManager();
		if (viewManagerId) {
			return sap.ui.getCore().byId(viewManagerId);
		}
	...
	ViewGallery.prototype.onAfterRendering = function() {
		...
		var host = null;
		if (this.getHost()) {
			host = sap.ui.getCore().byId(this.getHost());
			if (host.getContent().includes(this)){
				// If viewgallery is inside host we need to adjust step description box sizes
	...
	ViewGallery.prototype.adjustStepDescriptionBoxes = function() {
		if (this._stepDescription.getDomRef()){
			...
			var host = sap.ui.getCore().byId(this.getHost());
			var hostHeight = host.getDomRef().getBoundingClientRect().height;
	...
	ViewGallery.prototype._playProcedure = function() {
		...
		if (viewManager) {
			...
			if (selectedView) {
				viewManager.playViewGroup(selectedView, this._getSelectedViewGroup());
			}
		}
	};
	...
	ViewGallery.prototype._pauseAnimation = function() {
		var viewManager = this._getViewManager();
		if (viewManager) {
			viewManager.stopPlayingViewGroup();
		}
	};
	...
	ViewGallery.prototype.setAnimationPlayer = function(animationPlayer) {
		if (this._animationPlayer) {
			this._animationPlayer.detachTimeChanged(this._onAnimationPlayerTimeChanged, this);
			this._animationPlayer.detachStateChanged(this._onAnimationPlayerStateChanged, this);
		}

		this._animationPlayer = animationPlayer;

		if (this._animationPlayer) {
			this._animationPlayer.attachTimeChanged(this._onAnimationPlayerTimeChanged, this);
			this._animationPlayer.attachStateChanged(this._onAnimationPlayerStateChanged, this);
		}
		this._setupAnimationSlider();
	};
	...
	ViewGallery.prototype.setScene = function(scene) {
		...
		if (scene && scene.loaders) {
			for (var i = 0; i < scene.loaders.length; i++) {
				...
				if (loader && loader.getMetadata && loader.getMetadata().getName() === "sap.ui.vk.threejs.ContentDeliveryService") {
	...
	ViewGallery.prototype._onViewActivated = function(channel, eventId, event) {
		...
		if (viewGroups && viewGroups.length > 0) {
			...
			if (selectedGroup && selectedGroup.getViewGroupId() != view.viewGroupId) {
				...
				for (var j = 0; j < selectedGroup.getViews().length; j++){
	...
	ViewGallery.prototype.setHost = function(host) {
		this._detachAnimationTimeSlider(this.getAnimationTimeSlider());

		this.setAssociation("host", host, true);
	...
	ViewGallery.prototype.setSelectedItem = function(item) {
		if (item.getCustomData().length > 0) {
			...
			var viewManager = this._getViewManager();
			if (viewManager) {
				viewManager.activateView(modelView);
			}
		}
		return this;
	};
	...
	ViewGallery.prototype._setContent = function(content) {
		var isThreeJSScene = content && content.getMetadata && content.getMetadata().getName() === "sap.ui.vk.threejs.Scene";
