...
sap.ui.define([
	 ...
	 * or entities are passed the unspecified side effect is executed. This unspecified side effect uses either the
	 * annotated targets of the unspecified side effect or - in case of no available annotation - triggers a
	 * complete model refresh on the OData model.
	 *
	 * @param {sap.ui.model.Context} oContext The given binding context
	...
	ApplicationController.prototype.executeSideEffects = function (oContext, aSourceProperties, aSourceEntities, bForceGlobalRefresh) {
		...
		if (mRequests.bGlobal || (bGlobal && !bExecuteSideEffect)) {
			// execute side effect only if any unspecified side effect annotation is available
			if (bGlobal && oSideEffect) {
				fnExecuteSideEffect({});
				bSubmitNeeded = true;
			}
			// if bForceGlobalRefresh is true,then refresh. If false then do not update model.
	...
	ApplicationController.prototype._invokeAction = function (sFunctionName, oContext, sChangeSetID, mParams) {
		...
		var mParameters = {
			...
			successMsg: "Call of action succeeded",
			failedMsg: "Call of action failed",
			urlParameters: mParams.urlParameters,
			forceSubmit: true,
			context: oContext
		};
