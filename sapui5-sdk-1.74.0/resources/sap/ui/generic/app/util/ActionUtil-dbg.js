...
sap.ui.define([
	 ...
	 * @protected
	 */
	ActionUtil.prototype.call = function (sFunctionImportPath, sFunctionImportLabel, bIsDraftEnabled) {
		var that = this;
				...
				MessageBox.confirm(sMsgBoxText, {
					...
					onClose: function (oAction) {
						if (oAction === "OK") {
							...
							mActionParams.isDraftEnabled = bIsDraftEnabled;
	...
	ActionUtil.prototype._prepareParameters = function (aContexts) {
		...
		if (Array.isArray(aContexts) && aContexts.length != 1) {
			return undefined;
		} else {
	...
	ActionUtil.prototype._initiateCall = function (mActionParams) {
		if (mActionParams != undefined && mActionParams.additionalParameters && mActionParams.additionalParameters.length == 0) {
			this._call(mActionParams.parameterData, mActionParams.isDraftEnabled);
							...
							if (mParameterForm.hasNoClientErrors()) {
								if (mParameterForm.getEmptyMandatoryFields().length == 0) {
									...
									for (var i = 0; i < mParameterForm.getEmptyMandatoryFields().length; i++) {
										var sText = formatMessage(sRootMessage, mParameterForm.getEmptyMandatoryFields()[i].getTextLabel());
			...
			this._call(null, mActionParams.isDraftEnabled);
		}
	};

	ActionUtil.prototype._call = function (mUrlParameters, bIsDraftEnabled) {
	...
	ActionUtil.prototype._buildParametersForm = function (mParameters, oContext) {
		...
		for (var i = 0; i < mParameters.additionalParameters.length; i++) {
			...
			if (oParameter["com.sap.vocabularies.UI.v1.Hidden"] && !(oParameter["com.sap.vocabularies.UI.v1.Hidden"].Bool == "false")) {
				continue;
			}
			...
			oForm.addContent(oSmartLabel);
			oForm.addContent(oSmartField);
		}
