...
sap.ui.define([
	...
	"sap/ui/comp/odata/MetadataAnalyser"
			 ...
			 HBox, NumberFormat, DateFormat, CheckBox, Label, Input, Log, MetadataAnalyser) {
	"use strict";
				...
				oField.getModel().read("/" + oValueListAnnotation.valueListEntitySetName, {
					success: function (oResult) {
						if (oResult.results && oResult.results.length === 0 && oSmartField._oControl.edit) {
							oSmartField._oControl.edit.setValueState(ValueState.Error);
							oField._bClientError = true;
	...
	Field.prototype._performValidation = function () {
		...
		if (this.isComposite() && aInnerControl.length === 2 && aInnerControl[0].getBinding("value")) {
				...
				oValueBinding.getValue = function() {
					...
					var sSecondValue = this._getSecondInnerEdit().getValue();
	...
	Field.prototype._handleCompositeInputChange = function (oEvent) {
		var sFirstValue = this._getFirstInnerEdit().getValue(),
			sSecondValue = this._getSecondInnerEdit().getValue(),
			sFormattedValue;
