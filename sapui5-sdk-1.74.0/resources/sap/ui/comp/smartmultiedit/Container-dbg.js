		...
		if (!this._bIsInitialized && this.getEntitySet() && oModel
			...
			if (!this.getBindingContext()) {
				var oContext = oModel.createEntry("/" + this.getEntitySet()),
					oObject = oModel.mChangedEntities ? oModel.mChangedEntities[oContext.getPath().substr(1)] : null,
					oCreatedBackup = null;
				if (oObject && oObject.__metadata && oObject.__metadata.created) {
					oCreatedBackup = oObject.__metadata.created;
					oObject.__metadata.created = undefined;
				}
				this.setBindingContext(oContext);
				if (oCreatedBackup) {
					oObject.__metadata.created = oCreatedBackup;
				}
