...
sap.ui.define([
	...
	ODataControlFactory.prototype._createEdmString = function() {
		...
		oControlSelector = this._oSelector.checkComboBox();
		...
		if (bTextInEditModeSourceNotNone) {
		...
		var sTextInEditModeSource = this._oParent.getTextInEditModeSource(),
			mAttributes;

		this.oTextArrangementDelegate.bValidMetadata = bEdmPropertyTypeSupported && this.oTextArrangementDelegate.checkRequiredMetadata(sTextInEditModeSource);

		if (this.oTextArrangementDelegate.bValidMetadata) {
			mAttributes = this.createAttributes("", this._oMetaData.property, mNames);
			mAttributes[sValuePropertyMap] = this.oTextArrangementDelegate.getBindingInfo();
		} else {
		...
		oControl.attachEventOnce("modelContextChange", this._handleModelContextChangeEvent);
		...
		if (oCustomDataConfig && (typeof oCustomDataConfig.onInput === "function")) {
			oCustomDataConfig.onInput(oControl);
		}
	...
	ODataControlFactory.prototype._handleEventingForEdmString = function(oSettings) {
		...
		var bUpperCase = this._oHelper.oAnnotation.isUpperCase(oSettings.edmProperty);
		...
		oControl.attachChange(function onCurrencyInputFieldChange(oControlEvent) {
			...
			if (mParameters) {
				...
				if (oSettings.currency) {
					...
					this.updateModelPropertiesForCurrency("value", oUpdateModelPropertiesSettings);
				}
	...
	ODataControlFactory.prototype.getUOMChangeHandler = function(oSettings) {
		return function(oControlEvent) {
			try {
				...
				if (oSettings.currency) {
					var oUpdateModelPropertiesSettings = this.getCurrencyValidationSettings(oSettings);
					this.updateModelPropertiesForCurrency("value", oUpdateModelPropertiesSettings);
				}
	...
	ODataControlFactory.prototype.updateModelPropertiesForCurrency = function(sPropertyName, oSettings) {
		...
		function handleCurrencyFieldsValidationFinally() {
			...
			if (oErrorState.bComplex && oErrorState.bFirst && oErrorState.bSecond) {
				...
				var oValuesValidityPromise = oSettings.checkValidity(oCheckValiditySettings);
				oValuesValidityPromise.then(handleCurrencyFieldsValidationSuccess.bind(this, true))
										.catch(handleCurrencyFieldsValidationError);
			}
		}

		oValuesValidityPromise.then(handleCurrencyFieldsValidationSuccess)
								.catch(handleCurrencyFieldsValidationError)
	...
	ODataControlFactory.prototype._createComboBox = function(mSettings) {
		...
		var sValuePropertyMap = ODataControlFactory.getBoundPropertiesMapInfoForControl(ComboBox.getMetadata().getName(), {
			propertyName: "value"
		})[0];
		...
		var mAttributes = {
			...
			selectionChange: this._oHelper.getSelectionChangeHandler(this._oParent)
		};
	...
	ODataControlFactory.prototype._createSelect = function(mSettings) {
		...
		var mAttributes = {
			change: this._oHelper.getSelectionChangeHandler(this._oParent),
	...
	ODataControlFactory.prototype._createEdmDateTimeOffset = function() {
		var mDateFormatSettings = this.getFormatSettings("dateFormatSettings");
		...
		if (mDateFormatSettings) {
			mDateFormatSettings.UTC = false;
		}
		...
		mAttributes[sValuePropertyMap] = {
			model: this._oMetaData.model,
			path: this._oMetaData.path,
			type: this._oTypes.getType(this._oMetaData.property, mDateFormatSettings),
			mode: this._oParent.getBindingMode("value")
		};
	...
	ODataControlFactory.prototype._createEdmUOM = function() {
		var oEdmMeasureProperty = this.getEdmProperty(),
		...
		var oMeasureField = new Input(sSmartFieldID + "-input", mUOMAttributes);
		...
		var sValuePropertyMap = ODataControlFactory.getBoundPropertiesMapInfoForControl(SmartField.getMetadata().getName(), {
			propertyName: "value"
		})[0];
		...
		oCurrencySmartField.data("configdata", {
			"configdata": {
				...
				onText: function(oInnerControl) {
					oMeasureField.setLayoutData(new FlexItemData({
						growFactor: 1
					}));
					oCurrencySmartField.setLayoutData(that._getStaticUOMTextFlexItemData({
						shrinkFactor: 0,
						styleClass: "sapUiCompSmartFieldFlexItemUnitDisplay"
					}));

					oCurrencySmartField.addStyleClass("sapUiCompSmartFieldUOMDisplayText");
				...
				onInput: function(oInnerControl) {
					oMeasureField.setLayoutData(that._getDynamicAmountInputFlexItemData({ control: oCurrencySmartField }));
					oCurrencySmartField.setLayoutData(that._getStaticUOMInputFlexItemData({
						growFactor: 0,
	...
	ODataControlFactory.prototype.triggerCreationOfControls = function(oMetaData) {
		try {
			...
			this._oParent._toggleInnerControlIfRequired();
	...
	ODataControlFactory.prototype.bind = function() {
		...
		if (oConfig && oConfig.configdata) {
			this.triggerCreationOfControls(this._oMeta);
			return Promise.resolve();
		}
		...
		if (!bTextInEditModeSourceNotNone && this._oModel.bMetaModelLoaded) { // FIXME: bMetaModelLoaded is a private instance member of the ODataModel class
			this.triggerCreationOfControls(this._oMeta);
			return Promise.resolve();
		}
		...
		this.bPending = true;
		var oMetaModelLoadedPromise = this._oModel.getMetaModel().loaded();
		var oValueListAnnotationLoadedPromise = oMetaModelLoadedPromise.then(function onMetaModelLoaded() {
			...
			if (bTextInEditModeSourceNotNone) {
				...
				if (typeof vValueListAnnotationPath === "string") {
					...
					return this._oHelper.loadValueListAnnotation(vValueListAnnotationPath);
				} else {
					this.bPending = false;
		...
		oMetaModelLoadedPromise.catch(function(oError) {
			Log.error("The meta model could not be loaded.", oError, this.getMetadata().getName());
		}.bind(this));

		if (bTextInEditModeSourceNotNone) {
			oValueListAnnotationLoadedPromise.then(function onValueListAnnotationLoaded(oValueListAnnotations) {
				...
				if (oValueListAnnotations && this.oTextArrangementDelegate) {
					this.oTextArrangementDelegate.fetchIDAndDescriptionCollectionIfRequired();

					this._handleModelContextChangeEvent = function () {
						this.oTextArrangementDelegate.fetchIDAndDescriptionCollectionIfRequired();
					}.bind(this);
				}
			}.bind(this));
		...
		oValueListAnnotationLoadedPromise.catch(function(oError) {
		...
		return oValueListAnnotationLoadedPromise;
	};

	ODataControlFactory.prototype._handleModelContextChangeEvent = function() {};
