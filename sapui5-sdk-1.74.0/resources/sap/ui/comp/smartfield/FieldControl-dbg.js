	...
	FieldControl.prototype._getEditable = function(oMetaData, oBindingInfo, sProperty) {
		...
		return {
			...
			formatter: function formatEditable(vValue, p1, p2, p3) {
				...
				if (iEntitySetPos > -1 && (!oEntity || !oEntityMetadata || !bEntityMetadataCreated)) {
		...
		return {
			...
			formatter: function formatMandatory(p1, p2) {
				...
				if (oEdmProperty) {
					...
					if ((oEdmProperty.nullable === "false") || (that._oAnnotation && that._oAnnotation.isStaticMandatory(oEdmProperty))) {
						aMandatoryStates.push(true);
					} else if (oEdmProperty.nullable) {
						aMandatoryStates.push(false);
					}
				}

				var bFieldControl = iPropertyPos > -1;
				...
				if (bFieldControl) {
					aMandatoryStates.push(arguments[iPropertyPos] === 7);
				}

				var bPropertyBound = iBindingPos > -1;
				...
				if ((aMandatoryStates.length === 0) ||
					((oEdmProperty.nullable === "true") && !bFieldControl && !bPropertyBound)) {
