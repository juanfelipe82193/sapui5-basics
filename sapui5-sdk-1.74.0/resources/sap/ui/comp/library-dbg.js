...
sap.ui.define(['sap/ui/core/Core', 'sap/ui/core/library', 'sap/m/library'], function (Core, library1, library2) {
	...
	sap.ui.comp.smartform.inheritCostomDataToFields = function (oCustomData) {
		...
		var oBlackList = {
			startsWith: {
				...
				compareFunction: function(sBlacklistString) {
					return sKey.startsWith(sBlacklistString);
				}
			},
			wholeString: {
				strings: [
				...
				compareFunction: function(sBlacklistString) {
					return sKey == sBlacklistString;
				}
			}
		};

		var bReturn = true;
		Object.keys(oBlackList).forEach(function(sType) {
			if (bReturn && oBlackList[sType].strings.some(oBlackList[sType].compareFunction)) {
				bReturn = false;
				return true;
			}
		});
	 ...
	 * @ui5-metamodel This enumeration also will be described in the UI5 (legacy) designtime metamodel
	 */
	sap.ui.comp.personalization.ColumnType = {
