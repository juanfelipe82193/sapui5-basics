...
if (sap.ui && sap.ui.performance && sap.ui.performance.Measurement && sap.ui.performance.Measurement.getActive()) {
	sap.ui.performance.Measurement.start("SmartChartInit", "Measurement of SmartChart init time");
}

sap.ui.define([
	'jquery.sap.global', 'sap/ui/comp/library', 'sap/ui/comp/util/DateTimeUtil', 'sap/m/Button', 'sap/m/Text', 'sap/m/FlexItemData', 'sap/m/OverflowToolbar', 'sap/m/OverflowToolbarButton', 'sap/m/OverflowToolbarToggleButton', 'sap/m/ToolbarSeparator', 'sap/m/ToolbarSpacer', 'sap/m/FlexBox', 'sap/m/VBox', 'sap/ui/comp/providers/ChartProvider', 'sap/ui/comp/smartfilterbar/FilterProvider', 'sap/ui/comp/smartvariants/SmartVariantManagement', 'sap/ui/comp/smartvariants/PersonalizableInfo', 'sap/ui/model/json/JSONModel', 'sap/ui/model/Sorter', 'sap/ui/model/Filter', 'sap/ui/model/FilterOperator', 'sap/ui/comp/personalization/Util', 'sap/ui/Device', 'sap/ui/comp/odata/ODataModelUtil', 'sap/ui/comp/odata/MetadataAnalyser', 'sap/ui/comp/state/UIState', 'sap/m/SelectionDetails', 'sap/m/SelectionDetailsItem', 'sap/m/SelectionDetailsItemLine', 'sap/m/Title', 'sap/ui/comp/util/FormatUtil', 'sap/ui/comp/navpopover/NavigationPopoverHandler', 'sap/ui/core/InvisibleText', 'sap/m/OverflowToolbarLayoutData', 'sap/m/library', 'sap/ui/comp/personalization/Controller', 'sap/ui/comp/personalization/ChartWrapper', 'sap/ui/comp/navpopover/SemanticObjectController', 'sap/ui/performance/Measurement', 'sap/base/Log', 'sap/base/util/merge'
], function(jQuery, library, DateTimeUtil, Button, Text, FlexItemData, OverflowToolbar, OverflowToolbarButton, OverflowToolbarToggleButton, ToolbarSeparator, ToolbarSpacer, FlexBox, VBox, ChartProvider, FilterProvider, SmartVariantManagement, PersonalizableInfo, JSONModel, Sorter, Filter, FilterOperator, PersoUtil, Device, ODataModelUtil, MetadataAnalyser, UIState, SelectionDetails, SelectionDetailsItem, SelectionDetailsItemLine, Title, FormatUtil, NavigationPopoverHandler, InvisibleText, OverflowToolbarLayoutData, MLibrary, PersoController, ChartWrapper, SemanticObjectController, Measurement, Log, merge) {
	"use strict";
	 ...
	 * @class The SmartChart control creates a chart based on the OData metadata and the provided configuration. To use the control, the <code>entitySet</code>
		...
		metadata: {
			...
			properties: {
				...
				selectionMode: {
					...
					group: "Misc",
					defaultValue: sap.ui.comp.smartchart.SelectionMode.Multi
				},
			...
			events: {
				 ...
				 * event has been attached. Please use event <code>intialized</code> instead.
				 *
	...
	function _innerChartArtefactsLoaded(fnChartLibrary, fnChart, fnDimension, fnHierarchyDimension, fnTimeDimension, fnMeasure, fnVizTooltip) {
			...
			if ((!ChartLibrary || !Chart || !Dimension || !HierarchyDimension || !TimeDimension || !Measure || !VizTooltip) && !this._bChartRequested) {
				...
				if (!ChartLibrary || !Chart || !Dimension || !HierarchyDimension || !TimeDimension || !Measure || !VizTooltip) {
					sap.ui.getCore().loadLibrary("sap.chart", {
						...
						sap.ui.require([
							'sap/chart/library', 'sap/chart/Chart', 'sap/chart/data/Dimension', 'sap/chart/data/HierarchyDimension', 'sap/chart/data/TimeDimension', 'sap/chart/data/Measure', 'sap/viz/ui5/controls/VizTooltip'
			...
			if (oField.isDimension || oField.isHierarchyDimension || (aDimensionFields.indexOf(oField.name) != -1)) {
				...
				if (this._oChart.getDimensionByName(oField.name) === undefined) {
					if (oField.isDimension) {
						if (oField.isTimeDimension) {
							// Create TimeDimension instances
							oChartObject = new TimeDimension(mProperties);
							switch (oField.timeUnitType) {
								case "Date":
									oChartObject.setTimeUnit(sap.chart.TimeUnitType.Date);
									break;
								case "yearmonthday":
									oChartObject.setTimeUnit(sap.chart.TimeUnitType.yearmonthday);
									break;
								default:
									break;
							}
					...
					if (oField.description) {
						...
						oChartObject.setTextFormatter(FormatUtil.getFormatterFunctionFromDisplayBehaviour(oField.displayBehaviour));
	 ...
	 * @public
	 * @deprecated As of version 1.57.0. Please use <code>getChartAsync</code> instead.
	 */
	 ...
	 * inner {@link sap.chart.Chart chart} instance. To avoid this, please use the API provided by the <code>SmartChart</code> control itself.
	 *
	 * @returns {object} Promise for inner chart object
	 * @public
	 * @since 1.57.0
	 */
	SmartChart.prototype.getChartAsync = function() {
		...
		if (!this._oChart && this._bMetadataIsInitialised) {
			...
			return new Promise(function(resolve, reject) {
				...
				sap.ui.require([
					"sap/chart/library", "sap/chart/Chart", "sap/chart/data/Dimension", "sap/chart/data/HierarchyDimension", "sap/chart/data/TimeDimension", "sap/chart/data/Measure", "sap/viz/ui5/controls/VizTooltip"
				], function(fnChartLibrary, fnChart, fnDimension, fnHierarchyDimension, fnTimeDimension, fnMeasure, fnVizTooltip) {
	 ...
	 * @protected
	 *
	 * @returns {Object} The currently used variant in an internal format
	 */
		...
		if (this._oCurrentVariant.filter) {
			this._oCurrentVariant.filter.filterItems.forEach(function(oModelItem) {
				...
				if (oModelItem.exclude) {
					aExcludeFilters.push(new Filter(sPath, "NE", oModelItem.operation === "Empty" ? "" : oValue1));
	...
	SmartChart.prototype._toggleFullScreen = function(bValue, bForced) {
		...
		this._oFullScreenUtil.toggleFullScreen(this, this.bFullScreen, this.oFullScreenButton, this._toggleFullScreen, "sapContrastPlus");
