...
sap.ui.define([
	...
	ControlProvider.prototype._createFieldTemplate = function(oViewField, isEditable) {
		...
		if (this.useSmartField) {
			...
			if (ODataType.isNumeric(oViewField.type) || ODataType.isDateOrTime(oViewField.type) || CalendarMetadata.isCalendarOrFiscalValue(oViewField)) {
	...
	ControlProvider.prototype._completeSmartField = function(oViewField) {
		...
		if (oData.annotations.uom || ODataType.isNumeric(oViewField.type) || ODataType.isDateOrTime(oViewField.type) || CalendarMetadata.isCalendarOrFiscalValue(oViewField)) {
		...
		ControlProvider._createModelTypeInstance(oViewField, this._oDateFormatSettings, this._useUTCDateTime);
	};
	...
	ControlProvider.prototype._createEditableTemplate = function(oViewField, bBlockSmartLinkCreation) {
		var oTemplate = null, oType = ControlProvider._createModelTypeInstance(oViewField, this._oDateFormatSettings, this._useUTCDateTime);
		...
		if (!oTemplate) {
			if (oViewField.type === "Edm.Time") {
				...
				} else if (ODataType.isNumeric(oViewField.type) || ODataType.isDateOrTime(oViewField.type) || CalendarMetadata.isCalendarOrFiscalValue(oViewField)) {
	...
	ControlProvider.prototype._createDisplayOnlyTemplate = function(oViewField, bBlockSmartLinkCreation) {
		...
		oType = ControlProvider._createModelTypeInstance(oViewField, this._oDateFormatSettings, this._useUTCDateTime);
	 ...
	 * @private
	 */
	ControlProvider._createModelTypeInstance = function(oViewField, oDateFormatSettings, bUseUTCDateTime) {
		var oFormatOptions, oConstraints;
		...
		if (oViewField.type == "Edm.DateTime") {
			oViewField.modelType = ODataType.getType(oViewField.type, oFormatOptions, oConstraints, oViewField.isCalendarDate);
			oViewField.modelType.formatValue(new Date(), "string");
			oViewField.modelType.oFormat.oFormatOptions.UTC = false;
			return ODataType.getType(oViewField.type, oFormatOptions, oConstraints, oViewField.isCalendarDate);
		}
	...
	ControlProvider.prototype._getDefaultBindingInfo = function(oViewField, oType) {
		...
		if (this._bEnableDescriptions && oViewField.description) {
			oBindingInfo = {
				...
				formatter: FormatUtil.getFormatterFunctionFromDisplayBehaviour(oViewField.displayBehaviour)
			};
		}
		return oBindingInfo;
	};
	...
	ControlProvider.prototype._createObjectIdentifierTemplate = function(oViewField, oType, bFirstKeyField) {
		...
		var that = this;
		var fnIsTitleActive = function(oSemanticObjects) {
			...
			var aSemanticObjects = oViewField.semanticObjects.additionalSemanticObjects.concat(oViewField.semanticObjects.defaultSemanticObject);
		...
		if (oViewField.semanticObjects) {
			SemanticObjectController.getDistinctSemanticObjects().then(function(oSemanticObjects) {
				if (fnIsTitleActive(oSemanticObjects)) {
		...
		oObjectIdentifier = new ObjectIdentifier({
			...
			titleActive: oViewField.semanticObjects ? {
				...
				formatter: function(oSemanticObjects) {
					return fnIsTitleActive(oSemanticObjects);
				}
			} : false,
	...
	ControlProvider.prototype._createMeasureFieldTemplate = function(oViewField, oType) {
		...
		}).addStyleClass("sapUiCompUoMPart");
		...
		if (oViewField.isCurrencyField && this._isAnalyticalTable) {
			oAnalyticalMultiUnitLink = new Link({
				...
				visible: {
					path: oViewField.unit,
					formatter: MultiCurrencyUtil.isMultiCurrency
				},
				press: function(oEvt) {
					MultiCurrencyUtil.openMultiCurrencyPopover(oEvt, {
			...
			oAnalyticalMultiUnitCurrency.bindProperty("visible", {
				...
				formatter: function(sCurrency) {
					return !MultiCurrencyUtil.isMultiCurrency(sCurrency);
				}
			});
			oTemplate = new VBox({
				renderType: "Bare",
