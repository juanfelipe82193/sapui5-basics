...
sap.ui.define([
	'sap/ui/base/ManagedObject', 'sap/ui/comp/library', './Util', './ColumnHelper', 'sap/base/Log'
], function(ManagedObject, CompLibrary, Util, ColumnHelper, Log) {
	"use strict";
	...
	BaseController.prototype.getTable2JsonTransient = function(oJsonColumnKeys) {
		...
		oJsonColumnKeys[this.getType()][this.getItemType()].forEach(function(oColumnKey) {
			...
			if (sColumnBaseType === CompLibrary.personalization.ColumnType.TableColumn) {
				if (!oColumn.getLabel()) {
			...
			if (sColumnBaseType === CompLibrary.personalization.ColumnType.ResponsiveColumn) {
			...
			if (sColumnBaseType === CompLibrary.personalization.ColumnType.ColumnWrapper) {
				if (!oColumn.getLabel()) {
					throw "The column '" + oColumnKey.columnKey + "' should have a 'label' aggregation otherwise the column can not be identified in the personalization dialog.";
				}
