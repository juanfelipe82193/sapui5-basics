...
sap.ui.define([
	...
	'sap/m/library',
	'sap/base/Log'
], function(Link, ResponsivePopover, Button, Title, Image, Text, VBox, HBox, ResponsivePopoverRenderer, Factory,
		LinkData, JSONModel, ManagedObjectObserver, FlexItemData, BindingMode, Device, mLibrary, Log) {
	"use strict";
	...
	NavigationPopover.prototype.insertAvailableAction = function(oLinkData, iIndex) {
		...
		var oModel = this._getInternalModel();
		oModel.setProperty("/availableActions/" + iIndex + "/", oLinkData.getJson());
		return this;
	};
	...
	NavigationPopover.prototype._createContent = function() {
		...
		oContent.addItem(oSeparator).addItem(this._oActionArea).addItem(this._oPersonalizationButton);
