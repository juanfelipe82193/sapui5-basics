...
sap.ui.define([
...
], function(Link, Control, VBox, HBox, Button, Title, Image, Text, SimpleForm, Factory, LinkData, JSONModel, Util, HorizontalLayout, VerticalLayout, layoutLibrary, PersonalizationUtil, ManagedObjectObserver, FlexItemData, BindingMode, mLibrary, Controller, SelectionWrapper, ColumnWrapper, Log, AddLink, CustomData) {
	"use strict";
		...
		return this._getFlexRuntimeInfoAPI().then(function (FlexRuntimeInfoAPI) {
			return FlexRuntimeInfoAPI.waitForChanges({element: this}).then(function() {
				return new Promise(function(resolve) {
									...
									payload: {
										callbackSaveChanges: function(oPersistentDeltaData) {
											...
											if (bDialogConfirmedReset) {
												return NavigationContainer._discardChanges(this).then(function() {
					...
					if (oControl) {
						oControl.addDependent(this.oPersonalizationController._oDialog);
					}
				}.bind(this));
			}.bind(this));
		}.bind(this));
	...
	NavigationContainer.prototype._createContent = function() {
		...
		this._oObserver = new ManagedObjectObserver(function(oChanges) {
			if ((oChanges.type === "property" || oChanges.type === "binding") && oChanges.name === "visible") {
				...
				if (this.hasContent() && !this._getInternalModel().getProperty("/extraContent")) {
					oDefaultExtraConent.setVisible(false);
				}
			}
		}.bind(this));
		...
		var oDefaultExtraConent = new SimpleForm({
		...
		oDefaultExtraConent.addStyleClass("navigationPopoverDefaultExtraContent");
		...
		this.addItem(this._oHeaderArea).addItem(this._getInternalModel().getProperty("/extraContent") || oDefaultExtraConent).addItem(oSeparator).addItem(this._oActionArea).addItem(this._oPersonalizationButton);
