...
sap.ui.define([
	'sap/ui/core/library', 'sap/m/Bar', 'sap/m/Button', 'sap/m/CheckBox', 'sap/m/Label', 'sap/m/Link', 'sap/m/List', 'sap/m/Panel', 'sap/m/SearchField', 'sap/m/Text', 'sap/m/Toolbar', 'sap/m/ToolbarSpacer', 'sap/ui/Device', 'sap/ui/comp/state/UIState', './VariantConverterFrom', './VariantConverterTo', 'sap/ui/comp/library', 'sap/ui/comp/smartvariants/PersonalizableInfo', 'sap/ui/comp/smartvariants/SmartVariantManagementUi2', 'sap/ui/comp/variants/VariantManagement', 'sap/ui/core/InvisibleText', 'sap/ui/core/Icon', 'sap/ui/core/Title', 'sap/ui/layout/Grid', 'sap/ui/layout/GridRenderer', 'sap/ui/layout/GridData', 'sap/ui/layout/HorizontalLayout', 'sap/ui/layout/VerticalLayout', 'sap/ui/layout/AlignedFlowLayout', 'sap/ui/comp/util/IdentifierUtil', 'sap/m/library', 'sap/ui/model/json/JSONModel', 'sap/ui/comp/filterbar/FilterGroupItem', 'sap/base/Log', 'sap/base/util/merge'
], function(coreLib, Bar, Button, CheckBox, Label, Link, List, Panel, SearchField, Text, Toolbar, ToolbarSpacer, Device, UIState, VariantConverterFrom, VariantConverterTo, library, PersonalizableInfo, SmartVariantManagementUi2, VariantManagement, InvisibleText, Icon, Title, Grid, GridRenderer, GridData, HorizontalLayout, VerticalLayout, AlignedFlowLayout, IdentifierUtil, mLibrary, JSONModel, FilterGroupItem, Log, merge) {
	"use strict";
	...
	var ToolbarLayoutData;
	var ToolbarSeparator;
	var OverflowToolbarLayoutData;
	var Dialog;
	var CustomListItem;
	var Form;
	var FormContainer;
	...
	var FilterBar = Grid.extend("sap.ui.comp.filterbar.FilterBar", /** @lends sap.ui.comp.filterbar.FilterBar.prototype */
	{
		metadata: {
			...
			properties: {
				...
				/**
				 * Determines what design should be used. Default is the design with toolbar. The design with toolbar is always used on phones. When
	...
	FilterBar.prototype.applySettings = function(mSettings) {
		...
		if (!this.getUseToolbar() && !this.getFilterBarExpanded()) {
			...
			// usetToolBar=false sets the filterBarExpanded to true
	...
	FilterBar.prototype._applyLayoutDataToToolbarButtons = function(sPersistenceKey) {
		...
		if (!ToolbarLayoutData && !this._bToolbarLayoutDataRequested) {
			ToolbarLayoutData = sap.ui.require("sap/m/ToolbarLayoutData");
			if (!ToolbarLayoutData) {
				sap.ui.require([
					"sap/m/ToolbarLayoutData"
				], _ToolbarLayoutDataLoaded.bind(this));
				this._bToolbarLayoutDataRequested = true;
			}
		}
		if (!ToolbarLayoutData) {
			return;
		...
		this._oHideShowButton.setLayoutData(new ToolbarLayoutData({
			shrinkable: true
		}));

		this._oClearButtonOnFB.setLayoutData(new ToolbarLayoutData({
			shrinkable: true
		}));

		this._oRestoreButtonOnFB.setLayoutData(new ToolbarLayoutData({
			shrinkable: true
		}));

		this._oFiltersButton.setLayoutData(new ToolbarLayoutData({
			shrinkable: true
		}));

		this._oSearchButton.setLayoutData(new ToolbarLayoutData({
			shrinkable: false
		}));

	};

	function _ToolbarLayoutDataLoaded(fnToolbarLayoutData) {

		ToolbarLayoutData = fnToolbarLayoutData;
		this._bToolbarLayoutDataRequested = false;
...
// }
	...
	FilterBar.prototype.removeFilterGroupItem = function(vObject) {
		...
		if (oItem.getGroupName() === FilterBar.INTERNAL_GROUP) {
			Log.warning("sap.ui.comp.filterbar.FilterBar.prototype.removeFilterGroupItem() is trying to remove FilterItem");
			return null;
		}
...
// }
	...
	/**
	 * Initializes the variant management, when the prerequisites are fullfilled. In this case the "initialise" event will be triggered only after the
	 * variant management's initialization. Triggers the "initialise" event immediately in case the prerequisites are not fullfilled.
	 * @private
	 */
...
// };
	...
	FilterBar.prototype.removeAllFilterGroupItems = function() {
		...
		if (aFilters && aFilters.length) {
			for (i = 0; i < aFilters.length; i++) {
				if (aFilters[i].getGroupName() === FilterBar.INTERNAL_GROUP) {
					continue;
				}
...
// };
	...
	/**
	 * Returns a summary string that contains information about the filters currently assigned. The string starts with "Filtered By", followed by the
	 * number of set filters and their labels.<br>
	 * Example:<br>
	...
	FilterBar.prototype._updateToolbarText = function() {
		...
		if (sText !== this._oFiltersButton.getText()) {
			this._oFiltersButton.setText(sText);

			if (this._oBasicAreaLayout) {
				this._oBasicAreaLayout.reflow();
			}
		}
	...
	FilterBar.prototype._createFiltersAndAdaptBasicArea = function() {
		...
		this._retrieveVisibleAdvancedItems();
	...
	FilterBar.prototype._createForm = function(oAdvancedLayout) {
		var that = this;
		var oFiltersForm = new Form(this.getId() + "-form", {
			editable: true,
			layout: oAdvancedLayout
		});

		oFiltersForm.contentOnAfterRendering = function(oFormElement, oControl) {
			Form.prototype.contentOnAfterRendering.apply(this, arguments);

			if (oControl.isA("sap.m.MultiInput")) {
				that._adaptStyleSheet(oControl);
			}
		};
	...
	FilterBar.prototype._adaptStyleSheet = function(oControl) {

		var sOverFlowValue;
		var oTitleElement;

		if (oControl.isA("sap.m.MultiInput")) {
			oTitleElement = oControl.$();
			if (oTitleElement) {
...
// }));
	...
	FilterBar.prototype._createHeaderAddToFilterBar = function(oFormContainer) {
		...
		var oAddToFilterBarLabel;

		if (this._isNewFilterBarDesign() || !this._isPhone()) { // label 'Add To Filterbar'

			oAddToFilterBarLabel = new Label(this.getId() + "-addToFilterBar", {
				text: this._oRb.getText("FILTER_BAR_SHOW_IN_FILTERBAR")
			});

			var oTitle = new MTitle({
				text: oFormContainer.getTitle().getText(),
				level: coreLib.TitleLevel.H5,
				titleStyle: coreLib.TitleLevel.H5
			});
			var oToolbar = new Toolbar(this.getId() + "-addToFilterBarToolbar", {
				content: [
					oTitle, new ToolbarSpacer(), oAddToFilterBarLabel
				]
			});
			oFormContainer.setToolbar(oToolbar);
			oFormContainer.addAriaLabelledBy(oTitle);
	...
	FilterBar.prototype._createFilters = function() {
		...
		if (this._possibleToChangeVariantManagement() && this._oToolbar) {
			...
			if (idx >= 0) {
				// the cloning will lead to re-appliance of variants; this is not intended
				// so here we override an internal VM method for the duration of the cloning, to overcome this.
				var fnSetSelection = VariantManagement.prototype._setSelection;
				VariantManagement.prototype._setSelection = function() {};
				this._oClonedVM = this._oVariantManagement.clone();
				VariantManagement.prototype._setSelection = fnSetSelection;
	...
	FilterBar.prototype._focusOnFirstFilterInError = function() {
		for (var n in this._mAdvancedAreaFilter) {
			if (this._mAdvancedAreaFilter[n].items) {
				for (var i = 0; i < this._mAdvancedAreaFilter[n].items.length; i++) {
					var oItem = this._mAdvancedAreaFilter[n].items[i];
					if (oItem && oItem.control && oItem.control.getValueState && oItem.control.getValueState() === "Error") {
						oItem.control.getDomRef().scrollIntoView();
						return;
					}
				}
			}
	...
	FilterBar.prototype._searchRequested = function() {
		...
		if (bInErrorState) {
			...
			// in case of error remove eventual search entry
			if (this._oFiltersSearchField.getValue()) {
				this._resetFilterBarSearch();
			}

			this._activateMainContent();

			this._focusOnFirstFilterInError();
	...
	FilterBar.prototype._showFilterDialog = function() {
		...
		if ((!Dialog || !OverflowToolbarLayoutData || !Form || !FormContainer || !FormElement || !ResponsiveGridLayout || !MTitle) && !this._bDialogRequested) {
			Dialog = sap.ui.require("sap/m/Dialog");
			OverflowToolbarLayoutData = sap.ui.require("sap/m/OverflowToolbarLayoutData");
			...
			MTitle = sap.ui.require("sap/m/Title");
			if (!Dialog || !OverflowToolbarLayoutData || !Form || !FormContainer || !FormElement || !ResponsiveGridLayout || !MTitle) {
				sap.ui.require([
		...
		this._oFilterDialog = new Dialog(this.getId() + "-adapt-filters-dialog", {
		...
		this._oFilterDialog.isPopupAdaptationAllowed = function() {
			return false;
		};
		...
		oSubHeader.addContentMiddle(this._oFiltersSearchField);
		this._oFilterDialog.setSubHeader(oSubHeader);

		var oForm = this._createFiltersAndAdaptBasicArea();
		if (!oForm) {
			Log.error("Content for Advanced Filters Dialog could not be created");
			return;
		}
	...
	function _DialogLoaded(fnDialog, fnOverflowToolbarLayoutData, fnForm, fnFormContainer, fnFormElement, fnResponsiveGridLayout, fnTitle) {

		Dialog = fnDialog;
		OverflowToolbarLayoutData = fnOverflowToolbarLayoutData;
		Form = fnForm;
...
// }
		...
		if (this._aListItems) {
			...
			for (i = this._aListItems.length - 1; i >= 0; i--) {
				...
				if (aContent && (aContent.length > 0)) {
					...
					// we want to show an item if it matches the search criteria on text or tooltip
					if ((sText.toLowerCase().indexOf(sValue) >= 0 || (sTooltip && sTooltip.toLowerCase().indexOf(sValue) >= 0))) {
						this._aListItems[i].setVisible(true);
					} else {
	...
	FilterBar.prototype._triggerSearchByValueInFilterDialog = function(sValue) {
		...
		var n = null, i, sText, sTooltip, oGroupElement, oFilterItem, nCountInvisibleElements, nCountNonPartOfCurrentVariant;
							...
							if (oGroupElement.items[i] && oGroupElement.items[i].filterItem) {
								...
								if (oFilterItem.getVisible()) {
									...
									if (sValue && oGroupElement.items[i].formelement) {
										...
										sTooltip = oFilterItem.getControlTooltip();
	...
	FilterBar.prototype._generateListItems = function(sGroupName, oList) {
		...
		if (!CustomListItem) {
			// needed for qUnit test -> normally this can not happen because called via dialog function
			CustomListItem = sap.ui.require("sap/m/CustomListItem");
			if (!CustomListItem) {
				throw new Error("_generateListItems must only be called via _createAddRemoveFiltersDialog " + this);
			}
		}
			...
			if (aAdvacedElements && aAdvacedElements.items) {
				...
				for (i = 0; i < aAdvacedElements.items.length; i++) {
					var oItem = aAdvacedElements.items[i],
						sItemId = IdentifierUtil.replace(sGroupName + "-" + oItem.filterItem.getName() + "-selectFiltersItem");
					...
					if (oItem && oItem.filterItem) {
						...
						oListItem = new CustomListItem(sItemId);
						...
						oListItem.data("quickinfo", oItem.filterItem.getControlTooltip() || oItem.filterItem.getLabelTooltip());
						...
						oLabel = new Label({
							...
							tooltip: oItem.filterItem.getControlTooltip() || oItem.filterItem.getLabelTooltip()
						});
	...
	FilterBar.prototype._createAddRemoveFiltersDialog = function(sGroupName, oLink) {
		...
		// to be safe also check for Dialog. But it should be loaded as we should be already on the Filter Dialog
		if ((!Dialog || !CustomListItem) && !this._bCustomListItemRequested) {
			CustomListItem = sap.ui.require("sap/m/CustomListItem");
			Dialog = sap.ui.require("sap/m/Dialog");
			if (!Dialog || !CustomListItem) {
				sap.ui.require([
					"sap/m/Dialog", "sap/m/CustomListItem"
				], _CustomListItemLoaded.bind(this));
				this._bCustomListItemRequested = true;
				this._oFiltersDialogData = {
		...
		this._oDialog = new Dialog(this.getId() + "-set-filters-dialog", {
			stretch: Device.system.phone,
			resizable: true,
			draggable: true
		});
		...
		this._oDialog.attachAfterClose(function() {
			...
			oLink.getDomRef().scrollIntoView();
		});

		this._oDialog.open();
	};

	function _CustomListItemLoaded(fnDialog, fnCustomListItem) {

		Dialog = fnDialog;
		CustomListItem = fnCustomListItem;
		this._bCustomListItemRequested = false;
		if (!this._bIsBeingDestroyed) {
			this._createAddRemoveFiltersDialog(this._oFiltersDialogData.groupName, this._oFiltersDialogData.link);
			delete this._oFiltersDialogData;
		}
	...
	FilterBar.prototype.fetchVariant = function() {
		...
		oVariant.filterBarVariant = this._fetchVariantFiltersData(oVariant.version);
