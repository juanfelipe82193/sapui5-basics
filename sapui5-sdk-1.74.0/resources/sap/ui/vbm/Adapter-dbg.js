...
sap.ui.define([
	...
	Action.prototype.attach = function() {
		...
		if (!this._target) {
			log.warning("unable to attach action: " + this.id);
			return false;
		}
	...
	Adapter.prototype._addAction = function(def) {
		...
		if (action.attach()) {
			...
			log.info("unable to attach action: " + def.id);
		}
	};
		...
		if (actions.Remove) { // process remove section first
			...
			list.forEach(function(def) {
				...
				if (index !== -1) {
					...
					log.info("remove of nonexistent action: " + def.id);
				}
			}, this);
		}
					...
					that._attachHandler.call(
							...
							function(oEvent) {
								...
								if (oEvent.oSource === that._map()) {
									...
									if (jQuery.sap.startsWith(payload.Action.object, this._map().getId())) {
								...
								if ((payload.Data) && (payload.Data.Merge) && (payload.Data.Merge.N)) {
									[].concat(payload.Data.Merge.N).forEach(function(n) {
										if (n.name && jQuery.sap.startsWith(n.name, this._map().getId())) {
	...
	Adapter.prototype._handler = function(event, action) {
		...
		if (event.oSource instanceof sap.ui.vbm.ClusterBase) {
			...
			if (jQuery.sap.startsWith(oEventData.Action.object, this._map().getId())) {
