...
sap.ui.define([
	...
	Adapter3D.prototype.load = function(data) {
		...
		.then(function() {
			...
			if (that._lastHoverInstance && !that._lastHoverInstance.object3D) {
				that._lastHoverInstance = null;
			}
		});
			...
			switch (item.type) {
				...
				case "{00100000-2013-1000-2200-6B060A330B2C}": // Image
					if (item.vo.image && item.vo.image !== "") {
						obj = new Image({
							src: makeDataUri(this._context.resources.get(item.vo.image)),
							tooltip: item.vo.tooltip
						});
					}
					break;
	...
	Adapter3D.prototype._handleContextMenu = function(event) {
		...
		if (!instance) {
			...
			// But there might be an action defined for an artificial refVO with name 'Scene' or 'Map'.
			event.hitPoint ? event.voGroupId = "Map" : event.voGroupId = "Scene";
		}
	...
	Adapter3D.prototype._hitTest = function(event) {
		...
		// current hit test implementation for line segments considers world space distance only
		// not the screen space which makes it almost useless, disable for now to avoid false positives
		...
		} else if (this._context.scene && this._context.scene.sceneGeo) {
			// Check hit test with the map plane
			intersects = this._viewport._mapPlaneIntersect(raycaster);
			if (intersects && intersects.length > 0) {
				return {
					info: intersects[0],
