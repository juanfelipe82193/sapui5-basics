...
sap.ui.define([
	"jquery.sap.global", "sap/ui/base/Object", "./ObjectFactory", "./../library"
], function(jQuery, BaseObject, ObjectFactory, library) {
	"use strict";
	 ...
	 * @param {object|object[]|null|undefined} singleObjectOrArray The input object that can be a single object, an array or <code>undefined</code>.
	 ...
	 * @private
	 */
	var toArray = function(singleObjectOrArray) {
		return singleObjectOrArray === undefined ? [] : [].concat(singleObjectOrArray);
	};
	...
	VBIJSONParser.prototype._processDataTypes = function(dataTypes) {
		...
		if (dataTypes && dataTypes.Set && !Array.isArray(dataTypes.Set) && !dataTypes.Set.name) {
			...
			jQuery.sap.log.error("DataTypes: only the Set verb with no type attribute is supported.", "", thisModule);
		}
	...
	VBIJSONParser.prototype._processData = function(data) {
		...
		if (setElements.length === 1 && !setElements[0].name) {
			...
			setElements.forEach(function(setElement) {
				...
				jQuery.sap.assert(nodePath.length === 1, "Compound data node paths are not supported yet.");
					...
					for (var prop in voElement) {
						switch (prop) {
							...
							case "DropTarget":
								...
								break;
							default:
								if (jQuery.sap.endsWith(prop, ".bind")) {
	...
	VBIJSONParser.prototype._populateVisualObject = function(vo, dataInstance) {
		...
		for (var attributeName in template) {
			var attribute = template[attributeName];
			if ("path" in attribute) { // The attribute is data bound.
				if (dataInstance) {
					...
					vo[attributeName] = dataInstance[attribute.path[0]];
				} else {
					...
					var value = this._getDataValueByPath(attribute.path);
					if (value !== undefined) {
			...
			if (wasSelected !== isSelected) {
				if (wasSelected) {
					...
					if (voGroup.maxSel === "1") {
						...
						// ZUTUN: do we need to send update to backend???
