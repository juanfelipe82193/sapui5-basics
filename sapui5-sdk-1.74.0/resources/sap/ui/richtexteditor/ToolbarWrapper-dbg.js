...
sap.ui.define([
	...
	function(jQuery, Control, library, IconPool, Item, RTESplitButton, URLWhitelist, Log) {
		"use strict";
		...
		ToolbarWrapper.prototype._syncColors = function (sCommand, sColor) {
			...
			switch (sCommand) {
				...
				case "BackgroundColor":
					...
					oTextButtonIcon && sap.ui.getCore().byId(oTextButtonIcon.id).setColor(sColor);
					break;
			}
		};
		/**
		...
		*/
		...
		ToolbarWrapper.prototype._generateLinkHTML = function (sURL, sTitle, bTarget, sText) {
			var linkAttrs = {
					href: sURL  && URLWhitelist.validate(sURL) ? sURL : '',
					target: bTarget ? bTarget : null,
					title: sTitle && URLWhitelist.validate(sTitle) ? sTitle : ''
				},
				...
				aButtons.push(this._helper.createButton({
					...
					press: function () {
						var tableElm = oRTE.getNativeApi().plugins.table
											.insertTable(oColsInput.getValue(), oRowsInput.getValue()),
