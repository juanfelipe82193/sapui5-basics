...
sap.ui.define([
	...
	ContentHandler.prototype.init = function() {
		...
		this.setModel(oModel, "$sapuimdcbaseinfoContentHandler");
		...
		return sap.ui.getCore().loadLibrary('sap.ui.fl', {
			...
			return new Promise(function(resolve) {
				sap.ui.require([
					...
					return this._determineLinkHandlerItems().then(function(aLinkItems) {
						...
						var aMLinkHandlerItems = this._getInternalModel().getProperty("/linkHandlerItems");
						var aMBaselineLinkHandlerItems = this._getInternalModel().getProperty("/baselineLinkHandlerItems");
						...
						oPanel.setModel(new JSONModel({
							metadata: jQuery.extend(true, [], this._getInternalModel().getProperty("/linkHandlerItems")),
							baseline: jQuery.extend(true, [], this._getInternalModel().getProperty("/baselineLinkHandlerItems"))
						}), "$sapuimdcbaseinfoContentHandler");
	...
	ContentHandler.retrieveAllMetadata = function(oPanel) {
		if (!oPanel.getModel || !oPanel.getModel("$sapuimdcbaseinfoContentHandler")) {
			return [];
		}
		var oModel = oPanel.getModel("$sapuimdcbaseinfoContentHandler");
	...
	ContentHandler.prototype._setConvertedLinkHandlerItems = function(aLinkItems, oView) {
		...
		oModel.setProperty("/linkHandlerItems/", aMLinkItems);
		...
		oModel.setProperty("/baselineLinkHandlerItems/", aMBaselineLinkItems);
	};
	ContentHandler.prototype._determineLinkHandlerItems = function() {
		if (!this._oLinkHandlerItemsPromise) {
	...
	ContentHandler.prototype._getInternalModel = function() {
		return this.getModel("$sapuimdcbaseinfoContentHandler");
	};
