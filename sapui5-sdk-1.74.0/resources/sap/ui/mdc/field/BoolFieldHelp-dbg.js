...
sap.ui.define([
	 ...
	 * @class A field help used in the <code>FieldFelp</code> aggregation in <code>Field</code> controls that shows a list for boolean values
	...
	BoolFieldHelp.prototype.getTextForKey = function(vKey) {

		if (vKey === null || vKey === undefined || vKey === "") {
			return null;
		}
	...
	BoolFieldHelp.prototype.getKeyForText = function(sText) {
	...
	function _handleItemPress(oEvent) {
		...
		if (bSelected) {
			...
			this.setProperty("conditions", [oCondition], true); // do not invalidate whole FieldHelp
			this.close();
			this.fireSelect({conditions: [oCondition], add: true});
		}
	}
