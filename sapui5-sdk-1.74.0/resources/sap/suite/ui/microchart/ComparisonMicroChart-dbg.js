...
sap.ui.define([
...
], function(jQuery, library, Control, Size, Device, FlexBox, MicroChartUtils, mobileLibrary, ResizeHandler) {
	"use strict";
		...
		if (oBar && oBar.attr("data-bar-index")) {
			...
			if (oComparisonData && oComparisonData.hasListeners("press")) {
				...
				if (Device.browser.msie) {
					jQuery(document.getElementById(this.getId() + "-chart-item-bar-" + iIndex)).focus();
				}
	...
	ComparisonMicroChart.prototype.setBarPressable = function(iBarIndex, bPressable) {
		if (bPressable) {
			...
			jQuery(document.getElementById(this.getId() + "-chart-item-bar-" + iBarIndex)).addClass("sapSuiteUiMicroChartPointer").attr("tabindex", 0).attr("title", sBarAltText).attr("role", "presentation").attr("aria-label", sBarAltText);
		} else {
			jQuery(document.getElementById(this.getId() + "-chart-item-bar-" + iBarIndex)).removeAttr("tabindex").removeClass("sapSuiteUiMicroChartPointer").removeAttr("title").removeAttr("role").removeAttr("aria-label");
		}
	};
	...
	ComparisonMicroChart.prototype._resizeHorizontally = function() {
		...
		if ($this.hasClass("sapSuiteCpMCLookWide")) {
			...
			$values.each(function() {
				oBoundingRect = this.getBoundingClientRect();
				if (jQuery(this).css("visibility") === "visible" && oBoundingRect.width > fMaxValueWidth) {
