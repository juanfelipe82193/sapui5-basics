...
], function (Control, Device, MobileLibrary, jQuery) {
	"use strict";
		...
		extendMicroChart: function (MicroChart) {
			...
			MicroChart.prototype._isLabelTruncated = function(oLabel) {
				if (Device.browser.edge || Device.browser.msie) {
					// wired behvaior in IE https://stackoverflow.com/questions/30900154/workaround-for-issue-with-ie-scrollwidth
					// return oLabel.scrollWidth > oLabel.offsetWidth && oLabel.getBoundingClientRect().width === oLabel.offsetWidth;
					return Math.abs(oLabel.scrollWidth - oLabel.offsetWidth) > 1;
				} else {
