...
sap.ui.define([
	...
	], function(mobileLibrary, Parameters, library, MicroChartRenderUtils, encodeXML) {
	"use strict";
	...
	StackedBarMicroChartRenderer.render = function (oRm, oControl) {
		...
		StackedBarMicroChartRenderer._writeMainProperties = function(oRm, oControl) {
			...
			if (oControl.getAriaLabelledBy().length) {
				...
				oRm.writeAttributeEscaped("aria-label",
	...
	StackedBarMicroChartRenderer._renderInnerContent = function(oRm, oControl, chartData) {
		oRm.write("<div");
		oRm.addClass("sapSuiteStackedMCContainer");
		oRm.writeClasses();
		oRm.write(">");
	...
	StackedBarMicroChartRenderer._renderChartBar = function(oRm, oControl, oDataBar) {
		...
		if (oDataBar.oBarData) {
			oRm.writeElementData(oDataBar.oBarData);
		}

		if (!oDataBar.color) {
			oRm.addStyle("background-color", "transparent");
		} else if (ValueColor[oDataBar.color]) {
			oRm.addClass("sapSuiteStackedMCBarSemanticColor" + encodeXML(oDataBar.color));
			...
			oRm.addStyle("background-color", encodeXML(sColor));
		}
		oRm.addStyle("width", encodeXML(oDataBar.width + "%"));
		oRm.writeClasses();
		oRm.writeStyles();
		oRm.write(">");
	...
	StackedBarMicroChartRenderer._renderChartBarLabel = function(oRm, oControl, displayValue, backgroundColor) {
		...
		oRm.writeClasses();
		oRm.addStyle("color", encodeXML(this._getLabelColor(backgroundColor)));
		oRm.writeStyles();
		oRm.write(">");
