sap.ui.define(["sap/ui/base/Object", "sap/ui/model/Context", "sap/suite/ui/generic/template/lib/MessageUtils", "sap/ui/model/Filter", "sap/ui/model/FilterOperator"],
	function(BaseObject, Context, MessageUtils, Filter, FilterOperator) {
		"use strict";

		function createNonDraft(sBindingPath, oModel, setEditableNDC, fnRegisterNonDraftCreateContext, vPredefinedValues, bCreateRequestsCanonical) {
			setEditableNDC(true);
			var oCreateContext = oModel.createEntry(sBindingPath, {
				properties: vPredefinedValues,
				batchGroupId: "Changes",
				changeSetId: "Changes",
				canonicalRequest: bCreateRequestsCanonical
			});
			 fnRegisterNonDraftCreateContext(sBindingPath, oCreateContext);
			return oCreateContext;
		}

		// create a new instance for the specified entity set with the given binding path.
		// Returns a promise that resolves to the context.
		// Note that this context is only virtual in non-draft scenarios.
		// In draft scenarios the creation of the draft may fail. In this case the returned Promise is rejected.
		function create(oDraftController, sEntitySet, sBindingPath, oModel, oApplication, vPredefinedValues) {
			sBindingPath = sBindingPath || "/" + sEntitySet;
			var bCreateRequestsCanonical = oApplication.mustRequireRequestsCanonical();
			if (oDraftController.getDraftContext().isDraftEnabled(sEntitySet)) { // draft case
				return oDraftController.createNewDraftEntity(sEntitySet, sBindingPath, vPredefinedValues, bCreateRequestsCanonical).then(function(oResponse) {
					return oResponse.context; // map response onto the contained context
				});
			}
			// non-draft case
			var oCreateContext = createNonDraft(sBindingPath, oModel, oApplication.setEditableNDC, oApplication.registerNonDraftCreateContext.bind(null, sEntitySet), vPredefinedValues, bCreateRequestsCanonical);
			return Promise.resolve(oCreateContext);
		}
		...
		*/
		...
		function fnUnsavedChangesDialog(oTemplateContract, oDraftAdministrativeData, fnBeforeDialogCallback) {
			return new Promise(function(resolve, reject) {
				...
				// todo: To avoid this undesired call from lib to object page, the relevant texts (warning, edit, cancel) should also be moved to lib
				var oi18nModel = new sap.ui.model.resource.ResourceModel({
					bundleName: "sap/suite/ui/generic/template/ObjectPage/i18n/i18n",
					async: true
				});
				oUnsavedChangesDialog.setModel(oi18nModel, "i18n");
				...
				(fnBeforeDialogCallback || Function.prototype)();
