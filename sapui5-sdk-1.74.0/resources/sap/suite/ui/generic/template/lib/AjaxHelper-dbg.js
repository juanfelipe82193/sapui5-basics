sap.ui.define([
	...
	ajaxHelper.ajax = function (mSettings){
		if (!mSettings || typeof mSettings !== "object") {
			throw new Error("send a valid setting object to call ajax");
		}
		...
		if (!url || typeof url !== "string" ) {
			throw new Error("Provide a proper url to make AJAX call");
		}
...
=	 * {
	...
	ajaxHelper.sjax = function (mSettings){
		if (mSettings.async) {
			throw new Error("AjaxHelper.sjax should not be called with async flag set");
		}
...
=	 * {
	...
	function parseXML (xmlString) {
		var response = {};
		try {
			response.data = new window.DOMParser().parseFromString(xmlString, "text/xml" );
		} catch (error) {
			response.success = false;
			response.textStatus = RETURN_STATUS.PARSING_ERROR;
			response.error = error;
