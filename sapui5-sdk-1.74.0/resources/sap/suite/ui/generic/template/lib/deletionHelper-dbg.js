...
sap.ui.define(["sap/base/util/extend"], function(extend) {
	...
	function fnGetItemsForDisplayNextObjectAfterDelete(oTemplateContract, bIsCreateDraft){
		...
		if (bIsConfigured){ // if it is configured, check for the list list available
			var oRegistryEntry = getRegistryEntryIfActiveForLevel(oTemplateContract, 0);
			if (oRegistryEntry){
				if (bIsCreateDraft){
	...
	// mObjectsToDelete maps the canonical pathes of the objects that are planned to be deleted onto Promises that are resolved, resp. rejected when the deletion was successfull resp. failed.
	function fnPrepareDeletion(oTemplateContract, mObjectsToDelete){
