sap.ui.define([
	...
	"sap/base/util/deepExtend"
		...
		DropDownController, DatePickerController, FilterItemMicroBar, FilterItemMicroLine, FilterItemMicroDonut, JSONModel,
		CustomData, Text, OverflowToolbarLayoutData, FilterOperator, Device, SimpleType, SapCoreLibrary, SapMLibrary, DatePicker, Log, InvisibleText, StableIdHelper, deepExtend) {
	"use strict";
	...
	SmartVisualFilterBar.prototype._onMetadataInit = function() {
		if (this._smartFilterContext.isInitialised()) {
	...
	*/
	...
	SmartVisualFilterBar.prototype._createHeaderItems = function (sPath, sType, oProperties) {
		...
		var sId = "visualFilterBarInvisibleText" + oFilterItem.getParentProperty();
		var oInvisibleText = new InvisibleText({id : sId});
		if (this.getAriaLabelledBy().indexOf(sId) === -1) {
			this.addAriaLabelledBy(sId);
		}
		var oCell = new VBox({
			id:  StableIdHelper.getStableId({type: "VisualFilterBar", subType: "FilterItemContainer", sProperty: oFilterItem.getParentProperty()}),
	...
	*/
			...
			var oConfigObject = {
				...
				component: {
					...
					properties: {
						...
						stringdate: filterCfg.stringdate
					}
				}
			};
	...
	SmartVisualFilterBar.prototype._checkMandatoryFilters = function() {
		...
		if (isMandatoryFieldEmpty) {
			this._smartFilterContext.showFilterDialog();
		}
	};
	...
	*/
	...
	SmartVisualFilterBar.prototype._createTitleToolbar = function(props, filterItem) {
		...
		if (oInput) {
			...
			sIcon = bIsValuehelp ? "sap-icon://value-help" : sShowDatePicker || sShowDropdown, count,
			selectedBtn = new Button({
				id: StableIdHelper.getStableId({type: "VisualFilterBar", subType: "ValueHelpButton", sProperty: filterItem.getParentProperty()}),
				text: {
				...
				visible: {
					path: "_filter>/" + filterItem.getParentProperty(),
					formatter: function(oContext) {
						if (bIsValuehelp || props.isDropDown || (bIsDatePicker && !bIsDateTimePicker)) { //for valuehelp & Drop Down
							return true;
	...
	SmartVisualFilterBar.prototype._onTitleChange = function(ev) {
		...
		if (oFilterItem.data("sOverlay") !== "true") {
			...
			if (ev.getSource().getProperty("isMandatory")) {
				oLabelMD.addStyleClass("sapMLabelRequired");
			}
			...
			var oTooltipMD = oTitleObj.titleUnitCurr == "" ? oTitleObj.titleMD : rb.getText("VIS_FILTER_TITLE_MD_WITH_UNIT_CURR", [oTitleObj.titleMD, oTitleObj.titleUnitCurr]);
	...
	SmartVisualFilterBar.prototype._resolveChartType = function(type) {
		...
		if (!compInfo) {
			...
			Log.error("Could not resolve the filter component type: \"" + type + "\", falling back to " + aType);
			type = aType;
		}
	...
	SmartVisualFilterBar.prototype.getConfig = function(bIsVariantConfig) {
		...
		for (var i = 0; i < config.filterCompList.length; i++) {
			...
			if (bIsVariantConfig) {
				...
				if (!item) {
					Log.error("The configured selected filter bar items do not correspond to the actual filter bar items.  Could be an error during initialization, e.g. a chart class not found");
					return {filterCompList: []};
				}
	...
	SmartVisualFilterBar.prototype.setSmartVariant = function(oSmartVariantId) {
		...
		if (this._oVariantManagement) {
			...
			if (typeof oSmartVariantId === "string") {
				Log.error("Variant with id=" + oSmartVariantId + " cannot be found");
			} else if (oSmartVariantId instanceof sap.ui.core.Control) {
				Log.error("Variant with id=" + oSmartVariantId.getId() + " cannot be found");
			}
		} else {
			Log.error("Missing SmartVariant");
		}
	};
	...
	*/
	SmartVisualFilterBar.prototype.addVisualFiltersToBasicArea = function(aProperties) {
		...
		if (!config) {
			Log.error("Could not add filter to basic area. No config found!");
			return false;
		} else if (!iPropertiesLength) {
			Log.error("Improper parameter passed. Pass an array of properties.");
			return false;
		} else {
			for (var i = 0; i < config.filterCompList.length; i++) {
				var compConfig = config.filterCompList[i];
			...
			if (iCountFiltersAddedtoBasicArea) {
				...
				Log.info("Filters already present in visual filter basic area");
				return false;
			}
		}
	};
