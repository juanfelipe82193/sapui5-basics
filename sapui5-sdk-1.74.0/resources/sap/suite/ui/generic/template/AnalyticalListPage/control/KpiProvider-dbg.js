sap.ui.define([
	...
	"sap/base/Log"
], function(KpiUtil, CriticalityUtil, FilterUtil, Log) {
	"use strict";
	...
	KpiProvider.prototype._getDataFromAnnotations = function(oConfig) {
		...
		if (!oEntitySet) {
			Log.error("KPI error details - Incorrect entity set");
			return;
		}
		...
		if (oKpiAnnotation) {
			...
			if (!oSelectionPresentationVariant) {
				Log.error("KPI error details - SelectionPresentationVariant does not have Path.");
				return;
			}
			...
			if (!sPresentationVariantPath) {
				Log.error("KPI error details - PresentationVariant does not have Path.");
				return;
			}
		...
		if (!sDataPointPath) {
			Log.error("KPI error details - DataPoint does not have Path.");
			return;
		}
		//DATAPOINT
		oDataPoint = oEntityType[sDataPointPath];
		if (!sSelectionVariantPath) {
			Log.error("KPI error details - SelectionVariant does not have a path");
		}
