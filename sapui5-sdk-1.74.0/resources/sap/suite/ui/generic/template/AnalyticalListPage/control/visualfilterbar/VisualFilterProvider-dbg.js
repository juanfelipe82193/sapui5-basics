...
sap.ui.define([
	...
	"sap/base/Log"
], function(MetadataAnalyser, FilterUtil, V4Terms, Log) {
	"use strict";
	var VisualFilterProvider = function(filter) {
		Log.setLevel(Log.Level.WARNING, "VisualFilter");
	...
	VisualFilterProvider.prototype._getVisualFilterAnnotation = function(sEntityNameFull) {
		...
		var allGroups = this._filter._smartFilterContext.getFilterBarViewMetadata(),
			...
			fieldInfo, propInfo, displayBehaviour;
				...
				if (filterRestriction !== "interval" && isFilterable !== "false") {
					...
					for (var propertyFields in aFields[oProperty]) {
						...
						if (propertyFields.indexOf(V4Terms.ValueList) > -1 && aFields[oProperty][propertyFields].PresentationVariantQualifier) {
							if (bIsFieldHidden) {
								Log.warning("The dimension " + currentPropName + " is set to UI.Hidden in the collection " + fieldInfo.groupEntitySet + ". Corresponding chart cannot be displayed as VisualFilter.", "", "VisualFilter");
								break;
							}
							...
							this._getAnnotationFromValueList(sEntityNameFull, isSelected, valueList, currentPropName, filterRestriction, isDropDown, isMandatory, displayBehaviour);
						}
					}
				}
			}
	...
	*/
	VisualFilterProvider.prototype._getAnnotationFromValueList = function (entityFullName, isSelected, valueListProperties, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, displayBehaviour, entityType) {
		...
		if (valueListProperties !== undefined) {
			...
			if (presentationVariantQualifierString) {
				...
				if (dimension) {
					...
					// Needs to be done
					annotationObject = this._createConsumeableObjectFromAnnotation(presentationVariantAnnotation, collectionPath, isSelected, parameters, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, displayBehaviour, entityTypeCollectionPath, selectionVariantObject);
					if (annotationObject) {
	...
	*/
	VisualFilterProvider.prototype._isDimensionDateFilterStringType = function(sEntitySet, sDimension) {
		...
		if (oDimensionFieldInfo && oDimensionFieldInfo.type === "Edm.String") {
			...
			sFieldSemantics = oDimensionFieldInfo && oDimensionFieldInfo[V4Terms.CalendarYear];
			if (sFieldSemantics && sFieldSemantics.Bool && sFieldSemantics.Bool === "true") {
	...
	/*
		...
		and form the config.
	*/
	VisualFilterProvider.prototype._createConsumeableObjectFromAnnotation = function (presentationVariantAnnotation, collectionPath, isSelected, parameters, parentProperty, filterRestriction, isDropDown, bIsMandatoryProp, displayBehaviour, entityType, selectionVariant) {
				...
				if (param && recordType && valueListProperty && localDataProperty) {
					...
					if (localDataProperty !== parentProperty && (recordType === V4Terms.ValueListParameterIn || recordType === V4Terms.ValueListParameterInOut)) {
						...
						//if (entityProperty['sap:filterable'] === undefined || entityProperty['sap:filterable'] == "true") {
							Log.error('IN Parameter valueListProperty: ' + valueListProperty + ' is non filterable');
						} else {
	...
	VisualFilterProvider.prototype._getConfig = function(annotationData) {
		...
		for (var i = 0; i < filterList.length; i++) {
			...
			if (!dim) {
				Log.error("Unknown Dimension :" + dimField);
				continue;
			}
			...
			if (!measure) {
				Log.error("Unknown Measure :" + measureField);
				continue;
			}
	...
	*/
	VisualFilterProvider.prototype._isValidDimensionandMeasure = function(sEntitySet, sDimension, sMeasure) {
		...
		if (!(oDimensionFieldInfo && oMeasureFieldInfo)) {
			Log.error("The dimension " + sDimension + " or the measure " + sMeasure + " or both are either NOT given in the chart, or IS given but undefined in metadata. Corresponding chart cannot be displayed as VisualFilter.");
			return false;
		}
		...
		for (var item in measureList) {
			...
			if (oMeaureInfo[V4Terms.Hidden] && oMeaureInfo[V4Terms.Hidden].Bool === "true") {
				Log.warning("The measure " + oMeaureInfo.name + " is set to UI.Hidden in the collection " + sEntitySet + ". Chart configured with this measure cannot be rendered.", "", "VisualFilter");
				count++;
			}
		}
		...
		if (oDimensionFieldInfo && bIsDimensionHidden) {
			Log.warning("The dimension " + sDimension + " is set to UI.Hidden in the collection " + sEntitySet + ". Corresponding chart cannot be displayed as VisualFilter.", "", "VisualFilter");
			return false;
		}

		if (!(oDimensionFieldInfo['aggregationRole'] === "dimension" && oMeasureFieldInfo['aggregationRole'] === "measure")) {
			Log.error("The dimension " + sDimension + " or the measure " + sMeasure + " or both have NO 'sap:aggregation-role' defined in metadata. Corresponding chart cannot be displayed as VisualFilter.");
			return false;
		}

		if (Object.keys(measureList).length === count) {
			Log.warning("All the measures in the collection " + sEntitySet + " are set to UI.Hidden. Corresponding chart cannot be displayed as VisualFilter.", "", "VisualFilter");
			return false;
		}
		return true;
	};
