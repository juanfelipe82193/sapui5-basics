sap.ui.define([
		...
		"sap/ui/model/FilterOperator", "sap/m/DatePicker", "sap/base/Log", "sap/ui/core/InvisibleText",
		...
		"sap/base/util/deepExtend"
			...
			OverflowToolbar, OverflowToolbarLayoutData, CustomData, Device, SapMLibrary, SapCoreLibrary, FilterOperator, DatePicker, Log, InvisibleText, deepEqual, StableIdHelper, extend, deepExtend) {
	"use strict";
	...
	var vfdController = Controller.extend("sap.suite.ui.generic.template.AnalyticalListPage.controller.VisualFilterDialogController", {
		...
		_toggle: function() {
			...
			if (aContent && (aContent.length === 2)) {
				...
				if (aContent[0].getVisible()) {
					...
					aContent[1].getItems()[0].getContent()[1].addCustomData(new CustomData({
						key: 'isToggle',
						value: true
					}));
				...
				//If VF is visible switch to CF and hide VF
					...
					aContent[0].getToolbar().getContent()[1].addCustomData(new CustomData({
						key: 'isToggle',
						value: true
					}));
				}
		...
		_addFilterSwitch: function() {
			var filterSwitchItems = [
				new SegmentedButtonItem({icon:"sap-icon://filter-fields", width:"inherit", key:"compact", tooltip:"{i18n>FILTER_COMPACT}"}),
				new SegmentedButtonItem({icon:"sap-icon://filter-analytics", width:"inherit", key:"visual", tooltip:"{i18n>FILTER_VISUAL}"})
			];
		...
		*/
		...
		_closeDialog: function() {
			if (this._searchTriggered) { //on click of go button in the dialog
			...
			for (i = 0; i < this.filterCompList.length; i++) {
				if (!Array.isArray(this.filterCompList[i])) {
					...
					if (this.filterCompList[i].obj.shownInFilterDialog) {
						...
						var sId = "visualFilterDialogInvisibleText" + this.filterChartList[i].getParentProperty();
						...
						FilterUtil._updateVisualFilterAria(oChartBox.getItems()[0]);
						var oCustomListItem = new CustomListItem({
							id: StableIdHelper.getStableId({type: "VisualFilterDialog", subType: "FilterItemContainer", sProperty: that.filterChartList[i].getParentProperty()}),
							content: oChartBox
						});
		...
		*/
		...
		_addChartCustomToolbar: function(obj, idx) {
			...
			if (this.oConfig.filterCompList[idx].component.properties.isMandatory) {
				titleMD.addStyleClass("sapMLabelRequired");
			}
			var getMeasuresButtonEnablement = function() {
				...
				if (!bEnable) {
					Log.warning("Change measure button has been disabled in the dialog as only one visible measure exists in the collection " + collectionPath, "", "VisualFilter");
				}
				return bEnable;
			};
			...
			bIsDateTimePicker = oInput.getMetadata().getName() === "sap.m.DateTimePicker",
			sShowDatePicker = (bIsDatePicker && !bIsDateTimePicker) ? "sap-icon://appointment-2" : "",
			sShowDropdown = (props.isDropDown) ? "sap-icon://slim-arrow-down" : "",
			sIcon = bIsValuehelp ? "sap-icon://value-help" : sShowDatePicker || sShowDropdown,
			count,
			items = [
					new Button({
						type: "Transparent",
					...
					new Button({
						id: StableIdHelper.getStableId({type: "VisualFilterDialog", subType: "ValueHelpButton", sProperty: props.parentProperty}),
						type: "Transparent",
						icon: (bIsValuehelp || props.isDropDown || bIsDatePicker) ? sIcon : "",
						customData: [
							new CustomData({
								key: 'isF4Enabled',
								value: (bIsValuehelp || props.isDropDown || bIsDatePicker) ? true : false
							})
						],
						visible: {
							path: "_dialogFilter>/" + sParentProperty,
							formatter: function(oContext) {
								if (bIsValuehelp || props.isDropDown || (bIsDatePicker && !bIsDateTimePicker)) { //for valuehelp,Drop Down & DatePicker
									return true;
						...
						tooltip: {
							path: "_dialogFilter>/" + sParentProperty,
							formatter: function() {
								return FilterUtil.getTooltipForValueHelp(bIsValuehelp, dimLabel, rb, count, bIsDatePicker);
							}
						},
			...
			if (aBindingParts && aBindingParts.length > 0) {
				...
				chart.bindProperty('dimensionFilterExternal', {
					...
					formatter: function () {
						...
						return that.oState.alr_visualFilterBar._getFiltersForFilterItem(aInParameters, sParentProperty, oFilter, sCurrencyField, aSVOptions);
					}
				});
			...
			chart.attachTitleChange(function(oEvent) {
				...
				if (that.filterCompList[idx].toolbar.getContent().length > 0) {
					...
					if (oProp.isMandatory) {
						that.filterCompList[idx].toolbar.getContent()[0].addStyleClass("sapMLabelRequired");
					}
		...
		_getLabelForDimensionsAndMeasures: function(obj, prop) {
			var oMetadataAnalyser = this.oState.alr_visualFilterBar._oMetadataAnalyser,
				...
				propLabel = oMetaModel.getODataProperty(oEntityTypeDefinition, prop)[V4Terms.Label];
