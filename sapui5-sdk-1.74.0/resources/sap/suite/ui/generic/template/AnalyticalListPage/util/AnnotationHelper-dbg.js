sap.ui.define(["sap/ui/base/Object",
	...
	*/
	oAnnotationHelper.createWorkingContext = function(oContext) {
		...
		if (oUniqueHierarchyNodeIDForTreeTable && oSettings.tableType === "TreeTable" && oEntityType && oEntityType.hasOwnProperty('property') && oEntityType.property.length) {
			var aIgnoredFields = [],
				sHierarchyParentID = "",
	...
	oAnnotationHelper.hasGlobalActionsForALP = function(aTableTerm, aChartTerm, sEntitySet, oManifestExt) {
		if (sEntitySet && oManifestExt && oManifestExt["sap.suite.ui.generic.template.AnalyticalListPage.view.AnalyticalListPage"] && sap.suite.ui.generic.template.AnalyticalListPage.util.AnnotationHelper._hasCustomGlobalActions(sEntitySet, oManifestExt["sap.suite.ui.generic.template.AnalyticalListPage.view.AnalyticalListPage"]["sap.ui.generic.app"])) { //Check for AnalyticalListPage
			return true;
		}
		return false;
	};
	...
	oAnnotationHelper.getAreaShrinkRatio = function(aTableTerm, aChartTerm, sEntitySet, oManifestExt) {
		var bGlobalActionsPresent = sap.suite.ui.generic.template.AnalyticalListPage.util.AnnotationHelper.hasGlobalActionsForALP(aTableTerm, aChartTerm, sEntitySet, oManifestExt);
	...
	oAnnotationHelper._resolveLineItemCriticality = function(lineItem,lineItemQualifier) {
		if (lineItem) {
			...
			Log.error("Please add LineItem annotation for your application");
			return undefined;
		}
	};
	 ...
	 * @param  [array] array of SV parameter objects
	 * @return [array] array of parameter objects with correct parameter name
	 */
	 oAnnotationHelper.resolveParameterStringValue = function(aParameter) {
		if (aParameter) {
			aParameter.forEach(function(oParam) {
				oParam.PropertyName.PropertyPath = (oParam.PropertyName.PropertyPath.split("/")[1] || oParam.PropertyName.PropertyPath);
			});
		}
		return aParameter;
	 };
