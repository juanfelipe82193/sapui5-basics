sap.ui.define([
...
], function (Control, KpiTagController, KpiUtil, KpiAnnotationHelper, AnnotationHelper, FilterUtil, V4Terms,
	ODataModelUtil, KpiProvider, Device, SelectionVariant, Filter, JSONModel, Locale, ResponsivePopover, MessageStrip,
	NumberFormat, SapMLibrary, SapCoreLibrary, Log, GenericTag, ObjectNumber, KeyCodes) {
	"use strict";
	var TARGET = "Target",
		MAXIMIZE = "Maximize",
		MINIMIZE = "Minimize";
	return GenericTag.extend("sap.suite.ui.generic.template.AnalyticalListPage.control.SmartKpiTag", {
		...
		init: function () {
			...
			Log.setLevel(Log.Level.WARNING, this);
		},
		...
		_onMetaDataInit: function () {
			...
			this._oModel.getMetaModel().loaded().then(function () {
				...
				if (!this.kpiSettings) {
					Log.error("KPI Error details: incorrect KPI configuration");
					this._updateKpiList(true);
					return;
				}
		...
		_createFilterableKpi: function (oEvent) {
			...
			try {
				...
				var config = this.kpiSettings,
					...
					aParameters = AnnotationHelper.resolveParameterStringValue(config.parameters),
				...
				this._updateKpiList(true);
				Log.error("KPI Error details: " + oError);
			}
		},
		...
		_kpiSuccessHandler: function (data, response) {
			...
			try {
				...
				Log.warning("KPI error details: " + oRb.getText("KPI_NO_DATA"), " ", this);
		...
		_kpiErrorHandler: function (response, bIsReqNotFilterPresent, bIsKPIData, bIsReqFilterbarData) {
			...
			if (response.statusCode === "401" || response.statusCode === "403" || response.authorizationError) {
				...
				Log.warning("KPI error details: " + oRb.getText("KPI_AUTHORIZATION_ISSUE"), "", this);
				return;
			}
			if (this.kpiProvider.kpiConfig.oDefaultPV === false) { //Hide KPI if the DefaultPV is not present
				...
				Log.warning("KPI error details: " + oRb.getText("KPI_DEFAULT_PV_ERROR_MESSAGE"), "", this);
				return;
			}
			if (bIsReqNotFilterPresent) {
				this.setProperty('errorMessage', oRb.getText("REQUIRED_VH_FIELDS_OVERLAY_MESSAGE"));
				Log.error("KPI error details: " + oRb.getText("REQUIRED_VH_FIELDS_OVERLAY_MESSAGE"));
				this.setStatus("Error");
				...
				Log.info("KPI error details: " + oRb.getText("KPI_INFO_FOR_MISSING_MANDATE_FILTPAR"), " ", this);
				...
				Log.warning("KPI error details: " + oRb.getText("KPI_DRILLDOWN_NAVIGATION_MESSAGE"), "", this);
				this.setStatus("Warning");
			...
			if (response && response.statusCode && response.message) {
				Log.error("KPI error details: " + response.statusCode + " " + response.message);
			}
			this._setKpiName();
		...
		_onMouseClick: function (oEvent) {
			if (this.kpiSettings) {
				...
				Log.error("KPI error details - Incorrect KPI configuration. Unable to open card");
			}
		},
