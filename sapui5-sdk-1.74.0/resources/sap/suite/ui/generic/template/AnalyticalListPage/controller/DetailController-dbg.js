...
sap.ui.define([
	...
	JSONModel, Filter, RowSettings, FilterOperator, SapTableLibrary, listUtils, Log, isEmptyObject) {
		"use strict";
		...
		var tController = Controller.extend("sap.suite.ui.generic.template.AnalyticalListPage.controller.DetailController", {
			...
			_onBeforeRebindTable: function(oEvent) {
				...
				if (this.oState.chartController && this.oState.chartController.oChart) {
					var oChart = this.oState.chartController.oChart,
						oChartInfo = this.oState.chartController._chartInfo,
						filtersFromChartDrilldown = (oChartInfo.drillStack && oChartInfo.drillStack.length > 0) ? oChartInfo.drillStack[oChartInfo.drillStack.length - 1] : undefined;
				}
				...
				if (this.isFilter() && this.oState.oSmartChart && !_ignoreChartSelections) {
					this._applyChartSelectionOnTableAsFilter(oEvent, oChart);
				}
				//Apply drill down filters if available irrespective of whether ignoreChartselection is true/false only in FilterMode

				if (this.isFilter() && filtersFromChartDrilldown && filtersFromChartDrilldown.filter) {
					oEvent.getParameter("bindingParams").filters.push(filtersFromChartDrilldown.filter);
				}
				...
				this.oState.oTemplateUtils.oCommonEventHandlers.onBeforeRebindTable(oEvent, {
					ensureExtensionFields: this.oState.oController.templateBaseExtension.ensureFieldsForSelect,
					...
					if (pageFilterList[i].aFilters !== undefined) {
						...
						for (var j = 0; j < filterList.length; j++) {
							...
							if (!oBinding.getProperty(name)) {
								Log.warning("Could not apply filter with name \"" + name + "\" as that field does not exist in the entity type");
								continue;
							}
			...
			_applyParamsToTableAsHighlight: function(updateType) {
				...
				if (!binding) { // if columns haven't been choosen then binding is undefined
					Log.error("No table binding to apply the selection(s) to");
					return;
				}
