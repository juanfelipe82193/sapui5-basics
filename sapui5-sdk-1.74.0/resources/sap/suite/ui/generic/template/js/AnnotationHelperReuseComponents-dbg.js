...
*/
sap.ui.define(["sap/suite/ui/generic/template/extensionAPI/UIMode", "sap/base/util/extend"], function(UIMode, extend) {
	"use strict";
	...
	function getControlVisibleTerm(sControlId){
	...
	function getIsAreaVisibleBindingForSubsection(oReuseComponent, sSectionId, sSubSectionId){
		return "{= $" + BINDING_FOR_ISACTIVE + " && !!${_templPriv>/generic/embeddedComponents/" + oReuseComponent.id + "/isInVisibleArea} && " + getControlVisibleTerm(sSectionId) + " && " + getControlVisibleTerm(sSubSectionId) + " }";
	}
