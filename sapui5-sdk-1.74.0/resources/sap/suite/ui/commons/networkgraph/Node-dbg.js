...
sap.ui.define([
	...
	"sap/m/Text",
	"./NodeRenderer"
], function (jQuery, library, ElementBase, ElementAttribute, IconPool, Device, encodeXML, CheckBox, Text, NodeRenderer) {
	"use strict";
	...
	/* HTML Rendering */
	/* =========================================================== */
	Node.prototype._renderHtmlInfoIcon = function (mArguments) {
		mArguments = mArguments || {};

		var sHtml = "",
			sStatusIcon = this.getStatusIcon(),
			sStyle = this._convertToStyle(mArguments);
		sHtml += "<div title=\"" + oResourceBundle.getText("NETWORK_GRAPH_COLLAPSED") + "\" id=\"" + this.getId() + "-statusiconwrapper" + "\" " + sStyle + " class=\"sapSuiteUiCommonsNetworkNodeDivInfoWrapper sapSuiteUiCommonsNetworkNodeInfoWrapper\" >";

		sHtml += "<div class=\"sapSuiteUiCommonsNetworkNodeDivInfoIcon sapSuiteUiCommonsNetworkGraphDivNodeIcon\" >";
		sHtml += this._renderHtmlIcon(sStatusIcon || "sap-icon://notification");
		sHtml += "</div>";
	...
	Node.prototype._renderHtmlText = function (sText, iSize, sClass, sColor) {
		var sHtml = this._renderHtmlElement("div", {
	...
	Node.prototype._renderHtmlTitle = function (sColorStyle) {
		var sTitle = this.getTitle(),
			sHtml = "";

		if (sTitle) {
			sHtml += this._renderHtmlText(sTitle, this.getTitleLineSize(), "sapSuiteUiCommonsNetworkGraphDivNodeTitleText sapSuiteUiCommonsNetworkGraphDivNodeTitle", sColorStyle);
	...
	Node.prototype._renderHtmlDescription = function (sColorStyle) {
		var iDescriptionSize = this.getDescriptionLineSize(),
			sDescription = this.getDescription();

		if (sDescription && iDescriptionSize != -1) {
			return this._renderHtmlText(sDescription, iDescriptionSize, "sapSuiteUiCommonsNetworkGraphDivNodeDescription", sColorStyle);
		}

		return "";
	};

	Node.prototype._renderHtmlTitleAndDescription = function (bTitleOnly) {
		return this._renderHtmlTitle() + this._renderHtmlDescription();
	};
	...
	Node.prototype._renderHtmlAttributes = function (mOptions) {
		...
		if (aAttributes.length > 0) {
			mOptions.renderManager.write("<div class=\"sapSuiteUiCommonsNetworkGraphDivNodeAttributes\">");

			aAttributes.forEach(function (oAttribute) {
				this._renderClonedControl(mOptions, oAttribute);
			}.bind(this));

			mOptions.renderManager.write("</div>");
		}
	};
	...
	Node.prototype._getCorrectWidth = function () {
		var iWidth = (this._iWidth || this.getWidth()),
			iMaxWidth = "";
	...
	Node.prototype._renderHtml = function (mOptions) {
		...
		mOptions.renderManager.write(this._renderHtmlElement("div", {
		...
		this._processRenderManager(mOptions.renderManager, this.renderContent(mOptions));
		mOptions.renderManager && mOptions.renderManager.write("</div>");
	};
	...
	Node.prototype._renderHtmlContent = function (mOptions) {
		...
		var fnProcessContent = function () {
			...
			if (oContent) {
				mOptions.renderManager.write("<div id=\"" + this.getId() + "-contentwrapper\">");
				this._renderClonedControl(mOptions, oContent);
				mOptions.renderManager.write("</div>");
			} else {
				this._processRenderManager(mOptions.renderManager, this.renderItemContent(mOptions));
			}
		}.bind(this);
		...
		if (bIsCircle) {
			...
			mOptions.renderManager.write("<div id=\"" + sId + "-wrapperwithbuttons\"" + sMainStyle + " class=\"sapSuiteUiCommonsNetworkGraphDivInnerWrapper\">");
			mOptions.renderManager.write(this.renderHtmlActionButtons(mOptions));

			mOptions.renderManager.write("<div id=\"" + sId + "-wrapper\"" + sBorderColor + " class=\"sapSuiteUiCommonsNetworkGraphDivInner\">");

			mOptions.renderManager.write("<div class=\"sapSuiteUiCommonsNetworkDivCircleWrapper\">");
			mOptions.renderManager.write(this._renderHtmlInfoIcon());
			mOptions.renderManager.write("<div " + sFocusColor + " id=\"" + sId + "-focus\" class=\"sapSuiteUiCommonsNetworkDivFocus\">");
			mOptions.renderManager.write("<div id=\"" + sId + "-status\"" + sStatusStyle + " class=\"sapSuiteUiCommonsNetworkDivCircleStatus\">");
			...
			mOptions.renderManager.write(this._renderHtmlTitleAndDescription());
		} else {
			...
			mOptions.renderManager.write(this.renderHtmlActionButtons(mOptions));
			mOptions.renderManager.write("<div " + sMainStyle + " id=\"" + sId + "-wrapper\" class=\"sapSuiteUiCommonsNetworkGraphDivInner\">");
			mOptions.renderManager.write("<div " + sFocusColor + " id=\"" + sId + "-focus\" class=\"sapSuiteUiCommonsNetworkDivFocus\"></div>");
			...
			mOptions.renderManager.write("<div id=\"" + sId + "-header\" " + sStatusStyle + " class=\"sapSuiteUiCommonsNetworkGraphDivHeader sapSuiteUiCommonsNetworkGraphDivHeaderColor");

			if (!this._hasAdditionalContent()) {
				mOptions.renderManager.write(" sapSuiteUiCommonsNetworkGraphDivHeaderOnly");
			}
			mOptions.renderManager.write("\">");

			if (this._showHeaderCheckBox()) {
				this._renderClonedControl(mOptions, this._getHeaderCheckbox());
			}

			mOptions.renderManager.write(this._renderHtmlTitleIcon("sapSuiteUiCommonsNetworkGraphDivNodeIconTitle"));
			mOptions.renderManager.write(this._renderHtmlTitle(sContentColor));
			mOptions.renderManager.write("</div>");
	...
	Node.prototype._getHeaderCheckbox = function () {
		...
		if (!oCheckBox) {
			...
			oCheckBox.addEventDelegate({
				"onAfterRendering": function () {
					oCheckBox.$().mousedown(function (oEvent) {
	...
	Node.prototype._renderHtmlTitleIcon = function (sClass) {
		...
		if (oImage) {
			var oImageSize = this._getImageSize();

			sHtml = this._renderHtmlElement("img", {
				width: oImageSize.width + "px",
				height: oImageSize.height + "px"
			}, {
				"src": encodeXML(oImage.getSrc()),
				"class": "sapSuiteUiCommonsNetworkGraphDivNodeImage"
			});
		} else if (sIcon) {
			sHtml = "<div " + sColorStyle + " class=\"" + sClass + "\">" + this._renderHtmlIcon(sIcon, "", "", sFontSize) + "</div>";
		}
	...
	Node.prototype._renderHtmlBoxContent = function (mOptions) {
		var sHtml = "",
			bHasAdditionalContent = this._hasAdditionalContent(),
			sId = this._getElementId(mOptions.idSufix);

		if (bHasAdditionalContent) {
			sHtml += "<div id=\"" + sId + "-additionalwarpper\" class=\"sapSuiteUiCommonsNetworkGraphDivAdditionalContent\">";
			var sColorStyle = this._getColor(ElementBase.ColorType.Content);

			sHtml += this._renderHtmlDescription(sColorStyle);

			this._processRenderManager(mOptions.renderManager, sHtml);
			sHtml = "";
	...
	Node.prototype._renderHtmlCircleContent = function (mOptions) {
		return this._renderHtmlTitleIcon("sapSuiteUiCommonsNetworkGraphDivNodeCircleIcon");
	};
	...
	Node.prototype._renderInfoIcon = function (iX, iY, iSize, iFontSize) {
		...
		sHtml += this._renderIcon({
			...
			icon: sStatusIcon || "sap-icon://notification",
	...
	Node.prototype._setNodeOpacity = function (bSet) {
		...
		if (this._isCircle()) {
			...
			if (bSet) {
				this.$("wrapperwithbuttons").append(jQuery('<div/>', {
	 ...
	 * @public
	 */
	Node.prototype.renderHtmlIcon = function (sIcon) {
	...
	Node.prototype.renderHtmlActionButtons = function (mOptions) {
		...
		var sRightHtml = "<div id=\"" + sRightId + "\" class=\"sapSuiteUiCommonsNetworkGraphDivActionButtons sapSuiteUiCommonsNetworkGraphDivActionButtonsRight\"></div>",
			sLeftHtml = "<div id=\"" + sLeftId + "\" class=\"sapSuiteUiCommonsNetworkGraphDivActionButtons sapSuiteUiCommonsNetworkGraphDivActionButtonsLeft\"></div>";

		return sLeftHtml + sRightHtml;
	};
	 ...
	 * @public
	 */
	Node.prototype.renderHtmlInfoIcon = function (oStyle) {
		return this._renderHtmlInfoIcon(oStyle);
	};
	...
	Node.prototype._processRenderManager = function (oRm, sHtml) {
		if (oRm && sHtml) {
			oRm.write(sHtml);
		}
