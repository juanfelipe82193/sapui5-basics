...
sap.ui.define([
...
], function (library, jQuery, ElementBase, IconPool, CheckBox, encodeXML) {
	"use strict";
	...
	Group.prototype._renderTopGroup = function (mOptions) {
		...
		if (bIsLeft) {
			sHtml += "<div class=\"sapSuiteUiCommonsNetworkTopGroupDelimiter\"></div>";
		}

		sHtml += this._renderHtmlElement("div", {
			top: 0,
			left: iX + "%",
			width: "50%",
			height: "32px"
		...
		sHtml += "<div class=\"sapSuiteUiCommonsNetworkTopGroupHeader\">";
		if (sIcon) {
			sHtml += this._renderHtmlIcon(sIcon);
		}

		if (sTitle) {
			sHtml += "<div class=\"sapSuiteUiCommonsNetworkGroupHeaderTitle\">" + encodeXML(sTitle) + " </div>";
		}
	...
	Group.prototype._render = function (mOptions) {
		...
		if (this._iNestedLevel === 0 && oParent._isTwoColumnsLayout()) {
			mOptions.renderManager.write(this._renderTopGroup(mOptions));
			return "";
		}
	...
	Group.prototype._renderHeader = function (mOptions) {
		...
		mOptions.renderManager.write("<div " + sBackgroundStyle + " class=\"sapSuiteUiCommonsNetworkGroupHeader\">");

		if (this._bIsSwimLane && (sColorStyle || this._hasDefaultStatus())) {
			mOptions.renderManager.write("<div " + sColorStyle + " class=\"sapSuiteUiCommonsNetworkGroupHeaderStatus\"></div>");
		}

		mOptions.renderManager.write("<div class=\"sapSuiteUiCommonsNetworkGroupHeaderInner sapSuiteUiCommonsNetworkGroupHeaderInnerText\">");
		...
		if (sIcon) {
			mOptions.renderManager.write(this._renderHtmlIcon(sIcon, "sapSuiteUiCommonsNetworkGroupHeaderIcon", null, sContentStyle));
		}

		if (sTitle) {
			mOptions.renderManager.write("<div " + sContentStyle + " class=\"sapSuiteUiCommonsNetworkGroupHeaderTitle\">" + encodeXML(sTitle) + " </div>");
		}
		mOptions.renderManager.write("</div>");

		// icons
		mOptions.renderManager.write("<div class=\"sapSuiteUiCommonsNetworkGroupHeaderInner sapSuiteUiCommonsNetworkGroupHeaderInnerIcons\">");
		mOptions.renderManager.write(this._renderHtmlIcon(this.getCollapsed() ? EXPAND_ICON : COLLAPSE_ICON, "sapSuiteUiCommonsNetworkGroupHeaderMenuIcon", "collapse",
			sContentStyle, oResourceBundle.getText("NETWORK_GRAPH_EXPAND_COLLAPSE")));
		mOptions.renderManager.write(this._renderHtmlIcon(MENU_ICON, "sapSuiteUiCommonsNetworkGroupHeaderMenuIcon", "menu",
			sContentStyle, oResourceBundle.getText("NETWORK_GRAPH_GROUP_DETAILS")));
		mOptions.renderManager.write("</div>");
	...
	Group.prototype._setupEvents = function () {
		...
		$actionButtons.unbind("mouseover");
		$actionButtons.unbind("mouseout");
		...
		$actionButtons.mouseover(function (oEvent) {
		...
		$actionButtons.mouseout(function (oEvent) {
