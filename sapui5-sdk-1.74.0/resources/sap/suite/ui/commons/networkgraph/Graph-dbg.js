...
sap.ui.define([
	...
	"./GraphRenderer"
], function (library, jQuery, Control, SvgBase, Node, Line, Group, LayeredLayout, Tooltip, Popup, JSONModel, SuggestionItem, Utils, ButtonType,
			 ...
			 Parameters, Log, Measurement, uid, encodeXML, GraphRenderer) {
	"use strict";
		...
		if (this._isDelayedLayouting()) {
			...
			var fnProcess = function () {
				if (this._iNodeMaxWidth) {
					this.$divnodes.css("width", this._iNodeMaxWidth + 50);
				}
	...
	Graph.prototype._fireFailure = function (sType, sMsg) {
		...
		if (sType === Graph.FAILURE_TYPE.INCONSISTENT_MODEL) {
			sConsoleText += " This may be due to the model size limitation. For its increase use model's 'setSizeLimit' method.";
		}
	...
	Graph.prototype._render = function () {
		...
		if (this._fZoomLevel !== 1) {
			...
			this.$svg.width(this.$svg.width() * this._fZoomLevel);
			this.$svg.height(this.$svg.height() * this._fZoomLevel);

			this.$background.width(this.$svg.width() * this._fZoomLevel);
	...
	Graph.prototype._innerRender = function (mParams) {
		...
		if (this._isUseNodeHtml()) {
			aNodes.forEach(function (oNode) {
				...
				$node.css("top", oNode.getY());
				$node.css("left", oNode.getX());
			});
		}
	...
	Graph.prototype._renderSvg = function (sNodes, sLines) {
		...
		sHtml += " style=\"margin:auto;";

		if (!this._isUseNodeHtml()) {
			sHtml += "position:relative";
		}
	...
	Graph.prototype._toggleFullScreen = function () {
		var fnOpen = function () {
			...
			if (this._oFullScreenContainer.$content) {
				// for proper working this has to be 100%
				// some container controls (or Width property) sets width directly to element so we need to
				// overwrite it
				this.$().addClass("sapSuiteUiCommonsNetworkGraphFullScreen");
		...
		var fnClose = function () {
			this.$().removeClass("sapSuiteUiCommonsNetworkGraphFullScreen");
	...
	Graph.prototype._wheel = function (mArguments) {
		if (this.getEnableWheelZoom() || mArguments.ctrl) {
			...
			mArguments.div.css("transition-duration", "0.3s");
			mArguments.div.css("opacity", "0.6");
			setTimeout(function () {
				mArguments.div.css("transition-duration", "0.8s");
				mArguments.div.css("opacity", "0");
			}, 1500);
		}
