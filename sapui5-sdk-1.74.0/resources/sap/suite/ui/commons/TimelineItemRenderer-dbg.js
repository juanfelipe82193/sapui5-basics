...
sap.ui.define([
...
], function (library, HtmlElement, DateFormat, Icon, encodeXML, encodeCSS) {
	"use strict";
	...
	TimelineItemRenderer._getHorizontalTimelineItemElement = function (oTimelineItem) {
		...
		if (sGroupId) {
			eTimelineItem.setAttribute("groupid", oTimelineItem._groupID, true);
		}
	...
	TimelineItemRenderer._getVerticalTimelineItemElement = function (oTimelineItem) {
		...
		if (sGroupId) {
			eTimelineItem.setAttribute("groupID", sGroupId, true);
		}
		eTimelineItem.addControlData(oTimelineItem);
	...
	TimelineItemRenderer._addAccessibilityTags = function (eTimelineItem, oTimelineItem, bIsGroupHeader) {
		...
		if (bIsGroupHeader) {
			...
			eTimelineItem.setAttribute("aria-label", sGroupHeader, true);
		}
	};
