...
sap.ui.define([
	...
	"./FeedTileRenderer"
], function (jQuery, library, Control, FeedItemUtils, Log, encodeCSS, encodeXML, URLWhitelist, FeedTileRenderer) {
	"use strict";
	...
	FeedTile.prototype.cycle = function () {
		...
		if (jQuery.support.cssTransitions) {
			...
			setTimeout(function () {
				...
				$oFromFeed.bind("webkitTransitionEnd transitionend", fAfterTransition.bind(this));
				$oToFeed.bind("webkitTransitionEnd transitionend", fAfterTransition.bind(this));
	...
	FeedTile.prototype.setDefaultImages = function (oDefaultImages) {
		if (oDefaultImages && oDefaultImages.length > 0) {
			...
			for (var i = 0; i < oDefaultImages.length; i++) {
				...
				var validUrl = URLWhitelist.validate(oDefaultImage);
