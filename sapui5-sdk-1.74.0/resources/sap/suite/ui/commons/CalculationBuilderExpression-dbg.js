sap.ui.define([
	...
	"sap/ui/thirdparty/jqueryui/jquery-ui-selectable"
			 ...
			 Button, FlexBox, FlexRendertype, FlexAlignItems, MobileLibrary, SegmentedButton, SegmentedButtonItem,
			 StepInput, Input, InputType, Page, List, ListMode, ListType, StandardListItem, NavContainer, SearchField,
			 Label, Panel, ResponsivePopover, PlacementType, Toolbar, MessageStrip, ValidationResult, ControlProxy, Icon) {
	"use strict";
	...
	var CalculationBuilderExpression = Control.extend("sap.suite.ui.commons.CalculationBuilderExpression", /** @lends sap.suite.ui.commons.CalculationBuilder.prototype */ {
		...
		renderer: function (oRm, oCalculationBuilderExpression) {
			...
			oRm.write("<div id=\"" + oCalculationBuilderExpression.getId() + "-erroricon\"  class=\"sapCalculationBuilderExpressionErrorIcon\">");
	...
	CalculationBuilderExpression.prototype._renderDelimiter = function (iIndex) {
		...
		sHtml += "<div class=\"sapCalculationBuilderDroppableLine\"></div>";
		sHtml += "<div class=\"sapCalculationBuilderDroppableCircle\"></div>";
	...
	CalculationBuilderExpression.prototype._createPopoverConstantsItems = function (oPopoverItems) {
		if (this.getParent().getAllowStringConstants()) {
			oPopoverItems.constantInput = new Input({
				width: "100%",
				placeholder: oResourceBundle.getText("CALCULATION_BUILDER_ADD_CONSTANT_FIELD_PLACEHOLDER_ANY_STRING"),
				valueStateText: oResourceBundle.getText("CALCULATION_BUILDER_ADD_CONSTANT_FIELD_PLACEHOLDER_ERROR"),
				liveChange: function (oEvent) {
					...
					oPopoverItems.constantInput.okButton.setEnabled(bIsCorrect);
				}
			});
		} else {
			oPopoverItems.constantInput = new StepInput({
				width: "100%",
	...
	CalculationBuilderExpression.prototype._sanitizeStringConstant = function (sValue) {
		if (this.getParent()._isStringConstant(sValue)) {
	...
	CalculationBuilderExpression.prototype._createPopover = function (oPopoverItems) {
		var fnShowCorrectPage = function () {
			...
			oPopoverItems.constantInput.setValue(this.getParent().getAllowStringConstants() ? "" : 0);
			...
			if (!oItem) {
				...
				if (oItem._isFunction()) {
					...
					// Setup for Constants
					var sSanitizedKey = this._sanitizeStringConstant(oItem.getKey());
					oPopoverItems.constantInput.setValue(sSanitizedKey);
	...
	CalculationBuilderExpression.prototype._getPageFooter = function (sPageId, oPopoverItems) {
		...
		if (sPageId === (this.getId() + Ids.PAGE_CONSTANT)) {
			bEnabledConfirm = oPopoverItems.constantInput.getValueState() === ValueState.None;
			fnConfirm = function () {
				var sValue = oPopoverItems.constantInput.getValue();
				if (this.getParent() && this.getParent().getAllowStringConstants() && !jQuery.isNumeric(sValue)) {
					sValue = "\"" + sValue + "\"";
				}
		...
		oPopoverItems.constantInput.okButton = new Button({
		...
		return new Toolbar({
			content: [
				new ToolbarSpacer(),
				oPopoverItems.constantInput.okButton,
				new Button({
	...
	CalculationBuilderExpression.prototype._smartRender = function (aNewItems) {
		...
		for (var i = 0; i < aNewItems.length; i++) {
			...
			if (!oItem) {
				...
				$item[0].innerHTML = oItem._innerRender();
				$item.attr("class", oItem._getClass());
