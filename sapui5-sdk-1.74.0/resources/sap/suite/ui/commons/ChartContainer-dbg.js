...
], function (jQuery, library, MobileLibrary, VizLibrary, ManagedObject, Button, ButtonType, OverflowToolbar, OverflowToolbarButton,
			 SegmentedButton, Title, ToolbarSpacer, Device, Control, CustomData, Popup, ResizeHandler, ScrollEnablement, ToggleButton, uid, Log, ChartContainerRenderer) {
	"use strict";
		...
		if (this.getAutoAdjustHeight() || this.getFullScreen()) {
			...
			if ($Toolbar[0] && $VizFrame[0] && oSelectedContent) {
				...
				if (oInnerChart) {
					...
					} else if (oInnerChart.$().innerWidth() !== this.$("chartArea").innerWidth()) {
