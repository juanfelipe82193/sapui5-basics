sap.ui.define([
...
], function (jQuery, library, Control, Parameters, TAccountPanel, IconPool, Icon, Button, encodeXML, Configuration, ItemNavigation, TAccountUtils, ResizeHandler) {
	"use strict";
	...
	var TAccountGroup = Control.extend("sap.suite.ui.commons.taccount.TAccountGroup", {
		...
		renderer: function (oRm, oGroup) {
			...
			oRm.writeAttributeEscaped("aria-label", oGroup._getSumText());
			oRm.write(">");
	...
	TAccountGroup.prototype._getExpandAltText = function (bCollapse) {
		return (bCollapse ? oResourceBundle.getText("TACCOUNT_COLLAPSE") : oResourceBundle.getText("TACCOUNT_EXPAND")) + " " + oResourceBundle.getText("TACCOUNT_GROUP_TITLE");
	};
	...
	TAccountGroup.prototype._expandCollapse = function () {
		...
		this._getExpandCollapse().$().attr("aria-label", sText);
