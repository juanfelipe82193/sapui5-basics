sap.ui.define([
	...
	"./TAccountPanelRenderer"
			 ...
			 ToolbarSpacer, Text, SegmentedButton, SegmentedButtonItem, FilterOperator, encodeXML, NumberFormat, Core, Configuration, ButtonType, TAccountUtils, TAccountPanelRenderer) {
	"use strict";
	...
	TAccountPanel.prototype._getSumText = function () {
		...
		return TAccountUtils.formatCurrency(Math.abs(oSum.sum), oSum.measure) + " " + encodeXML(oSum.measure);
	};
	...
	TAccountPanel.prototype._setTotalBalanceTitle = function () {
		...
		if (!oSum.correct || oSum.sum === 0) {
			sTitle = oResourceBundle.getText("TACCOUNT_TOTALBALANCE");
		} else {
			sTitle = oSum.sum > 0 ? oResourceBundle.getText("TACCOUNT_TOTALCREDIT") : oResourceBundle.getText("TACCOUNT_TOTALDEBIT");
		}
	...
	TAccountPanel.prototype._valueChanged = function (iDiff, sMeasure) {
		var oSum = this.getSum();
		if (oSum.correct) {
