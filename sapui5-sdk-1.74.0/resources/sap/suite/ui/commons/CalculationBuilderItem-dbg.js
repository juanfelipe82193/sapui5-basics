sap.ui.define([
	...
	CalculationBuilderItem.prototype._innerRender = function () {
		...
		var fnRenderExpandButton = function () {
			sHtml += "<div class=\"sapCalculationBuilderItemExpandButtonWrapper\" title=\"" + oResourceBundle.getText("CALCULATION_BUILDER_EXPAND_BUTTON_TITLE") + "\">";
		...
		var fnGetItemAriaLabel = function (sTextBundle) {
			return " aria-label=\"" + oResourceBundle.getText(sTextBundle) + "\" ";
		};

		if (this._bIsNew || this._isEmpty()) {
			sAriaAttr = " title=\"" + oResourceBundle.getText("CALCULATION_BUILDER_NEW_ITEM_TITLE") + "\" ";
		} else if (oItemsWithAriaLabel[sLabel]) {
			sAriaAttr = fnGetItemAriaLabel("CALCULATION_BUILDER_" + oItemsWithAriaLabel[sLabel] + "_ARIA_LABEL");
		}

		sHtml += "<div class=\"sapCalculationBuilderItemContentWrapper\">";
		sHtml += "<div id=\"" + this.getId() + "-content\" class=\"sapCalculationBuilderItemContent\"" + sAriaAttr + ">";
		...
		if (this._isEmpty()) {
			...
			sHtml += encodeXML(oFunction.title || this.getKey());
			sHtml += "</span>";
	...
	CalculationBuilderItem.prototype._render = function () {
		...
		var bIsItemInBuilder = this._hasCorrectParent(),
			...
			sTooltipAttr = sTooltip ? "title=\"" + sTooltip + "\"" : "",
			sHtml = "";

		sHtml += "<div class=\"" + this._getClass(!!oError) + "\" id=\"" + this.getId() + "\" tabindex=\"" + (bIsItemInBuilder ? "-1" : "0") + "\"" + sTooltipAttr + ">";
