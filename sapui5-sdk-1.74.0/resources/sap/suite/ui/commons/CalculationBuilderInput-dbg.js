sap.ui.define([
...
], function (jQuery, library, Control, CalculationBuilderItem, List, ListMode, StandardListItem, Popover, PlacementType,
			 Button, ButtonType, Menu, MenuButton, MenuItem, OverflowToolbar, ToolbarSeparator, ToolbarSpacer, Title, OverflowToolbarToggleButton,
			 CustomData, Sorter, JSONModel, Device, containsOrEquals) {
	"use strict";
	...
	var CalculationBuilderInput = Control.extend("sap.suite.ui.commons.CalculationBuilderInput", {
		...
		renderer: function (oRm, oInput) {
			...
			oRm.write("<div aria-label=\"\" aria-describedby=\"" + oInput.getId() + "-error\" spellcheck=\"false\"" + sEditable + " id=\"" + oInput.getId() + "-input\" class=\"sapCalculationBuilderInput\"></div>");

			oRm.write("<div id=\"" + oInput.getId() + "-error\" class=\"sapCalculationBuilderInputErrorArea\"></div>");
	...
	CalculationBuilderInput.prototype._recreateText = function (mParameters) {
		...
		var sHtml = "", sText,
			...
			iItemIndex = 0,
			bAllowStringConstants = this.getParent().getAllowStringConstants(),
			bConstant = false,
		...
		for (var i = 0; i < sText.length; i++) {
			...
			if (sChar === "\"" && bAllowStringConstants) {
				bConstant = !bConstant;
			}
	...
	CalculationBuilderInput.prototype._stringToItems = function (sExpression) {
		...
		for (; iChar < sExpression.length; iChar++) {
			...
			if (sChar === "\"" && this.getParent().getAllowStringConstants()) {
				sItem += "\"";
				if (bConstant) {
