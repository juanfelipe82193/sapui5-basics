...
(function () {
        ...
        for (i = 0; i < events.length; ++i) {
            ...
            if (event.index !== index) {
                ...
                if (activeHighlights.length > 0) {
                    ...
                    highlight.typeMap = assembleTypesAndTexts(activeHighlights);
                }
                index = event.index;
            }
    ...
    var assembleTypesAndTexts = function (activeHighlights) {
        var typeMap = {};
        for (var i = 0; i < activeHighlights.length; ++i) {
            var highlight = activeHighlights[i];
            var textMap = typeMap[highlight.type];
            if (!textMap) {
                textMap = {};
                typeMap[highlight.type] = textMap;
            }
            var text = textMap[highlight.text];
            if (!text) {
                textMap[highlight.text] = true;
            }
        }
        return typeMap;
    };
