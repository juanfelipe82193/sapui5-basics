sap.ui.define([
	...
	return ManagedObject.extend("sap.ui.mdc.sample.filterbar.sample2.mockserver.mockServer", {

		started: null,
		init: function () {
			var mockData, metadata;

			this.started = jQuery.get("./../sample2/mockserver/ProductList.json").then(function (data, status, jqXHR) {
				mockData = data;
				return jQuery.get("./../sample2/mockserver/metadata.xml");
					...
					if (rExp.test(sODataQueryValue)) {
						...
						for (var i = 1; i < aParts.length; i++) {
							...
							if (sOperator === "or") {
								...
								aResult = aResult.concat(aSet2.filter(function (oEntry) {
									return aResult.indexOf(oEntry) < 0;
								}));
