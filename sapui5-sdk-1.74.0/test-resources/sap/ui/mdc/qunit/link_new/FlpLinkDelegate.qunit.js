...
sap.ui.define([
	...
	QUnit.test("Instance", function (assert) {
		...
		assert.deepEqual(this.oLink.getLinkItems(), []);
		assert.deepEqual(this.oLink.getSourceControl(), null);
	});

	QUnit.module("sap.ui.mdc.Link: 'semanticObjects'", {
		beforeEach: function () {
	...
	QUnit.test("semanticObjects and semanticObjectMapping with empty collection", function (assert) {
		...
		this.oLink.oDelegatePromise.then(function () {
			var oDELEGATE = this.oLink.DELEGATE;
	...
	QUnit.module("sap.ui.mdc.Link: calculateSemanticAttributes", {
		beforeEach: function () {
			this.oContextObject = {
				Category: "Monitor",
	...
	var _calculateSemanticAttributesOfLink = function (oLink, oContextObject) {
		return oLink.oDelegatePromise.then(function () {
			return oLink.DELEGATE._calculateSemanticAttributes(oContextObject, oLink.getDelegate().payload, oLink._getInfoLog());
		});
	};
	...
	QUnit.module("sap.ui.mdc.Link: 'retrieveNavigationTargets'", {
		beforeEach: function () {
			this.oLink;
	...
	QUnit.test("CrossApplicationNavigation service not available", function (assert) {
		...
		this.oLink.oDelegatePromise.then(function () {
			this.oLink.DELEGATE._retrieveNavigationTargets("", {}, this.oLink.getDelegate().payload).then(function (aLinks, oOwnNavigationLink) {
	...
	QUnit.module("sap.ui.mdc.Link: 'retrieveNavigationTargets' and main item", {
		beforeEach: function () {
			this.oLink;
			FakeFlpConnector.enableFakeConnector({
				SemanticObjectDisplayFactSheet: {
	...
	QUnit.module("sap.ui.mdc.Link: 'semanticObjectUnavailableActions'", {
		beforeEach: function () {
			this.oLink;
			FakeFlpConnector.enableFakeConnector({
				SemanticObjectTwoIntents: {
	...
	QUnit.test("retrieveNavigationTargets", function (assert) {
		...
		// act
		this.oLink.oDelegatePromise.then(function () {
			this.oLink.DELEGATE._retrieveNavigationTargets("", {
				"SOMainActionWithText": {}
			}, this.oLink.getDelegate().payload).then(function () {
				// assert
	...
	QUnit.test("determineItems", function (assert) {
		...
		// act
		this.oLink.oDelegatePromise.then(function () {
			this.oLink._useDelegateItems().then(function () {
				// assert
				assert.equal(SapBaseLog.getLevel(), SapBaseLog.Level.TRACE);
				assert.ok(this.oLink._getLogFormattedText().indexOf("SOMainActionWithText") > -1);
