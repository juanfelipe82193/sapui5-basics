			...
			return this.waitFor({
				...
				success: function(aLists) {
					var aItems = aLists[0].getItems().filter(function(oItem) {
						return oItem.getContent()[0].getText() === sItemText;
					});
					Opa5.assert.equal(aItems.length, 1);
					Opa5.assert.ok(aItems[0].getContent()[0]._getCompositeAggregation().getContent()[0].getItems()[1].getItems()[0].isA("sap.m.Link"), "List item contains sap.m.Link");
					Opa5.assert.equal(aItems[0].getContent()[0]._getCompositeAggregation().getContent()[0].getItems()[1].getItems()[0].getVisible(), true, "sap.m.Link is visible");
		...
		iShouldSeeLinkItemAsEnabled: function(sItemText, bEnabled) {
			return this.waitFor({
				...
				success: function(aLists) {
					...
					Opa5.assert.equal(aItems[0].getContent()[0]._getCompositeAggregation().getContent()[0].getItems()[1].getItems()[0].getEnabled(), bEnabled, sItemText + " is " + (bEnabled ? "enable" : "disabled"));
				}
			});
		},
		...
		iShouldSeeLinkItemOnPosition: function(sItemText, iIndex) {
			return this.waitFor({
				...
				success: function(aLists) {
					...
					Opa5.assert.equal(aItems.length, 1);
					Opa5.assert.ok(aItems[0].getContent()[0]._getCompositeAggregation().getContent()[0].getItems()[1].getItems()[0].isA("sap.m.Link"), "List item contains sap.m.Link");
					Opa5.assert.ok(aItems[0].getContent()[0]._getCompositeAggregation().getContent()[0].getItems()[1].getItems()[0], "sap.m.Link exists");
					Opa5.assert.equal(aItems[0].getContent()[0]._getCompositeAggregation().getContent()[0].getItems()[1].getItems()[0].getVisible(), true, "sap.m.Link is visible");
