...
sap.ui.define([
	...
	QUnit.test("setFilterbar", function(assert) {
		...
		var spy = sinon.spy(oValueHelpPanel._oFilterSplitter, "invalidate");
		qutils.triggerKeyboardEvent(oValueHelpPanel._oAdvButton.getFocusDomRef().id, jQuery.sap.KeyCodes.ENTER, false, false, false);
		assert.equal(spy.callCount, 1, "_oFilterSplitter.invalidate should be called");
		assert.notOk(oValueHelpPanel._oFilterVBox.getVisible(), "VBox for FilterBar invisible");
		spy.restore();
		...
		var oCloseButton = oValueHelpPanel.byId("closeAdvancedFilter");
		qutils.triggerKeyboardEvent(oCloseButton.getFocusDomRef().id, jQuery.sap.KeyCodes.ENTER, false, false, false);
		assert.notOk(oValueHelpPanel._oAdvButton.getPressed(), "Toggle button is not pressed");
		assert.notOk(oValueHelpPanel._oFilterVBox.getVisible(), "VBox for FilterBar invisible");
