...
sap.ui.define([
	...
	QUnit.module("Default type", {
		beforeEach: function() {
			oConditionsType = new ConditionsType({operators: ["EEQ"]});
		},
		afterEach: function() {
	...
	QUnit.test("Formatting: array of conditions", function(assert) {
		...
		oConditionsType.setFormatOptions({operators: ["EEQ"], maxConditions: -1});
	...
	QUnit.test("Formatting: simple Integer", function(assert) {
		...
		oConditionsType.setFormatOptions({operators: ["EEQ"], valueType: oValueType});
	...
	QUnit.test("Parsing: unsing condition from navigation", function(assert) {
		...
		var oNavigateCondition = Condition.createCondition("EEQ", ["I3"], {testIn: "A"}, {testOut: "B"});
		oConditionsType.setFormatOptions({operators: ["EEQ"], navigateCondition: oNavigateCondition});
	...
	QUnit.module("Async", {
		beforeEach: function() {
			oConditionsType = new ConditionsType({operators: ["EEQ"], asyncParsing: fnAsync});
		},
		afterEach: function() {
			oConditionsType.destroy();
			oConditionsType = undefined;
	...
	QUnit.test("Formatting", function(assert) {
		...
		var oCondition1 = Condition.createCondition("EEQ", ["I1"]);
		var oCondition2 = Condition.createCondition("EEQ", ["I2"]);
		var oCondition3 = Condition.createCondition("EEQ", ["I3"]);
	...
	QUnit.test("unit parsing", function(assert) {
		...
		assert.equal(aConditions[0].values.length, 2, "Condition 0: Values length");
		assert.equal(aConditions[0].operator, "EEQ", "Condition 0: Operator");
	...
	QUnit.test("unit Parsing unsing condition from navigation", function(assert) {
		...
		var oNavigateCondition = Condition.createCondition("EEQ", [[1, "USD"]], {testIn: "A"}, {testOut: "B"});
