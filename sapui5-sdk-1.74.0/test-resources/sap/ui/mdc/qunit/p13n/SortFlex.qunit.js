	...
	QUnit.test("addSort", function(assert) {
		...
		return ChangesWriteAPI.create({
			...
			this.fAddSort(oChange, this.oTable, {
				...
				var aSorters = Object.keys(oSortConditions);

				assert.equal(aSorters.length, 1, "one sorter has been created");
				assert.equal(aSorters[0], "Category", "correct sorter has been created");
				assert.equal(oSortConditions[aSorters[0]].descending, false, "correct sort order has been created");
			...
			this.fAddSort(oChange, this.oTable, {
				...
				return ChangesWriteAPI.create({
					...
					this.fRemoveSort(oChange, this.oTable, {
						...
						assert.equal(oSortConditions, null, "sort conditions is null");
