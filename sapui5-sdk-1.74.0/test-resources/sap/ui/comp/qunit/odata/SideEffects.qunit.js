...
(function() {
	...
	sap.ui.define([
		...
		QUnit.module("sap.ui.comp.odata.SideEffects", {
			beforeEach: function () {
				this.oSideEffects = new SideEffects();
			},
			afterEach: function () {
		...
		QUnit.test("_getForPath", function (assert) {
			var mAnnotation = this.oSideEffects._getForPath(oEntitySet, "TaxAmount");
			assert.ok(mAnnotation["com.sap.vocabularies.Common.v1.SideEffects#V_MANDATORY_ATTRIBUTES"]);

			mAnnotation = this.oSideEffects._getForPath(oEntitySet, "TaxAmount1");
			assert.ok(!mAnnotation["com.sap.vocabularies.Common.v1.SideEffects#V_MANDATORY_ATTRIBUTES"]);

			mAnnotation = this.oSideEffects._getForPath(oEntitySet, "Item/ScheduleLine");
			assert.ok(mAnnotation["com.sap.vocabularies.Common.v1.SideEffects#D_DURABLE_LOCK_CLEANUP_FAIL_SAVE"]);
		});

		QUnit.test("getForPath", function (assert) {
			var mAnnotation, oMetaData = {
				entitySet: oEntitySet,
				entityType: oEntityType,
				complexType: oComplexType
			};

			mAnnotation = this.oSideEffects.getSideEffects("TaxAmount", "ValidationResult/IsValid", oMetaData);
			assert.ok(mAnnotation.entitySet["com.sap.vocabularies.Common.v1.SideEffects#V_MANDATORY_ATTRIBUTES"]);
			assert.ok(!mAnnotation.entityType["com.sap.vocabularies.Common.v1.SideEffects#V_MANDATORY_ATTRIBUTES"]);
			assert.ok(!mAnnotation.complexType["com.sap.vocabularies.Common.v1.SideEffects#V_MANDATORY_ATTRIBUTES"]);
		});
