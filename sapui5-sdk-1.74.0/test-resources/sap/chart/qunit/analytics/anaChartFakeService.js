var _QUERY_REG = /(.*\$select=)([^&]*)(.*)/;

function sortQuery(uri) {
	var patterns = _QUERY_REG.exec(uri);
	if (patterns && patterns[2]) {
		return patterns[1] + patterns[2].split(",").sort().join(",") + patterns[3];
	} else {
		return uri;
	}
}
 ...
 */
var anaChartFakeService =  {
	// default base URL
	baseURI: "http://anaChartFakeServiceForAnalyticalChart:8080/",
	...
	addResponse: function (oResponse) {
		...
		var uri = oResponse.uri;
		if (uri instanceof Array) {
			uri = uri.map(sortQuery);
		} else {
			uri = sortQuery(uri);
		}
		oResponse.uri = uri;
		...
		for (var i = 0; i < this._aResponses.length; i++) {
			...
			if (bBatch && oResponse.batch) {
				...
				for (var j = 0; j < oResponse.uri.length; j++) {
					...
					var sRequestedQuery = sortQuery(oRequest.requestBody);
					//count how many sub requests match with the batch request
					if (sRequestedQuery.indexOf(sSingleUriFromBatch) !== -1) {
	 ...
	 * default is: http://anaChartFakeService:8080/
	 */
