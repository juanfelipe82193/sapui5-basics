var QUICKOVERVIEW_ID = "qvPopover-popover";
if (sap.ui.Device.system.phone) QUICKOVERVIEW_ID = "qvPopover-dialog";

var oEmployeeBusinessCard = new sap.m.Button({
	text : "Employee Business Card",
	press : function() {
		// require EmployeeLaunch and 
...
//var sImgPathC = jQuery.sap.getModulePath("sap/sil/ref09") + "/img/accenture.png";
		...
		oEmployeeLaunch.openBy(oEmployeeBusinessCard);
	}
});


/////////////////
//Testing Part: Employee Business Card
////////////////

module("Initial Check - Employee Business Card");

qunithelper.checkDialog_beforeInitialized(QUICKOVERVIEW_ID, "Employee");

module("Open and Close Employee Business Card");

asyncTest("Open Dialog", function(){
	oEmployeeBusinessCard.firePress();
	setTimeout(function(){
		qunithelper.checkDialog_isVisible(QUICKOVERVIEW_ID, "", "20em");
		start();
	}, 1000);
});

if (sap.ui.Device.system.phone) {
	qunithelper.closeDialog_andCheck(QUICKOVERVIEW_ID, "qvPopover-closeButton");
} else {
	asyncTest("Close Employee Business Card", function(){
		var oQuickOverview = sap.ui.getCore().byId(QUICKOVERVIEW_ID);
		oQuickOverview.close();
		setTimeout(function(){
			var $dialog = jQuery.sap.byId(QUICKOVERVIEW_ID);
			equal($dialog.css("visibility"), "hidden", "Employee Business Card is closed and invisible");
			start();
		}, 500);
	});	
}

module("Initial Check - Employee Business Card and Navigating to Company Business Card");
...
asyncTest("Nav to Company Business Card", function(){
	var oCompanyLink = sap.ui.getCore().byId("sap.ca.ui.quickoverview.Employee--qvCompLink");
	oCompanyLink.firePress();
	setTimeout(function(){
		qunithelper.checkDialog_isVisible(QUICKOVERVIEW_ID, "", "20em");
		start();
	}, 1000);
...
if (sap.ui.Device.system.phone) {
	...
	asyncTest("Close Company Business Card", function(){
			var oQuickOverview = sap.ui.getCore().byId(QUICKOVERVIEW_ID);
			oQuickOverview.close();
			setTimeout(function(){
				var $dialog = jQuery.sap.byId(QUICKOVERVIEW_ID);
				equal($dialog.css("visibility"), "hidden", "Company Business Card is closed and invisible");
				start();
			}, 500);
		});	
	}