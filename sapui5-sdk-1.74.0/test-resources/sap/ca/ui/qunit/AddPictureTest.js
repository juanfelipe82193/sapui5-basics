jQuery.sap.require("sap.ca.ui.AddPicture");
window.addEventListener("load", function () {
    var ADD_PICTURE_ID = "CA_VIEW_ADDPIC--ADDPIC";
    // todo: isLimitReached is useless
    var isLimitReached = false;
    var global__isLimitReached2 = false;
    var global__isShow = false;
    var customButtonText = "Custom Button Text";
    var customWidth = "50%";
    var pictureAdded = false;
    var global__pictureAdded2 = false;
    var image1 = "data:image/png;base64,";
    var image2 = "data:image/png;base64,";

// --------------------------------------------

// Add button tab style enabled.
    var oAppPicture1 = new sap.ca.ui.AddPicture({id: ADD_PICTURE_ID});
    oAppPicture1.setButtonPageType(sap.ca.ui.AddPicture.BUTTON_PAGE_TYPE.TAB);

    var oModel1 = new sap.ui.model.json.JSONModel();
    oModel1.setData({
        provider: [
            {
                id: 'PICTUREITEM1',
                base64content: image1
            },
            {
                id: 'PICTUREITEM2',
                base64content: image2
            }
        ]
    });
    var oPictureItemTemplate1 = new sap.ca.ui.PictureItem({status: sap.ca.ui.PictureItem.STATUS.ADD});
    oPictureItemTemplate1.bindProperty("source", "base64content");
    oAppPicture1.setModel(oModel1);
    oAppPicture1.bindAggregation("pictures", {path: "/provider", template: oPictureItemTemplate1});
    oAppPicture1.attachShow(
        function () {
            global__isShow = true;
        }
    );

// --------------------------------------------

// Add button tab style disabled (editable=false).
    var oAppPicture2 = new sap.ca.ui.AddPicture({id: ADD_PICTURE_ID + '2'});
    oAppPicture2.setButtonPageType(sap.ca.ui.AddPicture.BUTTON_PAGE_TYPE.TAB);
    oAppPicture2.setEditable(false);

// --------------------------------------------

// Add button form style enabled.
    var oAppPicture3 = new sap.ca.ui.AddPicture({id: ADD_PICTURE_ID + '3'});
    oAppPicture3.setButtonPageType(sap.ca.ui.AddPicture.BUTTON_PAGE_TYPE.FORM);
    var oModel3 = new sap.ui.model.json.JSONModel();
    oModel3.setData({
        provider: [
            {
                id: 'PICTUREITEM3_1',
                base64content: image1
            },
            {
                id: 'PICTUREITEM3_2',
                base64content: image2
            }
        ]
    });
    var oPictureItemTemplate3 = new sap.ca.ui.PictureItem({status: sap.ca.ui.PictureItem.STATUS.ADD});
    oPictureItemTemplate3.bindProperty("source", "base64content");
    oAppPicture3.setModel(oModel3);
    oAppPicture3.bindAggregation("pictures", {path: "/provider", template: oPictureItemTemplate3});

// --------------------------------------------

// Add button form style disabled by a max limit reached.
    var oAppPicture4 = new sap.ca.ui.AddPicture({id: ADD_PICTURE_ID + '4'});
    oAppPicture4.setButtonPageType(sap.ca.ui.AddPicture.BUTTON_PAGE_TYPE.FORM);

// set a max picture value to fire the event that disable the button
    oAppPicture4.setMaxPictureNumber(2);

// attach a flag to maxPictureLimitReached event
    oAppPicture4.attachMaxPictureLimitReached(
        function () {
            global__isLimitReached2 = true;
        }
    );

    var oPictureItemTemplate4 = new sap.ca.ui.PictureItem({status: sap.ca.ui.PictureItem.STATUS.ADD});
    oPictureItemTemplate4.bindProperty("source", "base64content");
    var oModel4 = new sap.ui.model.json.JSONModel();
    oModel4.setData({
        provider: [
            {
                id: 'PICTUREITEM_4_1',
                base64content: image1
            },
            {
                id: 'PICTUREITEM_4_2',
                base64content: image2
            }
        ]
    });
    oAppPicture4.setModel(oModel4);
    oAppPicture4.bindAggregation("pictures", {path: "/provider", template: oPictureItemTemplate4});

// --------------------------------------------

// Add button text and width properties methods.
    var oAppPicture5 = new sap.ca.ui.AddPicture({id: ADD_PICTURE_ID + '5'});
    oAppPicture5.setText(customButtonText);
    oAppPicture5.setWidth(customWidth);

// attach a flag to pictureAdded event
    oAppPicture5.attachPictureAdded(
        function () {
            pictureAdded = true;
        }
    );
// --------------------------------------------

// Add button text and width properties methods in constructor.
    var oAppPicture6 = new sap.ca.ui.AddPicture({id: ADD_PICTURE_ID + '6', text: customButtonText, width: customWidth});

// attach a flag to pictureAdded event
    oAppPicture6.attachPictureAdded(
        function () {
            global__pictureAdded2 = true;
        }
    );

    oAppPicture1.placeAt("contentHolder1");
    oAppPicture2.placeAt("contentHolder2");
    oAppPicture3.placeAt("contentHolder3");
    oAppPicture4.placeAt("contentHolder4");
    oAppPicture5.placeAt("contentHolder5");
    oAppPicture6.placeAt("contentHolder6");

///////////////
//Testing Part: Add Picture
///////////////
    module("AddPicture - Properties tests");

    test("AddPicture - Creation ", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID);
        notEqual(oAppPicture, undefined, "AddPicture control not found");
    });

    test("AddPicture - Check model content ", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID);
        notEqual(oAppPicture.getPictures(), undefined, "The AddPicture control should contain a picture aggregation");
        equal(oAppPicture.getPictures().length, 2, "The AddPicture control aggregation should have 2 pictures binded");
    });

    test("AddPicture - maxPicture Number. Check Limit default property. Check event was not fired", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID);
        equal(oAppPicture.getMaxPictureNumber(), '10', "The AddPicture maxPictureNumber default value should be 10");
        equal(isLimitReached, false, "The maxPictureLimitReached event should not be fired");
    });

    test("AddPicture - default button text.", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID);
        notEqual(oAppPicture.getText(), "", "The AddPicture button text should be not be empty");
        var oButton = oAppPicture._getButton();
        equal(oAppPicture.getText(), oButton.getText(), "Property and button text should be equal");
    });

    test("AddPicture - custom button text (method call)", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID + '5');
        equal(oAppPicture.getText(), customButtonText, "The AddPicture button text should be '" + customButtonText + "'");
        var oButton = oAppPicture._getButton();
        equal(oAppPicture.getText(), oButton.getText(), "Property and button text should be equal");
        equal(oButton.getText(), customButtonText, "Button text is set correctly");
    });


    test("AddPicture - default width.", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID);
        equal(oAppPicture.getWidth(), "100%", "The AddPicture width should be 100%");
    });

    test("AddPicture - custom width (method call)", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID + '5');
        equal(oAppPicture.getWidth(), customWidth, "The AddPicture width should be '" + customWidth + "'");
    });

    test("AddPicture - custom button text (constructor)", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID + '6');
        equal(oAppPicture.getText(), customButtonText, "The AddPicture button text should be '" + customButtonText + "'");
        var oButton = oAppPicture._getButton();
        equal(oAppPicture.getText(), oButton.getText(), "Property and button text should be equal");
        equal(oButton.getText(), customButtonText, "Button text is set correctly");
    });

    test("AddPicture - custom width (constructor)", function () {
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID + '6');
        equal(oAppPicture.getWidth(), customWidth, "The AddPicture width should be '" + customWidth + "'");
    });

    module("AddPicture - Events tests");

    test("AddPicture - maxPicture Number. Check Limit property has been reached. Check event was fired", function (assert) {
        var done = assert.async();
        assert.expect(2);
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID + '4');
        equal(oAppPicture.getMaxPictureNumber(), '2', "The AddPicture maxPictureNumber value should be 2");
        // Try to add a picture using the button, this should fire the reached limit event
        var oButton = oAppPicture._getButton();
        oButton.firePress();
        // allow 1/2 second before checking
        setTimeout(function () {
                ok(global__isLimitReached2, "The expected maxPictureLimitReached event should be fired");
                done();
            },
            500);
    });

    test("AddPicture - Test show event. Check event was fired", function (assert) {
        var done = assert.async();
        assert.expect(1);
        var oImage = sap.ui.getCore().byId("__item0-CA_VIEW_ADDPIC--ADDPIC-0-image");
        oImage.firePress();
        // allow 1/2 second before checking
        setTimeout(function () {
                ok(global__isShow, "The expected show event should be fired");
                done();
            },
            500);
    });

    test("AddPicture - Test pictureAdded event. Check event was not fired", function () {
        equal(pictureAdded, false, "The AddPicture event should not have fired.  'pictureAdded' should be false.");
    });

    test("AddPicture - Test show event. Check event was fired", function (assert) {
        var done = assert.async();
        assert.expect(1);
        var oAppPicture = sap.ui.getCore().byId(ADD_PICTURE_ID + '6');
        oAppPicture.attachPictureAdded(function () {
            ok(global__pictureAdded2, "The AddPicture event should have fired.");
            done();
        });
        // add a new picture item, this will fire the event
        oAppPicture._createAndAddPictureItem(image1, "Image1");
    });

    module("AddPicture - Design tests");

    test("AddPicture - Test Horizontal Ruler after Button (Tab display mode only)", function () {
        var oHorizontalRuler = window.document.getElementsByClassName("sapCaAPAddButton")[0].getElementsByClassName("sapCaUiHorizontalRuler")[0];
        notEqual(oHorizontalRuler, '<hr class="sapCaUiHorizontalRuler">', "Tab display mode with pictures: horizontal ruler should be placed after an enabled button");
        var oHorizontalRuler2 = window.document.getElementById("contentHolder2").getElementsByClassName("sapCaUiHorizontalRuler")[0];
        equal(oHorizontalRuler2, undefined, "Tab display mode: no horizontal ruler should be placed after a disabled Button");
        var oHorizontalRuler3 = window.document.getElementsByClassName("sapCaAPAddButton")[1].getElementsByClassName("sapCaUiHorizontalRuler")[0];
        equal(oHorizontalRuler3, undefined, "Form display mode: no horizontal ruler should be placed after an enabled Button");
        var oHorizontalRuler4 = window.document.getElementsByClassName("sapCaAPAddButton")[2].getElementsByClassName("sapCaUiHorizontalRuler")[0];
        equal(oHorizontalRuler4, undefined, "Form display mode: no horizontal ruler should be placed after a disabled Button");
    });
});
