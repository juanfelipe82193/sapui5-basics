...
sap.ui.controller("views.sample.chart.chartFormatterSample", {
    onInit: function () {
        ...
        this.oToolBar = this.byId("chartToolBar");
...
//                date: 1374422492000,
        ...
        this._oModelAPIs = new sap.ui.model.json.JSONModel();
        this._oModelAPIs.setData({
            apis: [
                {name: "Test some ChartToolBar APIs", key: ""},
                {name: "Get select ids", key: "SELECT_AS_ASSOCIATION"},
                {name: "Toggle full screen", key: "TOGGLE_FULL_SCREEN"},
                {name: "Chart type update", key: "CHART_TYPE"}
            ]
        });
        ...
        this.chart3.attachSelectDataPoint(jQuery.proxy(this._onSelectDataPoint,this));
        ...
        this.getView().setModel(this._oModelAPIs, "apis");
    ...
    onPersonalizationPressed: function (oEvent) {
        var selectionMode;
        if (this.chart1.getSelectionMode() === sap.ca.ui.charts.ChartSelectionMode.Multiple.toLowerCase()) {
            selectionMode = sap.ca.ui.charts.ChartSelectionMode.Single;
            sap.m.MessageToast.show("Single selection mode activated");
        } else {
            selectionMode = sap.ca.ui.charts.ChartSelectionMode.Multiple;
            sap.m.MessageToast.show("Multiple selection mode activated");
        }

        this.chart1.setSelectionMode(selectionMode);
        this.chart2.setSelectionMode(selectionMode);
        this.chart3.setSelectionMode(selectionMode);
        this.chart4.setSelectionMode(selectionMode);
        this.chart5.setSelectionMode(selectionMode);
        this.chart6.setSelectionMode(selectionMode);
        this.chart7.setSelectionMode(selectionMode);
        this.chart8.setSelectionMode(selectionMode);
        this.chart9.setSelectionMode(selectionMode);
    },

    onFirstDimensionKeySelected: function (oEvent) {
        alert("user is selecting " + oEvent.getParameter("selectedKey"));
    },

    onSecondDimensionKeySelected: function (oEvent) {
        switch (oEvent.getParameter("selectedKey")) {
            case "SELECT_AS_ASSOCIATION":

                var oSettings = {
                    type : sap.ca.ui.message.Type.SUCCESS,
                    message : "Select Ids: " + this.oToolBar.getFirstDimensionSelect() + " and " + this.oToolBar.getSecondDimensionSelect()
                };

                sap.ca.ui.message.showMessageBox(oSettings);
                break;
            case "TOGGLE_FULL_SCREEN":
                var bIsFullScreen = this.oToolBar.getFullScreen();
                this.oToolBar.setFullScreen(! bIsFullScreen);
                break;
            case "CHART_TYPE":
                var aCharts = this.oToolBar.removeAllCharts();

                //switch 2 charts
                var tmpChart = aCharts[0];
                aCharts[0] = aCharts[1];
                aCharts[1] = tmpChart;

                for (var i=0; i < aCharts.length; i++) {
                    this.oToolBar.addChart(aCharts[i]);
                }
        ...
        var oSelect = sap.ui.getCore().byId(this.oToolBar.getSecondDimensionSelect());
    ...
    _onListTap: function (oEvent) {
        ...
        if(this.chartPopover) {
            this.chartPopover.close();
        }
    },
    _onSelectDataPoint: function(oEvent){
        this.chartPopover = oEvent.getParameter("popover");
    }
});
