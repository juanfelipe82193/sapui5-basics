...
sap.ui.controller("sap.ca.ui.sample.views.control.FileUpload", {
    ...
    onInit: function () {
        ...
        util.UiFactory.fillPageHeader(page, this.getView(), util.Title.FILE_UPLOAD, "sap.ca.ui.FileUpload");
        ...
        // Regression test
        var oLbl = oFileUpload.getAttachmentNumberLabel();
        jQuery.sap.log.info(oLbl.getText);
    },
    ...
    updateProperty: function (oEvent) {
        var fileUploadControl = this.byId("fileupload");
        fileUploadControl[oEvent.getSource().data().setter].call(fileUploadControl, oEvent.getParameter("state"));
    },
    updateCompactMode: function (oEvent) {
        var oPage = this.byId("page");
        oPage.toggleStyleClass("sapUiSizeCompact", oEvent.getParameter("state"));
    },
    embedInForm:function(oEvent){
        var bEmbed = oEvent.getParameter("state");
        var fileUploadControl = this.byId("fileupload");
        var oForm = this.byId("form");
        var oPage = this.byId("page");
        if( bEmbed && (fileUploadControl.getParent()==oPage)){
            oPage.removeContent(fileUploadControl);
            oForm.addContent(fileUploadControl);
        }  else {
            oForm.removeContent(fileUploadControl);
            oPage.insertContent(fileUploadControl,0);
        }
    }
