...
function generateChartConfigList(oList, oChart) {
	...
	//Range Selector StartPosition
	oListItem = new sap.m.InputListItem({label:"Range Selector Start Position"});
	var oInput = new sap.m.Input({
		type: sap.m.InputType.Number,
		value : oChart.getRangeSelectorStartPosition(),
		change : function() {
			oChart.setRangeSelectorStartPosition(parseFloat(this.getValue()));
		}
	});
...
function generateCategoryAxisConfigList(oList, oChart) {
	...
	//Number of Visible CATEGORY 
	oListItem = new sap.m.InputListItem({label:"Number Of Visible Cateogories"});
	var oInput = new sap.m.Input({
		type: sap.m.InputType.Number,
		value : oChart.getNumberOfVisibleCategories(),
		change : function() {
			oChart.setNumberOfVisibleCategories(parseFloat(this.getValue()));
		}
	});
	oListItem.addContent(oInput);
	oList.addItem(oListItem);
