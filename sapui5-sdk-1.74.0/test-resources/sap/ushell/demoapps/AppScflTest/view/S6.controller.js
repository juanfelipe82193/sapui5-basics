...
sap.ca.scfld.md.controller.BaseDetailController.extend("AppScflTest.view.S6", {
	...
	onInit : function() {
		this.oRouter.attachRoutePatternMatched(function(oEvent) {
			if (oEvent.getParameter("name") === "Invoice") {
				...
				this.sDocType = (oParams && oParams.doctype) ? oParams.doctype : this.oApplicationFacade.getResourceBundle().getText("XTIT_SupplierInvoice") ;
				var oObjectHeader = this.byId("InvoiceObjectHeader");
				if (oObjectHeader)
				...
				if (this.oApplicationFacade.isMock())
					this.getView().bindElement("/InvoiceHeaders(SAP__Origin='" + this.sOrigin + "',Year='" + this.sYear + "',InvoiceNumber='" + this.sDocId + "')?$expand=InvoiceItems");
				else
					this.getView().bindElement("/InvoiceHeaders(SAP__Origin='" + this.sOrigin + "',Year=" + this.sYear + ",InvoiceNumber='" + this.sDocId + "')?$expand=InvoiceItems");
			}
		}, this);
