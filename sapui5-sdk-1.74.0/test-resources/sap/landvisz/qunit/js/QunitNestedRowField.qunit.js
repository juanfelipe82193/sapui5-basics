...
sap.ui.define([
	...
	QUnit.module("NestedRowFieldCreation");
	QUnit.test("NestedRowFieldCreationOk", function(assert) {
		var oDomRef = window.document.getElementById("aminId-CLVEntityVLayoutProperties");
		...
		if(oLandEntity.getModel().getData().myHeaders[0].selected == true){
			...
			assert.ok("Nested Row field rendered");
			}
