...
sap.ui.define([
	...
	QUnit.test("CLVEntityActionsCreationOk", function(assert) {
		...
		assert.equal(oLandEntity.getModel().getData().actions[2].action , "paste", "Action 1 is paste");
		...
		assert.equal(oLandEntity.getModel().getData().actions[0].action , "Relations", "Action 3 is Relations");
	});
