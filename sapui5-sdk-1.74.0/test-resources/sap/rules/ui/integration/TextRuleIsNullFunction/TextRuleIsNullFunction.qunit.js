sap.ui.require([
	...
	"use strict";
		...
		Utils.startRequestRecorder({
					...
					filePath: "data/",
					fileName: "TextRuleIsNullFunction"
				});
	...
	var sViewName = "TextRuleIsNullFunction";
	Opa5.extendConfig({
		viewNamespace: "TextRuleIsNullFunction.view.",
		// we only have one view
		viewName: "TextRuleIsNullFunction",
		timeout: 20,
		pollingInterval: 100
	});
	...
	opaTest("TextRule :  Forming an AST Expression for ISNULL Function ", function (Given, When, Then) {
		...
		.iClickOnDisplayItemList("When: I Select ISNULL ", sViewName, "ISNULL").and
		...
		var sExpression = "ISNULL ( EmployeeDetails.EmpName ) ";
		Then.onTextRulePage.iCanSeeASTConditionText("Then: I read If condition", sViewName, sExpression, 0);
	});
	
	opaTest("TextRule :  Forming an AST Expression for ISNOTNULL Function ", function (Given, When, Then) {

			When.onTextRulePage.iPressCtrlSpaceInInput("When: I Press Ctrl+Space in Result ProductID", sViewName, 1,false);
		Then.onTextRulePage.iCanSeeDisplayItem("Then: I can see DiscountDetails,Discounts,EmployeeDetails,Employees in Vocabulary Panel", sViewName, ["DiscountDetails","Discounts","EmployeeDetails","Employees"]);
		When.onTextRulePage.iExpandPanelASTControl("When: I Expand Advanced Function Panel", sViewName, "Advanced Functions", true).and
		.iClickOnDisplayItemList("When: I Select ISNOTNULL ", sViewName, "ISNOTNULL").and
		.iClickOnDisplayItemList("When: I Select EmployeeDetails DataObject", sViewName, "EmployeeDetails").and
		...
		var sExpression = "ISNOTNULL ( EmployeeDetails.EmpAge + EmployeeDetails.EmpAge ) ";
