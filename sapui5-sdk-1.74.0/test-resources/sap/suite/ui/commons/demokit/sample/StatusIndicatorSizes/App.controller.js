sap.ui.define(
	...
	function ($, JSONModel, Controller, MessageToast) {
		...
		var VALUES = [51, 1, 99, 79];

		return Controller.extend("sap.suite.ui.commons.sample.StatusIndicatorSizes.App", {
			...
			onInit: function() {
				...
				aIndicators.forEach(function (aIndicators) {
					aIndicators.forEach(function (oIndicator, pos) {
						var sSvg = pos === 0 ? bulbSvg : cerealsSvg,
							oCustomShape = oIndicator.getGroups()[0].getShapes()[0];
						oCustomShape.setDefinition(sSvg);
						oIndicator.setValue(VALUES[pos]);
					});
				});
			},
			getAllStatusIndicators: function () {
				var oView = this.getView();
				return [
					oView.byId("smallIndicators").getItems(),
					oView.byId("mediumIndicators").getItems(),
					oView.byId("largeIndicators").getItems(),
					oView.byId("xlIndicators").getItems()
				];
			},
			valueChanged: function (oEvent) {
				var iValue = oEvent.getParameter("value"),
					aStatusIndicators = this.getAllStatusIndicators();

				aStatusIndicators.forEach(function (aStatusIndicators) {
					aStatusIndicators.forEach(function (oStatusIndicator) {
