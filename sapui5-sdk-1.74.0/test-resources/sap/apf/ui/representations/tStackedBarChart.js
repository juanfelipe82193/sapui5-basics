...
sap.ui.define("sap/apf/ui/representations/tStackedBarChart", [
	...
	"sap/viz/ui5/controls/VizFrame"
			...
			StackedBarChart, HorizontalLayout, VizFrame) {
	"use strict";
	...
	function _commonSetupForCreatingChart(api, requiredParameter) {
		...
		var chart = new StackedBarChart(interfaceProxy, requiredParameter);
	...
	BaseVizFrameChartRepresentationTestHelper.run(StackedBarChart);
	...
	QUnit.test("When Stacked Bar Chart is initialized", function(assert) {
		//assert
	...
	QUnit.module("Stacked Bar Chart Tests - Interaction on charts based on required property", {
		beforeEach : function(assert) {
			var testEnv = this;
			var done = assert.async();
	...
	QUnit.test("When Stacked Bar Chart is initialized with multiple parameters", function(assert) {
		//arrange
