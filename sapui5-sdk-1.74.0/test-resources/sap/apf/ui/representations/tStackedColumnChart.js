...
sap.ui.define("sap/apf/ui/representations/tStackedColumnChart", [
	...
	"sap/viz/ui5/controls/VizFrame"
			...
			StackedColumnChart, HorizontalLayout, VizFrame) {
	"use strict";
	...
	function _commonSetupForCreatingChart(api, requiredParameter) {
		...
		var chart = new StackedColumnChart(interfaceProxy, requiredParameter);
	...
	BaseVizFrameChartRepresentationTestHelper.run(StackedColumnChart);
	...
	QUnit.test("When Stacked Column Chart is initialized", function(assert) {
		//assert
	...
	QUnit.module("Stacked Column Chart Tests - Interaction on charts based on required property", {
		beforeEach : function(assert) {
			var testEnv = this;
			var done = assert.async();
			...
			createUiApiAsPromise().done(function(api) {
				...
				testEnv.requiredParameterForInteraction = representationHelper.representationDataWithDimension();
				testEnv.chart = _commonSetupForCreatingChart(api, testEnv.requiredParameterForInteraction);
	...
	QUnit.test("When there is no required filter", function(assert) {
		...
		assert.deepEqual(this.chart.chart.getVizProperties().interaction, oInteraction, "Then vizProperties are applied to the chart");
	});
	...
	QUnit.module("Stacked Column Chart Tests - With multiple parameters (dimensions and measures)", {
		beforeEach : function(assert) {
			...
			createUiApiAsPromise().done(function(api) {
				...
				testEnv.attachsSelectionFormatSpy = sinon.spy(BaseUI5ChartRepresentation.prototype, "attachSelectionAndFormatValue");
		...
		afterEach : function() {
			...
			this.attachsSelectionFormatSpy.restore();
			this.isAllMeasureSameUnitSpy.restore();

			_commonTearDown(this.api, this.chart);
		}
	});
	QUnit.test("When Stacked Column Chart is initialized with multiple parameters", function(assert) {
		//arrange
		...
		assert.strictEqual(this.attachsSelectionFormatSpy.calledOnce, true, "Then Required method called for selection format");
