...
sap.ui.define("sap/apf/ui/representations/tBarChart", [
...
], function(BarChart, Constants, BaseUI5ChartRepresentationTestHelper, BaseVizFrameChartRepresentationTestHelper,
			RepresentationHelper, createUiApiAsPromise, RepresentationInterfaceProxy, BaseUI5ChartRepresentation,
			BaseVizFrameChartRepresentation, HorizontalLayout, VizFrame) {
	"use strict";
	...
	function _commonSetupForCreatingChart(api, requiredParameter) {
		...
		var chart = new BarChart(interfaceProxy, requiredParameter);
	...
	BaseVizFrameChartRepresentationTestHelper.run(BarChart);
	...
	QUnit.module("Bar Chart Tests - Basic Check", {
		beforeEach : function(assert) {
			...
			createUiApiAsPromise().done(function(api) {
				...
				var requiredParameter = modifyParametersA(RepresentationHelper.prototype.representationDataWithDimension());
	...
	QUnit.test("When Bar Chart is initialized", function(assert) {
		//assert
		_commonInitializationAsserts(this.chart, assert);

		//arrange
		var vizPropOnChart = RepresentationHelper.prototype.getVizPropertiesJSONOnChart();
	...
	QUnit.test("When Bar Chart is initialized with multiple parameters", function(assert) {
		//arrange
		var requiredParameter = modifyParametersB(RepresentationHelper.prototype.representatationDataWithTwoDimensionAndMeasure());
	...
	QUnit.test("When custom text is set to filter", function(assert) {
		//arrange
		var requiredParameter = RepresentationHelper.prototype.representatationDataWithTwoDimensionAndMeasure();
